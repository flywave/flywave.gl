 ...
 */
function checkYear(sourceFile, [start, end], callback) {
    ...
    exec([
        ...
        if (error) {
            ...
            if (copyrightStart !== firstCommit ||
                ...
                const expected = `${firstCommit}${firstCommit !== currentYear ? "-" + currentYear : ""}`;
                if (found !== expected) {
 ...
 */
export function checkLicenses(sourceFiles, licenseRegEx, callback, fix = false) {
            ...
            checkYear(sourceFile, [match[1], match[2]], error => {
                if (error) {
                    ...
                    if (fix && errorMatch !== null) {
                        fs.writeFileSync(sourceFile, content.replace(licenseRegEx, match[0].replace(errorMatch[2], errorMatch[1])), { encoding: "utf8" });
                    }
                    else {
                        errors.push(error);
                    }
...
//# sourceMappingURL=checkLicenses.js.map