...
function project(lng, lat, config) {
    switch (config.name) {
        case 'mercator':
            return {
                x: lng / 360 + 0.5,
...
function projectAlbers(lng, lat, config) {
    var _a, _b;
    const center = (_a = config.center) !== null && _a !== void 0 ? _a : [-96, 37.5];
    const parallels = (_b = config.parallels) !== null && _b !== void 0 ? _b : [29.5, 45.5];
...
function unprojectAlbers(x, y, config) {
    var _a, _b;
    const center = (_a = config.center) !== null && _a !== void 0 ? _a : [-96, 37.5];
    const parallels = (_b = config.parallels) !== null && _b !== void 0 ? _b : [29.5, 45.5];
    const n = (Math.sin(degToRad(parallels[0])) + Math.sin(degToRad(parallels[1]))) / 2;
...
function projectEqualEarth(lng, lat) {
    const phi = degToRad(lat);
...
function unprojectEqualEarth(x, y) {
...
function projectNaturalEarth(lng, lat) {
    const phi = degToRad(lat);
    const lambda = degToRad(lng);
    const p2 = phi * phi;
    const p4 = p2 * p2;
...
function unprojectNaturalEarth(x, y) {
    const xn = (2 * x - 0.5) * Math.PI;
    const yn = (2 * (1 - y) - 1) * Math.PI;
    ...
    for (let i = 0; i < 25; i++) {
        const p2 = phi * phi;
        const p4 = p2 * p2;
        const delta = (phi * (1.007226 + p2 * (0.015085 + p4 * (-0.044475 + 0.028874 * p2 - 0.005916 * p4))) - yn) /
...
function projectWinkelTripel(lng, lat) {
    const phi = degToRad(lat);
    const lambda = degToRad(lng);
...
function unprojectWinkelTripel(x, y) {
    const xn = (2 * x - 0.5) * Math.PI;
    const yn = (2 * (1 - y) - 1) * Math.PI;
    const maxPhi = degToRad(MAX_LAT);
    let lambda = xn;
    ...
    for (let i = 0; i < 25; i++) {
        ...
        if (Math.abs(dlambda) < 1e-6 && Math.abs(dphi) < 1e-6)
            break;
    }
    return { lng: radToDeg(lambda), lat: radToDeg(phi) };
}
function projectLambert(lng, lat, config) {
    var _a, _b;
    const center = (_a = config.center) !== null && _a !== void 0 ? _a : [0, 30];
    const parallels = (_b = config.parallels) !== null && _b !== void 0 ? _b : [30, 30];
...
function unprojectLambert(x, y, config) {
    var _a, _b;
    const center = (_a = config.center) !== null && _a !== void 0 ? _a : [0, 30];
    const parallels = (_b = config.parallels) !== null && _b !== void 0 ? _b : [30, 30];
    const y0 = degToRad(parallels[0]);
    ...
    if (fy * n < 0)
        l -= Math.PI * Math.sign(xn) * Math.sign(fy);
...
function parseProjection(styleProjection) {
    var _a;
    if (!styleProjection)
        return { name: 'mercator' };
    if (typeof styleProjection === 'string')
        return { name: styleProjection };
    const name = (_a = styleProjection.name) !== null && _a !== void 0 ? _a : 'mercator';
    return {
        name: name,
...
function projectSphere(lng, lat) {
    const phi = degToRad(lat);
    const lambda = degToRad(lng);
    const r = 0.5;
    return {
...
//# sourceMappingURL=MBProjection.js.map