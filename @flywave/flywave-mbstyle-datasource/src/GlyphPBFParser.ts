...
function readSVarint(data: Uint8Array, offset: number): [number, number] {
...
function skipField(data: Uint8Array, offset: number, wireType: number): number {
    switch (wireType) {
        case 0: { const [, pos] = readVarint(data, offset); return pos; }
        case 1: return offset + 8;
        case 2: { const [, pos] = readBytes(data, offset); return pos; }
        case 5: return offset + 4;
        default: return data.length;
    }
}

export function parseGlyphPBF(data: ArrayBuffer | Uint8Array): ParsedFontstack | null {
    const bytes = data instanceof Uint8Array ? data : new Uint8Array(data);
    if (bytes.length === 0) return null;

    const fontstack: ParsedFontstack = {
        name: '',
        range: '',
        glyphs: new Map(),
    };
...
function parseFontstack(data: Uint8Array, fontstack: ParsedFontstack): void {
    let offset = 0;
...
function parseGlyph(data: Uint8Array): ParsedGlyph | null {
