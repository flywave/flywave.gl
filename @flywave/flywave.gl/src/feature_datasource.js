...
import GeoServerProvider, { GeoStoreProvider } from "./feature_provider";
...
class GeoServerFeatureDataSource extends FeaturesDataSource {
    constructor(options) {
        super({
            decoderUrl: config.DECODER_URL,
            workerTilerUrl: config.DECODER_URL,
            styleSetName: "geojson",
            url: options.dataUrl,
            apiFormat: APIFormat.XYZJson,
            minDataLevel: 16,
            maxDataLevel: 16,
            ...
            ...options,
            maxDisplayLevel: 25,
            dataProvider: options.dataUrl ? new GeoServerProvider(NAME, DEFAULT_GEOJSON, options) : new GeoStoreProvider(NAME, DEFAULT_GEOJSON, options)
        })
    }
}

export { GeoServerFeatureDataSource }

class GeoStoreFeatureDataSourceMock extends FeaturesDataSource {
    constructor(options) {
        super({
            decoderUrl: config.DECODER_URL,
            workerTilerUrl: config.DECODER_URL,
            ...
            addGroundPlane: false,
            ...options,
            dataProvider: new GeoStoreProvider(NAME, options.features || DEFAULT_GEOJSON, options)
        })
    }
}

export { GeoStoreFeatureDataSourceMock }