...
export class TilesRenderer extends TilesRendererBase {
	...
	preprocessNode(tile, parentTile, tileSetDir) {
		...
		if ('orientedBox' in tile.boundingVolume) {
			orientedBox = tile.boundingVolume.orientedBox.clone();
		}
	...
	tileInView(tile) {
		...
		if (sphere) {
			...
			for (let i = 0, l = cameraInfo.length; i < l; i++) {
				...
				if (frustum.intersectsSphere(sphere) && (!orientedBox ? true : orientedBox.intersects(frustum))) {
