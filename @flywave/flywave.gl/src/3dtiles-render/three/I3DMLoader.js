...
import { DefaultLoadingManager, Matrix4, InstancedMesh, Vector3, Quaternion, Scene } from 'three';
				...
				return new Promise((resolve, reject) => {
					...
					loader.parse(gltfBuffer, workingPath, model => {
						...
						instanceMap.forEach(({ instances, geometry, material }) => {
							...
							const instancedMesh = new InstancedMesh(geometry, material, INSTANCES_LENGTH * instances.length);
							...
							for (let i = 0; i < INSTANCES_LENGTH; i++) {
								...
								tempPos.copy(tempPos.sub(averageVector));
								...
								for (let j = 0, l = instances.length; j < l; j++) {
									...
									instancedMesh.setMatrixAt(i, tempMat.clone().multiply(instanceMat));
