...
import DeveloperError from "./developer-error.js";
...
var RadiansPerDegree = MathUtils.RADIANS_PER_DEGREE;
var RadiansPerArcSecond = MathUtils.RADIANS_PER_ARCSECOND;
var MetersPerAstronomicalUnit = 1.4959787e11; // IAU 1976 value

var perifocalToEquatorial = new THREE.Matrix3();
function elementsToCartesian(
  ...
  if (inclination < 0.0) {
    ...
    longitudeOfNode += MathUtils.PI;
  }

  //>>includeStart('debug', pragmas.debug);
  if (inclination < 0 || inclination > MathUtils.PI) {
  ...
  if (
    type === "Hyperbolic" &&
    Math.abs(MathUtils.negativePiToPi(trueAnomaly)) >=
  ...
  if (!defined(result)) {
    result = new THREE.Vector3(radius * costheta, radius * sintheta, 0.0);
  } else {
  ...
  return result.applyMatrix3(perifocalToEquatorial);
}
...
var keplerEqConvergence = MathUtils.EPSILON8;
...
function meanAnomalyToEccentricAnomaly(meanAnomaly, eccentricity) {
  ...
  var revs = Math.floor(meanAnomaly / MathUtils.TWO_PI);

  // Find angle in current revolution
  meanAnomaly -= revs * MathUtils.TWO_PI;
  ...
  eccentricAnomaly = iterationValue + revs * MathUtils.TWO_PI;
...
function eccentricAnomalyToTrueAnomaly(eccentricAnomaly, eccentricity) {
  ...
  var revs = Math.floor(eccentricAnomaly / MathUtils.TWO_PI);

  // Find angle in current revolution
  eccentricAnomaly -= revs * MathUtils.TWO_PI;
  ...
  trueAnomaly = MathUtils.zeroToTwoPi(trueAnomaly);
  if (eccentricAnomaly < 0) {
    trueAnomaly -= MathUtils.TWO_PI;
  }

  // Add on previous revolutions
  trueAnomaly += revs * MathUtils.TWO_PI;
...
function computeSimonEarth(date, result) {
  ...
  return result.multiplyScalar(factor);
}
...
var axesTransformation = new THREE.Matrix3().set(
  1.0000000000000002,
...
Simon1994PlanetaryPositions.computeSunPositionInEarthInertialFrame = function (
  ...
  result =result.copy(translation).negate();
