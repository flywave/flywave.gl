...
class ElevationProvider {
    ...
    getAtPoint(geoPoint: GeoCoordinates, defaultIfNotLoaded: ?number): number | null {
        ...
        const terrTile = this.dataSource.dataProvider().getBestAvailableTile(TileKeyUtils.geoCoordinatesToTileKey(
            this.dataSource.getTilingScheme(), geoPoint, this.dataSource.dataTerrainProvider._availability._maximumLevel));

        if (!(terrTile && terrTile.tinData)) { return defaultIfNotLoaded; }

        if (terrTile.tileKey.level <= 6) return defaultIfNotLoaded;

        return terrTile.getHeight(geoPoint);
    }
    ...
    */
    ...
    rayCast(x, y) {return false;
        ...
        const tileList = this.dataSource.mapView.visibleTileSet.dataSourceTileList.find(s => s.dataSource == this.dataSource);
        ...
        for (var i = 0; i < tiles.length; i++) {
            ...
            if (this.lru.get(tiles[i].tile.tileKey.mortonCode())) {
                const { screen, camPositon, value } = this.lru.get(tiles[i].tile.tileKey.mortonCode());
                if (screen.x == x && screen.y == y && camPositon.equals(this.dataSource.mapView.camera.position)) {
                    rayRets.push(value);
                    continue;
                }
            }
