...
TileAvailability.prototype.addAvailableTileRange = function (
  ...
  var rectangleScratch = tilingScheme.getGeoBox(new TileKey((1 << level) - 1 - startY, startX, level));
  var west = rectangleScratch.west;
  var north = rectangleScratch.north;

  rectangleScratch = tilingScheme.getGeoBox(new TileKey((1 << level) - 1 - endY, endX, level));
  ...
  if (rectangle.east < rectangle.west) {
    ...
    rectangles.push(
      GeoBox.fromCoordinates(
        new GeoCoordinates(rectangle.south, -Math.PI),
    ...
    rectangles.push(
      GeoBox.fromCoordinates(
        ...
        new GeoCoordinates(rectangle.north, Math.PI)
        // eastScratch
      )
    );
  } else {
...
TileAvailability.prototype.isTileAvailable = function (level, x, y) {
  ...
  var rectangle = this._tilingScheme.getGeoBox(new TileKey(y, x, level));
  cartographicScratch.copy(rectangle.center);
    ...
    if (!rectanglesOverlap(rectangle, rectangleToSubtract)) {
      ...
      if (rectangle.west < rectangleToSubtract.west) {
        result.push(
          new GeoBox(
            rectangle.southWest,
            new GeoCoordinates(rectangle.north, rectangleToSubtract.west)
          )
        );
      }
      if (rectangle.east > rectangleToSubtract.east) {
        result.push(
          new GeoBox(
            new GeoCoordinates(rectangle.south, rectangleToSubtract.east),
            rectangle.northEast
          )
        );
      }
      if (rectangle.south < rectangleToSubtract.south) {
        result.push(
          new GeoBox(
            new GeoCoordinates(rectangle.south, Math.max(rectangleToSubtract.west, rectangle.west)),
            new GeoCoordinates(rectangleToSubtract.south, Math.min(rectangleToSubtract.east, rectangle.east))
          )
        );
      }
      if (rectangle.north > rectangleToSubtract.north) {
        result.push(
          new GeoBox(
            new GeoCoordinates(rectangleToSubtract.north, Math.max(rectangleToSubtract.west, rectangle.west)),
            new GeoCoordinates(rectangle.north, Math.min(rectangleToSubtract.east, rectangle.east))
          )
        );
      }
    }
