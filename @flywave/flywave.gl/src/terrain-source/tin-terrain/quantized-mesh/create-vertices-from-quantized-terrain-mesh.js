...
export function createVerticesFromQuantizedTerrainMesh(
  ...
  var rectangle = new GeoBox(GeoCoordinates.fromObject((parameters.rectangle.southWest)), GeoCoordinates.fromObject((parameters.rectangle.northEast)));
  ...
  if (includeWebMercatorT) {
    ...
    oneOverMercatorHeight =
      1.0 / (geodeticLatitudeToMercatorAngle(north) -
        southMercatorY);
  }
...
function findMinMaxSkirts(
  ...
  for (var i = 0; i < length; ++i) {
    ...
    var position = cartesian3Scratch.copy(projection.projectPoint(
      cartographicScratch
    ));
