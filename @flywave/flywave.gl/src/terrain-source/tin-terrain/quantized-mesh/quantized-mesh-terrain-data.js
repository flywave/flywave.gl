...
import Intersections2D from "../math/intersections2D";
 ...
 *          be retried later.
 */
QuantizedMeshTerrainData.prototype.createMesh = function (options, task) {
  ...
  var rectangle = tilingScheme.getGeoBox(new TileKey(y, x, level));

  var verticesPromise = task({
    x,y,level,
  ...
  return verticesPromise.then(function (result) {
    ...
    var textureCoordAndEncodedNormals = new Float32Array(result.textureCoordAndEncodedNormals);
    var position3DAndHeight = new Float32Array(result.position3DAndHeight);
    ...
    var obb = defaultValue(
      result.orientedBoundingBox && OrientedBoundingBox.clone(result.orientedBoundingBox),
      that._orientedBoundingBox
    );
    var occludeePointInScaledSpace = defaultValue(
      result.occludeePointInScaledSpace && new THREE.Vector3().copy(result.occludeePointInScaledSpace),
...
QuantizedMeshTerrainData.prototype.upsample = function (
  ...
  var childRectangle = tilingScheme.getGeoBox(new TileKey((1 << descendantLevel) - 1 - descendantY, descendantX, descendantLevel));

  var upsamplePromise = task({
    thisX,
    thisY,
    thisLevel,
    descendantX,
...
var maxShort = 32767;
var barycentricCoordinateScratch = new THREE.Vector3();
...
var texCoordScratch0 = new THREE.Vector2();
var texCoordScratch1 = new THREE.Vector2();
var texCoordScratch2 = new THREE.Vector2();

function interpolateMeshHeight(terrainData, u, v) {
  var mesh = terrainData._mesh;
  var position3DAndHeight = mesh.position3DAndHeight;
  var textureCoordAndEncodedNormals = mesh.textureCoordAndEncodedNormals;
  ...
  for (var i = 0, len = indices.length; i < len; i += 3) {
    ...
    var uv0 = encoding.decodeTextureCoordinates(textureCoordAndEncodedNormals, i0, texCoordScratch0);
    var uv1 = encoding.decodeTextureCoordinates(textureCoordAndEncodedNormals, i1, texCoordScratch1);
    var uv2 = encoding.decodeTextureCoordinates(textureCoordAndEncodedNormals, i2, texCoordScratch2);
    ...
    if (pointInBoundingBox(u, v, uv0.x, uv0.y, uv1.x, uv1.y, uv2.x, uv2.y)) {
      ...
      if (
        ...
        var h0 = encoding.decodeHeight(position3DAndHeight, i0);
        var h1 = encoding.decodeHeight(position3DAndHeight, i1);
        var h2 = encoding.decodeHeight(position3DAndHeight, i2);
