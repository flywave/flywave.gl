...
function createQuantizedMeshTerrainData(provider, buffer, level, x, y, layer) {
      ...
      if (stringLength > 0) {
        ...
        if (defined(availableTiles)) {
          for (var offset = 0; offset < availableTiles.length; ++offset) {
            ...
            var yTiles = provider.tilingScheme.subdivisionScheme.getLevelDimensionY(
              availableLevel
            );
  ...
  if (waterMaskBuffer) {
    ...
    texture = new THREE.DataTexture(waterMaskBuffer, textureSize, textureSize, THREE.LuminanceFormat, THREE.UnsignedByteType);
...
class DataTerrainProvider {
  ...
  connect() {
    return new Promise((reslove, reject) => {
      ...
      return downloadManager.downloadJson(`${this.url}/layer.json`).then(this.metadataSuccess).then(reslove).catch(reject);
    });
  }
  ...
  makeLoaderTile(tileKey, parentTileTinData) {
    ...
    tile.tileLoader = new TinMeshLoader(this.dataSource, tileKey, tile, this.dataSource.decoder, parentTileTinData);
    return tile;
  }
  ...
  parseMetadataSuccess = (data) => {
    ...
    if (!data.projection || data.projection === "EPSG:4326") {
      this.tilingScheme = new TilingScheme(new GeographicTilingScheme({
    ...
    this._levelZeroMaximumGeometricError = this.getEstimatedLevelZeroGeometricErrorForAHeightmap(
    ...
    if (defined(parentUrl)) {
      ...
      return downloadManager.downloadJson(`${parentUrl}/layer.json`)
        .catch(parseMetadataFailure).then(() => {
...
function requestTileGeometry(provider, x, y, level, layerToUse) {
  ...
  var promise = downloadManager
    .downloadArrayBuffer(formatUrl(url, {
