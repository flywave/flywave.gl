...
import { isFloatLinearSupported } from "./utils/capabilities";
import { resolveIncludes } from "./utils/resolveIncludes";
import type { AnyFloatType } from "./utils/types";
...
import { common, definitions, precompute } from "./shaders/bruneton";
import {
    directIrradianceShader,
    indirectIrradianceShader,
    multipleScatteringShader,
    scatteringDensityShader,
    singleScatteringShader,
    transmittanceShader
} from "./shaders/precompute";
...
function createRenderTarget(type: AnyFloatType, width: number, height: number): WebGLRenderTarget {
...
class PrecomputeMaterial extends RawShaderMaterial {
    ...
    setUniforms(context: Context): void {
        ...
        if (uniforms.singleMieScatteringTexture != null) {
            uniforms.singleMieScatteringTexture.value = context.deltaMieScattering.texture;
        }
        if (uniforms.multipleScatteringTexture != null) {
            uniforms.multipleScatteringTexture.value = context.deltaMultipleScattering.texture;
        }
        if (uniforms.scatteringDensityTexture != null) {
            uniforms.scatteringDensityTexture.value = context.deltaScatteringDensity.texture;
        }
...
export class PrecomputedTexturesGenerator {
    ...
    private computeTransmittance(params: { renderTarget: WebGLRenderTarget }): void {
    ...
    private computeOpticalDepth(params: { renderTarget: WebGLRenderTarget }): void {
        const material = this.transmittanceMaterial;
    ...
    private computeDirectIrradiance(params: {
        ...
        uniforms.transmittanceTexture.value =
            params.context.opticalDepth?.texture ?? this.transmittanceRenderTarget.texture;
    ...
    async update(atmosphere = AtmosphereParameters.DEFAULT): Promise<PrecomputedTextures> {
        this.updating = true;
