...
import { define, defineInt } from "./utils/decorators";
import { Ellipsoid } from "./geospatial/Ellipsoid";
import { Geodetic } from "./geospatial/Geodetic";
import { remap, saturate } from "./utils/math";
import { resolveIncludes } from "./utils/resolveIncludes";
import { unrollLoops } from "./utils/unrollLoops";
import type { UniformMap } from "./utils/types";
import {
    cascadedShadowMaps,
    depth,
...
import {
    aerialPerspectiveFragmentShader as fragmentShader,
    aerialPerspectiveVertexShader as vertexShader,
    skyShader
} from "./shaders";
import { common, definitions, runtime } from "./shaders/bruneton";
...
export class AerialPerspectiveEffect extends Effect {
    ...
    private copyCameraSettings(camera: Camera): void {
        ...
        const cameraPosition = camera.getWorldPosition(uniforms.get("cameraPosition").value);
    ...
    get altitudeCorrection(): Vector3 {
        return this.uniforms.get("altitudeCorrection").value;
    }

    set altitudeCorrection(value: Vector3) {
        this.uniforms.get("altitudeCorrection").value.copy(value);
    }
