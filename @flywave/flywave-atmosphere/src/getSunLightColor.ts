...
import { TRANSMITTANCE_TEXTURE_HEIGHT, TRANSMITTANCE_TEXTURE_WIDTH } from "./utils/constants";
import {
...
export function getSunLightColor(
    ...
    result = new Color(),
    { ellipsoid = Ellipsoid.WGS84, correctAltitude = true }: SunLightColorOptions = {},
    atmosphere = AtmosphereParameters.DEFAULT
): Color {
    const camera = vectorScratch1.copy(worldPosition);
    if (correctAltitude) {
        const surfacePosition = ellipsoid.projectOnSurface(worldPosition, vectorScratch2);
