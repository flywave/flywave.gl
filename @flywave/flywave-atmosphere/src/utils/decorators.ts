            ...
            Object.defineProperty(target, propertyKey, {
                ...
                set(this: Extract<T, EffectLike>, value: boolean) {
                    if (value !== this[propertyKey]) {
                        ...
                        (this as unknown as { setChanged(): void }).setChanged();
                    }
                }
            });
        }
...
export function defineInt(
    name: string,
    { min = Number.MIN_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER }: DefineIntDecoratorOptions = {}
) {
    return <T extends MaterialLike | EffectLike, K extends keyof T>(
...
export function defineFloat(
    name: string,
    { min = -Infinity, max = Infinity, precision = 7 }: DefineFloatDecoratorOptions = {}
) {
    return <T extends MaterialLike | EffectLike, K extends keyof T>(
        target: T[K] extends number ? T : never,
        propertyKey: K
