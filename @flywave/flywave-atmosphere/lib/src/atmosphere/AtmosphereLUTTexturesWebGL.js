...
import { NoColorSpace, RenderTarget, RenderTarget3D, RGBAFormat } from "three";
...
import { computeMultipleScatteringTexture, computeScatteringTexture, getTextureUnitFromSubUV } from "./multiscattering";
import { computeIrradianceTexture, computeTransmittanceTexture } from "./precompute";
...
function createRenderTarget3D(name) {
...
function setupRenderTarget(renderTarget, textureType, size) {
    renderTarget.texture.type = textureType;
    renderTarget.setSize(size.x, size.y);
}
function setupRenderTarget3D(renderTarget, textureType, size) {
    renderTarget.texture.type = textureType;
...
export class AtmosphereLUTTexturesWebGL extends AtmosphereLUTTextures {
    ...
    createContext() {
        if (this.parameters == null) {
            throw new Error("Parameters must be set before creating context.");
        }
        if (this.textureType == null) {
            throw new Error("Texture type must be set before creating context.");
        }
        return new AtmosphereLUTTexturesContextWebGL(this.parameters, this.textureType);
    }
    renderToRenderTarget(renderer, renderTarget, textures) {
        if (textures != null) {
    ...
    renderToRenderTarget3D(renderer, renderTarget, layer, textures) {
        if (textures != null) {
            renderTarget.textures.push(...textures.filter(value => value != null));
        }
    ...
    computeMultipleScattering(renderer, context) {
        ...
        const getRayDirection = FnVar((index) => {
        ...
        const getMultipleScattering = Fn(() => {
            ...
            const uv = getTextureUnitFromSubUV(screenCoordinate.div(size), size).toConst();
            ...
            const lightDirection = vec3(0, sqrt(cosLightZenith.pow2().oneMinus().saturate()), cosLightZenith).toConst();
            ...
            Loop({ start: 0, end: sampleCount }, ({ i: index }) => {
                ...
                const result = computeMultipleScatteringTexture(parametersNode, texture(this.transmittanceRT.texture), radius, cosView, cosLightZenith, cosViewLight)
            ...
            return totalMultipleScattering.mul(totalTransferFactor.oneMinus().reciprocal());
        });
    ...
    computeScattering(renderer, context) {
        const { parameters } = context;
        this.scatteringMaterial?.dispose();
        this.scatteringMaterial = this.createMaterial((() => {
            const result = computeScatteringTexture(texture(this.transmittanceRT.texture), texture(this.multipleScatteringRT.texture), vec3(screenCoordinate, this.layer.add(0.5)))
                .context({ getAtmosphere: () => context })
                .toConst();
        ...
        this.renderToRenderTarget3D(renderer, this.scatteringRT, this.layer, textures);
    }
    computeIrradiance(renderer, context) {
        this.irradianceMaterial?.dispose();
        this.irradianceMaterial = this.createMaterial(
        // BUG: Context is not merged unless we wrap the node by OutputStructNode.
        mrt({
            irradiance: computeIrradianceTexture(texture3D(this.scatteringRT.texture), texture3D(this.higherOrderScatteringRT.texture), screenCoordinate).context({ getAtmosphere: () => context })
        }));
    ...
    setup(parameters, textureType) {
        setupRenderTarget(this.transmittanceRT, textureType, parameters.transmittanceTextureSize);
        setupRenderTarget(this.multipleScatteringRT, textureType, parameters.multipleScatteringTextureSize);
        setupRenderTarget3D(this.scatteringRT, textureType, parameters.scatteringTextureSize);
        if (!parameters.combinedScatteringTextures) {
            setupRenderTarget3D(this.singleMieScatteringRT, textureType, parameters.scatteringTextureSize);
        }
        if (parameters.higherOrderScatteringTexture) {
            setupRenderTarget3D(this.higherOrderScatteringRT, textureType, parameters.scatteringTextureSize);
        }
        setupRenderTarget(this.irradianceRT, textureType, parameters.irradianceTextureSize);
...
//# sourceMappingURL=AtmosphereLUTTexturesWebGL.js.map