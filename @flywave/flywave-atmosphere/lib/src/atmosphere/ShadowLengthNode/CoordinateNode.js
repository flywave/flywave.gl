...
import { getCameraZUnit, isValidScreenLocation, transformNDCToUV } from "./common";
...
export class CoordinateNode extends Node {
    ...
    setupFragmentNode(builder) {
        ...
        return Fn(() => {
            ...
            const sliceEndPoints = sliceEndpointsNode.load(ivec2(screenCoordinate.y, 0)).toConst();
            ...
            If(isValidScreenLocation(sliceEndPoints.xy, screenSize), () => {
                ...
                let samplePositionOnEpipolarLine = uvNode.x.sub(float(0.5).div(maxSliceSampleCount));
                ...
                const xy = mix(sliceEndPoints.xy, sliceEndPoints.zw, samplePositionOnEpipolarLine).toConst();
                ...
                If(isValidScreenLocation(xy, screenSize), () => {
                    const cameraZ = getCameraZUnit(camera, transformNDCToUV(xy), viewZUnitNode);
...
//# sourceMappingURL=CoordinateNode.js.map