...
export class UnwarpEpipolarNode extends Node {
    ...
    constructor() {
        super();
        this.refinementThreshold = uniform(0.03);
    ...
    setupFragmentNode(builder) {
        const { sliceEndpointsNode, coordinateNode, epipolarShadowLengthNode, refinementThreshold, viewZUnitNode, camera, maxSliceSampleCount, epipolarSliceCount, screenSize, lightScreenPosition } = this;
        return Fn(() => {
            ...
            const halfSpaceFlags = halfSpaceEquationTerms.xyyx
                ...
                .toConst();
            const sectorFlags = bvecAnd(halfSpaceFlags.wxyz, bvecNot(halfSpaceFlags.xyzw)).toConst();
            ...
            const precedingSliceIndex = min(epipolarSliceValue.mul(epipolarSliceCount).floor(), epipolarSliceCount.sub(1)).toConst();
            ...
            for (let i = 0; i < 2; ++i) {
                ...
                const shadowLengthTextureSize = vec2(maxSliceSampleCount, epipolarSliceCount).toConst();
                const sourceLocationsCameraZ = textureGather(coordinateNode, shadowLengthUV.add(vec2(0.5).div(shadowLengthTextureSize)), 2).wz;
                ...
                const bilateralUWeight = vec2(uWeight.oneMinus(), uWeight)
                    ...
                    .mul(sliceWeights[i])
                    .toVar();
                bilateralUWeight.mulAssign(vec2(samplePositionOnLine
                    .sub(0.5)
                    .abs()
                ...
                const filteredShadowLength = bilateralUWeight.x
                    ...
                    .mul(epipolarShadowLengthNode.sample(shadowLengthUV.add(vec2(subpixelUOffset, 0))).xy)
                    .toConst();
...
//# sourceMappingURL=UnwarpEpipolarNode.js.map