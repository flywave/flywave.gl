...
import { HalfFloatType, RenderTarget, RGFormat } from "three";
...
const getRaySphereIntersections = /*#__PURE__*/ FnVar((rayOrigin, rayDirection, sphereCenter, sphereRadius) => {
    const intersections = raySpheresIntersections(rayOrigin, rayDirection, sphereCenter, sphereRadius).toConst();
    return vec4(intersections.get("near").x, intersections.get("far").x, intersections.get("near").y, intersections.get("far").y);
});
export class EpipolarShadowLengthNode extends Node {
    ...
    setupFragmentNode(builder) {
        const { csmShadowNode, coordinateNode, sliceUVDirectionNode, minMaxLevelsNode, shadowDepthNodes, camera, epipolarSliceCount, firstCascade, maxShadowStep, shadowCascadeArray, shadowMatrixArray } = this;
        ...
        const biasedCameraFar = uniform("float").onRenderUpdate(() => camera.far * worldToUnit * 0.999999);
        const sampleShadow = FnVar((shadowUVInLightSpace, cascadeIndex, depthInLightSpace) => {
            const isInLight = float(0).toVar();
            for (let cascade = 0; cascade < cascadeCount; ++cascade) {
                If(cascadeIndex.equal(cascade), () => {
                    isInLight.assign(shadowDepthNodes[cascade]
        ...
        const processCascade = FnVar((cascadeIndex, rayEndCameraZ, cascadeStartCameraZ, cascadeEndCameraZ, fullRayLength, viewDirection, rayTopIntersection) => {
            ...
            If(rayLength.lessThanEqual(10 * worldToUnit).not(), () => {
                ...
                const startUVAndDepthInLightSpace = transformUnitToShadowUV(rayStart, shadowMatrix);
                ...
                const shadowTraceDirection = endUVAndDepthInLightSpace
                    ...
                    .toVar();
                const traceLengthInShadowUVSpace = max(shadowTraceDirection.xy.length(), 1e-7).toConst();
                ...
                const minLevel = 0;
                const currentSamplePosition = uint(startUVAndDepthInLightSpace.xy
                    .sub(sliceOriginUV)
                    .length()
                ...
                Loop(distanceMarchedInCascade.lessThan(rayLength), () => {
                    const currentDepthInLightSpace = currentShadowUVAndDepthInLightSpace.z
                        ...
                        .toConst();
                    const isInLight = float(0).toVar();
                    If(and(stepScale.mul(2).lessThan(maxStepScale), currentSamplePosition
                        .bitAnd(uint(2).shiftLeft(currentTreeLevel).sub(1))
                        .equal(0)), () => {
                        levelDataOffset.addAssign(minMaxShadowMapSize.shiftRight(currentTreeLevel));
                        currentTreeLevel.addAssign(1);
                        stepScale.mulAssign(2);
                    });
                    Loop(currentTreeLevel.greaterThan(minLevel), () => {
                        const nextLightSpaceDepth = currentShadowUVAndDepthInLightSpace.z.add(shadowUVAndDepthStep.z.mul(stepScale.sub(1)));
                        const startEndDepthOnRaySection = vec2(currentShadowUVAndDepthInLightSpace.z, nextLightSpaceDepth)
                            .saturate()
                            .toConst();
                        const minMaxTextureCoord = ivec2(int(currentSamplePosition.shiftRight(currentTreeLevel)).add(levelDataOffset), minMaxTextureYIndex);
                        const currentMinMaxDepth = minMaxLevelsNode
                            .load(minMaxTextureCoord)
                            .xy.toConst();
                        isInLight.assign(builder.renderer.reversedDepthBuffer
                            ...
                            : startEndDepthOnRaySection
                                ...
                                .all());
                        const isInShadow = (builder.renderer.reversedDepthBuffer
                            ? startEndDepthOnRaySection
                        ...
                        levelDataOffset.subAssign(minMaxShadowMapSize.shiftRight(currentTreeLevel));
                        stepScale.divAssign(2);
                    });
                    If(currentTreeLevel.lessThanEqual(minLevel), () => {
                        isInLight.assign(sampleShadow(currentShadowUVAndDepthInLightSpace.xy, cascadeIndex, currentDepthInLightSpace));
                    });
                    ...
                    const integrationStep = rayStepLengthUnit
                        ...
                        .toConst();
                    currentShadowUVAndDepthInLightSpace.addAssign(shadowUVAndDepthStep.mul(stepScale));
        ...
        return Fn(() => {
            ...
            If(sampleLocation
                ...
                .any()
                .not(), () => {
                const rayTermination = transformSliceToUnit(sampleLocation, rayEndCameraZ, camera).toConst();
                ...
                const intersections = getRaySphereIntersections(cameraPositionUnit, viewDirection, vec3(0), vec2(topRadius, bottomRadius)).toConst();
                ...
                If(rayTopIntersection.y.greaterThan(0), () => {
                    ...
                    Loop({
                        ...
                        totalShadowLength.addAssign(processCascade(cascadeIndex, rayEndCameraZ, cascadeStartCameraZ, cascadeEndCameraZ, rayLength, viewDirection, rayTopIntersection));
                    });
                });
            });
            If(totalShadowLength.x.greaterThan(1e-6), () => {
                const distanceToFirstShadow = totalShadowLength.y
                    ...
                    .toConst();
                totalShadowLength.y.assign(distanceToFirstShadow
...
//# sourceMappingURL=EpipolarShadowLengthNode.js.map