...
export class MinMaxLevelsNode extends Node {
    ...
    constructor() {
        super();
        this.gatherMaterial = new NodeMaterial();
        this.mipmapMaterial = new NodeMaterial();
        this.mesh = new QuadMesh(this.gatherMaterial);
        this.mipmapSourceNode = uniformTexture();
        this.mipmapOffsetNode = uniform("uvec2");
    ...
    render(renderer, width, height) {
        ...
        for (let step = 2; step <= maxStep; step *= 2, parity = (parity + 1) % 2) {
            ...
            const [sourceRT, targetRT] = parity === 0 ? [renderTargetB, renderTargetA] : [renderTargetA, renderTargetB];
            if (step === 2) {
            ...
            if (parity === 1) {
                ...
                renderer.copyTextureToTexture(renderTargetB.texture, renderTargetA.texture, boxScratch, vector2Scratch);
            }
            prevOffsetX = offsetX;
            offsetX += targetWidth;
        }
    ...
    setupGatherNode(builder) {
        const { csmShadowNode, sliceUVDirectionNode, shadowDepthNodes, epipolarSliceCount, firstCascade } = this;
        const { cascades: cascadeCount } = csmShadowNode;
        return Fn(() => {
            ...
            for (let cascade = 0; cascade < cascadeCount; ++cascade) {
                If(cascadeIndex.equal(cascade), () => {
                    for (let i = 0; i <= 1; ++i) {
                        ...
                        If(and(sampleUV.greaterThanEqual(0).all(), sampleUV.lessThanEqual(1).all()), () => {
                            const depths = textureGather(shadowDepthNodes[cascade], sampleUV).toConst();
            ...
            return vec2(min(minDepths.x, minDepths.y, minDepths.z, minDepths.w), max(maxDepths.x, maxDepths.y, maxDepths.z, maxDepths.w));
        })();
    }
    setupMipmapNode(builder) {
        const { mipmapSourceNode: sourceNode, mipmapOffsetNode: offsetNode } = this;
        return Fn(() => {
            const coordNode = uvec2(screenCoordinate).toConst();
            const x1 = offsetNode.x.add(coordNode.x.sub(offsetNode.y).mul(2)).toConst();
            ...
            return vec2(min(minMaxDepth1.x, minMaxDepth2.x), max(minMaxDepth1.y, minMaxDepth2.y));
        })();
    }
    setup(builder) {
...
//# sourceMappingURL=MinMaxLevelsNode.js.map