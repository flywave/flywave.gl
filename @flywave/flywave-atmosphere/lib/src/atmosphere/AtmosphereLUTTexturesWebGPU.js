...
export function createStorageTexture(name) {
...
export function createStorage3DTexture(name) {
...
export function setupStorageTexture(texture, textureType, size) {
    texture.type = textureType;
    reinterpretType(texture.image);
    texture.image.width = size.x;
    texture.image.height = size.y;
}
export function setupStorage3DTexture(texture, textureType, size) {
    texture.type = textureType;
    reinterpretType(texture.image);
    texture.image.width = size.x;
    texture.image.height = size.y;
...
export class AtmosphereLUTTexturesWebGPU extends AtmosphereLUTTextures {
    ...
    createContext() {
        ...
        return new AtmosphereLUTTexturesContextWebGPU(this.parameters, this.textureType);
    }
    computeTransmittance(renderer, context) {
        ...
        this.transmittanceNode = Fn(() => {
            ...
            const transmittance = computeTransmittanceTexture(vec2(globalId.xy).add(0.5));
    ...
    computeMultipleScattering(renderer, context) {
        ...
        this.multipleScatteringNode = Fn(() => {
            ...
            const result = computeMultipleScatteringTexture(parametersNode, texture(this.transmittance), radius, cosView, cosLightZenith, cosViewLight).toConst();
            ...
            textureStore(this.multipleScattering, globalId.xy, 
    ...
    computeScattering(renderer, context) {
        ...
        this.scatteringNode = Fn(() => {
            ...
            const result = computeScatteringTexture(texture(this.transmittance), texture(this.multipleScattering), vec3(globalId).add(0.5)).toConst();
            ...
            if (parameters.combinedScatteringTextures) {
                textureStore(this.scattering, globalId, vec4(scattering, singleMieScattering.r));
            }
            else {
                textureStore(this.scattering, globalId, vec4(scattering, 1));
                textureStore(this.singleMieScattering, globalId, vec4(singleMieScattering, 1));
            }
            if (parameters.higherOrderScatteringTexture) {
                const higherOrderScattering = result.get("higherOrderScattering");
                textureStore(this.higherOrderScattering, globalId, vec4(higherOrderScattering, 1));
            }
        })()
            .context({ getAtmosphere: () => context })
            .computeKernel([4, 4, 4])
    ...
    computeIrradiance(renderer, context) {
        ...
        this.irradianceNode = Fn(() => {
            ...
            const irradiance = computeIrradianceTexture(texture3D(this.scattering), texture3D(this.higherOrderScattering), vec2(globalId.xy).add(0.5));
    ...
    setup(parameters, textureType) {
        setupStorageTexture(this.transmittance, textureType, parameters.transmittanceTextureSize);
        setupStorageTexture(this.multipleScattering, textureType, parameters.multipleScatteringTextureSize);
        setupStorage3DTexture(this.scattering, textureType, parameters.scatteringTextureSize);
        if (!parameters.combinedScatteringTextures) {
            setupStorage3DTexture(this.singleMieScattering, textureType, parameters.scatteringTextureSize);
        }
        if (parameters.higherOrderScatteringTexture) {
            setupStorage3DTexture(this.higherOrderScattering, textureType, parameters.scatteringTextureSize);
        }
        setupStorageTexture(this.irradiance, textureType, parameters.irradianceTextureSize);
...
//# sourceMappingURL=AtmosphereLUTTexturesWebGPU.js.map