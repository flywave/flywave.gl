 ...
 */
import { DirectionalLight, MathUtils, Matrix4, PerspectiveCamera, Vector2, Vector3, Vector4 } from "three";
import { float, texture, uniform, uniformArray, uv, vec2, vec3, vec4 } from "three/tsl";
import { NodeMaterial, NodeUpdateType, TempNode } from "three/webgpu";
...
export class ShadowLengthNode extends TempNode {
    ...
    constructor(csmShadowNode, viewZUnitNode) {
        super("vec2");
        this.resolutionScale = 1;
        this.autoSampleResolution = true;
        ...
        this.epipolarSliceCount = uniform(512, "float");
        ...
        this.maxSliceSampleCount = uniform(256, "float");
        // First cascade used for ray marching.
        this.firstCascade = uniform(0, "uint");
        this.screenSize = uniform("vec2");
        this.lightScreenPosition = uniform("vec4");
        this.isLightOnScreen = uniform("bool");
        this.currentCascades = 0;
    ...
    update({ renderer, material }) {
        ...
        const viewProjection = matrixScratch.multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse);
        ...
        if (this.autoSampleResolution) {
            ...
            const size = MathUtils.floorPowerOfTwo(Math.max(width, height) / pixelRatio);
            this.epipolarSliceCount.value = size;
            this.maxSliceSampleCount.value = size >>> 1;
        }
    }
    setup(builder) {
        const { csmShadowNode, viewZUnitNode, sliceEndpointsNode, coordinateNode, sliceUVDirectionNode, minMaxLevelsNode, epipolarShadowLengthNode, unwarpEpipolarNode, epipolarSliceCount, maxSliceSampleCount, firstCascade, screenSize, lightScreenPosition, isLightOnScreen } = this;
        ...
        const shadowCascadeArray = uniformArray(Array.from({ length: cascadeCount }, () => new Vector2()), "vec2");
        OnBeforeFrameUpdate(() => {
            const array = shadowCascadeArray.array;
        ...
        const shadowMatrixArray = uniformArray(Array.from({ length: cascadeCount }, () => new Matrix4()), "mat4");
        OnBeforeFrameUpdate(() => {
            const array = shadowMatrixArray.array;
    ...
    getDebugInternalTexturesNode(uvNode = uv()) {
        ...
        return uvNode.y
            ...
            .select(uv1.y
            .lessThan(0.5)
            .select(sliceEndpoints.sample(uv2.xy), sliceUVDirection.sample(uv2.xz))
            .uniformFlow(), coordinate.sample(uv1.zy))
            .uniformFlow(), uvNode.x
            .lessThan(0.5)
            .select(minMaxLevels.sample(uv1.xw), vec3(epipolarShadowLength.sample(uv1.zw).xy, 0))
            .uniformFlow())
            .uniformFlow().rgb;
    }
    dispose() {
...
export const shadowLength = (...args) => new ShadowLengthNode(...args);
//# sourceMappingURL=ShadowLengthNode.js.map