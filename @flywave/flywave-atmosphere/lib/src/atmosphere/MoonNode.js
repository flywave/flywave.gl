...
import { atmosphereParametersStruct, makeDestructible } from "./AtmosphereContextBase";
...
const getLunarRadiance = /*#__PURE__*/ FnLayout({
    ...
    const { solarIrradiance, sunRadianceToLuminance, luminanceScale } = makeDestructible(parameters);
...
const orenNayarDiffuse = /*#__PURE__*/ FnLayout({
    ...
    const s = lightDirection.dot(viewDirection).sub(cosLight.mul(cosView)).toConst();
...
export class MoonNode extends TempNode {
    ...
    constructor() {
        ...
        this.colorNode = null;
        this.displacementNode = null;
        this.angularRadius = uniform(0.0045); // ≈ 15.5 arcminutes
        this.intensity = uniform(1);
        // For NASA Moon Kit unsigned 16 bit half-meter images:
        // https://svs.gsfc.nasa.gov/4720/
        this.displacementScale = uniform((0xffff * 0.5) / 1727400);
    }
    setup(builder) {
        ...
        const { sunDirectionECEF, moonDirectionECEF: directionECEF, matrixMoonFixedToECEF: matrixFixedToECEF } = atmosphereContext;
        return Fn(() => {
            ...
            If(chordLength.lessThan(chordThreshold), () => {
                normalECEF.assign(raySphereIntersectionNormal(rayDirectionECEF, directionECEF, this.angularRadius));
                normalMF.assign(matrixFixedToECEF.transpose().mul(vec4(normalECEF, 0)).xyz);
            ...
            If(chordLength.lessThan(chordThreshold), () => {
                if (this.displacementNode != null) {
                    ...
                    const perturbedNormalMF = normalMF
                        ...
                        .normalize();
                    normalMF.assign(mix(normalMF, perturbedNormalMF, 
                    // Avoid artifact at the edge:
                ...
                const diffuse = orenNayarDiffuse(sunDirectionECEF, rayDirectionECEF.negate(), normalECEF);
                const antialias = smoothstep(chordThreshold, chordThreshold.sub(filterWidth), chordLength);
                luminance.assign(vec4(getLunarRadiance(atmosphereContext.parametersNode, this.angularRadius)
                    .mul(this.intensity)
                    .mul(color)
                    .mul(diffuse), antialias));
            });
            return luminance;
        })();
    }
}
//# sourceMappingURL=MoonNode.js.map