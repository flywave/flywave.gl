// @ts-nocheck
import { AdditiveBlending, CanvasTexture, Mesh, PerspectiveCamera, PlaneGeometry, SRGBColorSpace, Vector2 } from "three";
...
import { IndirectStorageBufferAttribute, MeshBasicNodeMaterial, RendererUtils } from "three/webgpu";
...
export class LensGlareNode extends FilterNode {
    ...
    constructor(inputNode = null) {
        super(inputNode);
        this.quadTexture = createQuadTexture();
        this.quadCount = 6;
        this.wireframe = false;
        this.intensity = uniform(1e-5);
        this.sizeScale = uniform(new Vector2(1, 0.01)); // length, width
        this.luminanceThreshold = uniform(100);
        // drawIndexedIndirect format:
        // [indexCount, instanceCount, firstIndex, baseVertex, firstInstance]
        this.indirectBuffer = new IndirectStorageBufferAttribute(new Uint32Array([6, 0, 0, 0, 0]), 1);
        this.instanceBuffer = instancedArray(1, instanceStruct);
        ...
        this.tileSize = uniform("uvec2");
        this.inputTexelSize = uniform("vec2");
        this.outputTexelSize = uniform("vec2");
        this.geometryRatio = uniform("vec2");
    ...
    setupCompute() {
        const { quadCount, inputNode, indirectBuffer, instanceBuffer, tileSize, outputTexelSize } = this;
        invariant(inputNode != null, "inputNode cannot be null during setup.");
        const indirectStorage = storage(indirectBuffer, "uint", indirectBuffer.count).toAtomic();
        this.computeNode = Fn(() => {
    ...
    setupMaterial() {
        const { quadTexture, instanceBuffer, luminanceThreshold, intensity, sizeScale, outputTexelSize, geometryRatio } = this;
        ...
        this.material.vertexNode = Fn(() => {
            ...
            const scale = vec2(luminanceScale, 1).mul(instance.get("scale"), sizeScale, 
...
export const lensGlare = (inputNode) => new LensGlareNode(convertToTexture(inputNode, { name: "LensGlare [Input]" }));
//# sourceMappingURL=LensGlareNode.js.map