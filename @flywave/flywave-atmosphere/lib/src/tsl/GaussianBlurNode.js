...
function createGaussianKernel(kernelSize, edgeBias = 2) {
...
export class GaussianBlurNode extends SeparableFilterNode {
    ...
    constructor(inputNode, kernelSize = 35) {
        super(inputNode);
    ...
    setupOutputNode(builder) {
        ...
        return Fn(() => {
            ...
            for (let i = 1; i < offsets.length; ++i) {
                ...
                output.addAssign(add(inputNode.sample(center.add(offset)), inputNode.sample(center.sub(offset))).mul(weights[i]));
            }
            return output;
        })();
    }
}
export const gaussianBlur = (...args) => new GaussianBlurNode(...args);
