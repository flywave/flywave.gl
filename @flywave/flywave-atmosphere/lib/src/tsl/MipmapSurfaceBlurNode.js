...
export class MipmapSurfaceBlurNode extends DualMipmapFilterNode {
    ...
    setupDownsampleNode(builder) {
        ...
        return Fn(() => {
            ...
            let weight;
            weight = vec4(clampToBorder(uv01), clampToBorder(uv02), clampToBorder(uv03), clampToBorder(uv04)).mul(innerWeight);
            output.addAssign(inputNode.sample(uv01).mul(weight.x), inputNode.sample(uv02).mul(weight.y), inputNode.sample(uv03).mul(weight.z), inputNode.sample(uv04).mul(weight.w));
            weight = vec4(clampToBorder(uv05), clampToBorder(uv06), clampToBorder(uv07), clampToBorder(uv08)).mul(outerWeight);
            output.addAssign(inputNode.sample(uv05).mul(weight.x), inputNode.sample(uv06).mul(weight.y), inputNode.sample(uv07).mul(weight.z), inputNode.sample(uv08).mul(weight.w));
            weight = vec4(clampToBorder(uv09), clampToBorder(uv10), clampToBorder(uv11), clampToBorder(uv12)).mul(outerWeight);
            output.addAssign(inputNode.sample(uv09).mul(weight.x), inputNode.sample(uv10).mul(weight.y), inputNode.sample(uv11).mul(weight.z), inputNode.sample(uv12).mul(weight.w));
            return output;
        })();
    }
    setupUpsampleNode(builder) {
        ...
        invariant(inputNode != null, "inputNode cannot be null during setup.");
        const center = uv();
        const offset = vec4(1, 1, -1, -1).mul(inputTexelSize.xyxy).add(center.xyxy).toVertexStage();
        ...
        const output = add(inputNode.sample(center).mul(1 / 4), add(inputNode.sample(uv1), inputNode.sample(uv2), inputNode.sample(uv3), inputNode.sample(uv4)).mul(1 / 8), add(inputNode.sample(uv5), inputNode.sample(uv6), inputNode.sample(uv7), inputNode.sample(uv8)).mul(1 / 16));
        return mix(downsampleNode.sample(center), output, this.blendAmount);
    }
}
export const mipmapSurfaceBlur = (...args) => new MipmapSurfaceBlurNode(...args);
//# sourceMappingURL=MipmapSurfaceBlurNode.js.map