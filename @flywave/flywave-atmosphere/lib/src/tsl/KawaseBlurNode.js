...
export class KawaseBlurNode extends DualMipmapFilterNode {
    ...
    setupDownsampleNode(builder) {
        ...
        return add(inputNode.sample(center).mul(4), inputNode.sample(uv1), inputNode.sample(uv2), inputNode.sample(uv3), inputNode.sample(uv4)).mul(1 / 8);
    }
    setupUpsampleNode(builder) {
        ...
        return add(add(inputNode.sample(uv1), inputNode.sample(uv2), inputNode.sample(uv3), inputNode.sample(uv4)).mul(1 / 12), add(inputNode.sample(uv5), inputNode.sample(uv6), inputNode.sample(uv7), inputNode.sample(uv8)).mul(1 / 6));
    }
}
export const kawaseBlur = (...args) => new KawaseBlurNode(...args);
//# sourceMappingURL=KawaseBlurNode.js.map