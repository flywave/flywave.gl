...
import { Data3DTexture, NearestFilter, RedFormat, RepeatWrapping, Texture3DNode } from "three/webgpu";
...
export class STBNTextureNode extends Texture3DNode {
    constructor() {
        ...
        this.url = DEFAULT_STBN_URL;
    }
    customCacheKey() {
        return hashString(this.url);
    }
    setup(builder) {
        this.dataPromise ?? (this.dataPromise = new STBNLoader()
            .loadAsync(this.url)
            .then(texture => {
            texture.name = "STBN";
            this.value = texture;
...
//# sourceMappingURL=STBNTextureNode.js.map