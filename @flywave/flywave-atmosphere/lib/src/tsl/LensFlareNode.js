...
import { convertToTexture, renderTarget } from "./RenderTargetNode";
...
export class LensFlareNode extends TempNode {
    ...
    constructor(inputNode = null) {
        super("vec4");
        this.bloomIntensity = uniform(0.05);
    ...
    setup(builder) {
        const { inputNode, thresholdNode, blurNode, ghostNode, haloNode, bloomNode, featuresNode, glareNode } = this;
    ...
    getDebugInternalTexturesNode(uvNode = uv()) {
        ...
        return uvNode.y
            ...
            .select(uvNode.x
            .lessThan(0.5)
            .select(mix(blur.sample(uv.xy).rgb, vec3(1, 0, 0), luminance(threshold.sample(uv.xy).rgb).saturate()), bloom.sample(uv.zy).rgb)
            .uniformFlow(), uvNode.x
            .lessThan(0.5)
            .select(features.sample(uv.xw).rgb, glare
            .sample(uv.zw)
            .rgb.add(quadTexture
...
export const lensFlare = (inputNode) => new LensFlareNode(convertToTexture(inputNode, { name: "LensFlare [Input]" }));
