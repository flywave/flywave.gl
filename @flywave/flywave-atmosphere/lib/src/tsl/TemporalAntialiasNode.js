// @ts-nocheck
import { DepthTexture, HalfFloatType, Matrix4, RenderTarget, Vector2 } from "three";
...
import { NodeMaterial, NodeUpdateType, QuadMesh, RendererUtils, TempNode } from "three/webgpu";
...
function isSupportedCamera(camera) {
...
const varianceClipping = /*#__PURE__*/ FnVar((inputNode, coord, current, history, gamma) => {
    ...
    const variance = sqrt(moment2.div(N).sub(mean.pow2()).max(0)).mul(gamma).toConst();
...
const getCurrentDepth = /*#__PURE__*/ FnVar((depthNode, inputCoord) => builder => {
...
const subpixelCorrection = /*#__PURE__*/ FnVar((velocityUV, textureSize) => {
...
export class TemporalAntialiasNode extends TempNode {
    ...
    constructor(inputNode, depthNode, velocityNode, camera) {
        super("vec4");
        this.temporalAlpha = uniform(0.05);
        this.varianceGamma = uniform(1);
        this.velocityThreshold = uniform(0.1);
        this.depthError = uniform(0.001);
        this.debugShowRejection = false;
        this.resolveRT = this.createRenderTarget("Resolve");
        this.historyRT = this.createRenderTarget("History");
        this.resolveMaterial = new NodeMaterial();
        this.mesh = new QuadMesh();
        this.needsSyncRenderPipeline = false;
        this.needsClearHistory = false;
        this.resolveNode = texture(this.resolveRT.texture);
        this.historyNode = texture(this.historyRT.texture);
        this.previousDepthNode = texture(emptyDepthTexture);
        this.originalProjectionMatrix = new Matrix4();
        this.jitterIndex = 0;
    ...
    copyDepthTexture(renderer) {
        ...
        if (this.previousDepthTexture == null) {
            this.previousDepthTexture = current.clone();
            this.previousDepthTexture.name = "TemporalAntialias [PreviousDepth]";
        }
        const previous = this.previousDepthTexture;
        if (previous.image.width !== current.width || previous.image.height !== current.height) {
    ...
    setupResolveNode({ renderer }) {
        const getPreviousDepth = (uv) => {
            const { previousDepthNode: depthNode } = this;
            const depth = depthNode.load(ivec2(uv.mul(depthNode.size()).sub(0.5))).toConst();
            return renderer.logarithmicDepthBuffer
                ? logarithmicToPerspectiveDepth(depth, cameraNear(this.camera), cameraFar(this.camera))
        ...
        return Fn(() => {
            ...
            const expectedDepth = renderer.logarithmicDepthBuffer
                ? logarithmicToPerspectiveDepth(closestDepth, cameraNear(this.camera), cameraFar(this.camera))
                : closestDepth;
            const depthConfidence = step(expectedDepth.add(velocityUVW.z), prevDepth.add(this.depthError));
            const confidence = velocityConfidence.mul(depthConfidence);
            const uvWeight = and(prevUV.greaterThanEqual(0).all(), prevUV.lessThanEqual(1).all()).toFloat();
            ...
            If(uvWeight.mul(depthWeight).mul(confidence).greaterThan(0), () => {
                ...
                const clippedColor = varianceClipping(this.inputNode, coord, outputColor, historyColor, this.varianceGamma);
                ...
                const temporalAlpha = mix(this.temporalAlpha, 0.4, subpixelCorrection(velocityUVW.xy, textureSize(this.inputNode))).saturate();
...
export function temporalAntialias(...args) {
    if (args.length === 1) {
        return (inputNode, depthNode, velocityNode, camera) => new TemporalAntialiasNode(convertToTexture(inputNode, { name: "TemporalAntialias [Input]" }), depthNode, velocityNode, camera);
    }
    const [inputNode, depthNode, velocityNode, camera] = args;
    return new TemporalAntialiasNode(convertToTexture(inputNode, { name: "TemporalAntialias [Input]" }), depthNode, velocityNode, camera);
}
...
//     camera
//   )
//# sourceMappingURL=TemporalAntialiasNode.js.map