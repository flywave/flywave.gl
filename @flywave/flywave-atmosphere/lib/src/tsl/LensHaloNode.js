...
export class LensHaloNode extends TempNode {
    ...
    constructor(inputNode = null) {
        ...
        this.chromaticAberration = uniform(0.005);
        this.aspectRatio = uniform(0);
    ...
    setup(builder) {
        ...
        const sampleHalo = FnLayout({
            ...
            const result = vec3(inputNode.sample(direction.mul(offset.r).add(suv)).r, inputNode.sample(direction.mul(offset.g).add(suv)).g, inputNode.sample(direction.mul(offset.b).add(suv)).b);
...
//# sourceMappingURL=LensHaloNode.js.map