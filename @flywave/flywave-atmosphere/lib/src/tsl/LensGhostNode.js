...
export class LensGhostNode extends TempNode {
    ...
    setup(builder) {
        ...
        const color = add(sampleGhost(uvNode, direction, vec3(0.8, 0.8, 1), -5.0), sampleGhost(uvNode, direction, vec3(1, 0.8, 0.4), -1.5), sampleGhost(uvNode, direction, vec3(0.9, 1, 0.8), -0.4), sampleGhost(uvNode, direction, vec3(1, 0.8, 0.4), -0.2), sampleGhost(uvNode, direction, vec3(0.9, 0.7, 0.7), -0.1), sampleGhost(uvNode, direction, vec3(0.5, 1, 0.4), 0.7), sampleGhost(uvNode, direction, vec3(0.5, 0.5, 0.5), 1), sampleGhost(uvNode, direction, vec3(1, 1, 0.6), 2.5), sampleGhost(uvNode, direction, vec3(0.5, 0.8, 1), 10));
        return color.mul(intensity);
    }
}
//# sourceMappingURL=LensGhostNode.js.map