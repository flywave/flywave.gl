...
const LINEAR_SRGB_TO_LINEAR_REC2020 = mat3(vec3(0.6274, 0.0691, 0.0164), vec3(0.3293, 0.9195, 0.088), vec3(0.0433, 0.0113, 0.8956));
const approximateDefaultContrast = FnVarFn((x) => {
    const x2 = x.pow2().toConst();
    const x4 = x2.pow2().toConst();
    return add(x4.mul(x2).mul(15.5), x4.mul(x).mul(-40.14), x4.mul(31.96), x2.mul(x).mul(-6.868), x2.mul(0.4298), x.mul(0.1191), -0.00232);
});
const insetMatrix = mat3(vec3(0.856627153315983, 0.137318972929847, 0.11189821299995), vec3(0.0951212405381588, 0.761241990602591, 0.0767994186031903), vec3(0.0482516061458583, 0.101439036467562, 0.811302368396859));
const outsetMatrix = mat3(vec3(1.1271005818144368, -0.1413297634984383, -0.14132976349843826), vec3(-0.11060664309660323, 1.157823702216272, -0.11060664309660294), vec3(-0.016493938717834573, -0.016493938717834257, 1.2519364065950405));
...
export const agxPunchyToneMapping = FnLayout({
    ...
    value.assign(cdl(value, vec3(1), vec3(0), vec3(1.35), float(1.2)));
    value.assign(agxEOTF(value));
    return value;
});
export const AgXPunchyToneMapping = 100;
//# sourceMappingURL=AgxToneMapping.js.map