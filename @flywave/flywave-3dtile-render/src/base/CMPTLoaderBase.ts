...
export interface CMPTDescription extends Description {
    tiles: TileData[];
}

export abstract class CMPTLoaderBase<ParseData> extends LoaderBase<CMPTDescription, ParseData> {
    async unpack(buffer: ArrayBuffer): Promise<CMPTDescription> {
        ...
        for (let i = 0; i < tilesLength; i++) {
            ...
            const tileByteLength = tileView.getUint32(8, true);

            const tileBuffer = new Uint8Array(buffer, offset, tileByteLength);
            tiles.push({
                type: tileMagic,
                buffer: tileBuffer,
                version: tileVersion
