...
export function sectionProfileToSVG(
    ...
    projectedProfiles.forEach((profile, index) => {
        ...
        profile.stratumProfiles.forEach(stratum => {
            const color = colorMap.getStratumColor(stratum.stratumID);
            ...
            polygon.setAttribute("fill", rgbaToHex(color));
            polygon.setAttribute("fill-opacity", "0.4");
            polygon.setAttribute("stroke", rgbaToHex(color));
        ...
        profile.collapseProfiles.forEach(collapse => {
            const color = colorMap.getCollapseColor(collapse.collapseID);
...
export function createSectionPlanes(
    ...
    profiles.forEach((profile, profileIndex) => {
        ...
        profile.collapseProfiles.forEach(collapse => {
            // 获取塌陷材质
            const color = colorMap.getCollapseColor(collapse.collapseID);
            const material = getMaterial(color, 0.7);
