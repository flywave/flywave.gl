...
import { Vector3, Vector3Like, Vector3Tuple } from "three";

import DEMData from "./DemData";
...
export default class DemMinMaxQuadTree {
    private _maximums: Float32Array;
    private _minimums: Float32Array;
    private _leaves: Uint8Array;
    private _childOffsets: Int32Array;
...
const buildDemMipmap = (dem: DEMData): MipLevel[] => {
    ...
    for (let y = 0; y < blockCount; y++) {
        for (let x = 0; x < blockCount; x++) {
            ...
            const e0 = sampleElevation(minx, miny, dem) - dem.getDigBoxHeight(minx, miny);
            const e1 = sampleElevation(maxx, miny, dem) - dem.getDigBoxHeight(maxx, miny);
            const e2 = sampleElevation(maxx, maxy, dem) - dem.getDigBoxHeight(maxx, maxy);
            const e3 = sampleElevation(minx, maxy, dem) - dem.getDigBoxHeight(minx, maxy);
