...
export class BrushEngine {
  ...
  public applyBrushOperation(heightmap: Heightmap, operation: BrushOperation): Heightmap {
    ...
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        // Check boundaries
        if (pixelX < 0 || pixelX >= heightmap.width || pixelY < 0 || pixelY >= heightmap.height) {
          continue;
        }
        ...
        switch (type) {
          ...
          case BrushType.FLATTEN:
            ...
            newHeight = this.applyFlatten(currentHeight, targetHeight, weight, strength);
            break;
   ...
   */
  private applySmooth(heightmap: Heightmap, x: number, y: number, radius: number, strength: number): number {
    let sum = 0;
    let count = 0;
    ...
    return heightmap.data[y * heightmap.width + x] / 255 * (1 - strength) + averageHeight * strength;
  }
   ...
   */
  private applyFlatten(currentHeight: number, targetHeight: number, weight: number, strength: number): number {
   ...
   */
  private applyErosion(heightmap: Heightmap, x: number, y: number, weight: number, strength: number): number {
...
export class HeightmapGenerator {
    ...
    if (options?.baseNoise) {
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          heightmap.data[y * width + x] = Math.round(Math.max(0, Math.min(1, newValue)) * 255);
        }
      }
    }
  ...
  public generateCanvas(heightmap: Heightmap): HTMLCanvasElement {
    ...
    if (typeof document === 'undefined') {
      ...
      const canvas = { width: heightmap.width, height: heightmap.height, getContext: () => null } as unknown as HTMLCanvasElement;
      return canvas;
    }
   ...
   */
  public exportToDataURL(heightmap: Heightmap, format: 'png' | 'webp' = 'png', quality: number = 0.92): string {
    // Check if running in browser environment
    if (typeof document === 'undefined') {
