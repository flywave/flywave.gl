...
function createMaterial(maxSphericalHarmonicsDegree: number): NodeMaterial {
    ...
    const invViewport = uniform(new Vector2()).onObjectUpdate(({ camera, renderer }) => {
        const size = renderer.getDrawingBufferSize(new Vector2());
        invViewport.value.set(1 / size.x, 1 / size.y);
    });

    const focal = uniform(new Vector2()).onObjectUpdate(({ camera, renderer }) => {
        const size = renderer.getDrawingBufferSize(new Vector2());
        focal.value.set(
            camera.projectionMatrix.elements[0] * 0.5 * size.x,
            camera.projectionMatrix.elements[5] * 0.5 * size.y
        );
    });

    const covariancesATexture = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.covariancesATexture);

    const covariancesBTexture = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.covariancesBTexture);

    const centersTexture = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.centersTexture);

    const colorsTexture = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.colorsTexture);

    const dataTextureSize = uniform(new Vector2()).onObjectUpdate(({ object }) => {
        const tex = object.covariancesATexture;
        if (tex) dataTextureSize.value.set(tex.image.width, tex.image.height);
    });

    const shTexture0 = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.shTextures?.[0]);

    const shTexture1 = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.shTextures?.[1]);

    const shTexture2 = texture(placeholder)
        .onReference(({ object }) => object)
        .onObjectUpdate(({ object }) => object.shTextures?.[2]);
...
export class SplatMaterial {
    static build(maxSphericalHarmonicsDegree: number = 0): NodeMaterial {
        if (maxSphericalHarmonicsDegree > 0) {
            if (!_sharedMaterialSH) {
                _sharedMaterialSH = createMaterial(maxSphericalHarmonicsDegree);
            }
            return _sharedMaterialSH;
        }
        if (!_sharedMaterial) {
            _sharedMaterial = createMaterial(maxSphericalHarmonicsDegree);
        }
