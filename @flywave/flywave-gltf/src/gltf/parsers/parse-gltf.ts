...
async function loadImage(gltf: GLTFWithBuffers, image: any, index: number, options: any) {
    ...
    // Create a blob and use browser's native image decoding
    const blob = new Blob([arrayBuffer], { type: image.mimeType || "image/png" });
    ...
    if (typeof createImageBitmap === "function") {
        ...
        parsedImage = await createImageBitmap(blob);
    } else {
        // Fallback to regular Image loading
        parsedImage = await new Promise((resolve, reject) => {
            const img = new Image();
    ...
    // Store the loaded image in a format similar to what was expected
