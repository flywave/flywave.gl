import { flipPlane, Plane } from "@flywave/flywave-geoutils";
...
export const buildFn = (polygons: readonly Polygon[], node: BSPNode): BSPNode => {
    const split = polygons.reduce(
        (acc, polygon) => {
            const [coplanarFront, coplanarBack, frontSplit, backSplit] = splitPolygonByPlane(
                node.plane,
                polygon
            );
            return {
...
export const clipPolygons = (node: BSPNode, polygons: readonly Polygon[]): readonly Polygon[] => {
    ...
    return (node.front ? clipPolygons(node.front, split.front) : split.front).concat(
        node.back ? clipPolygons(node.back, split.back) : []
    );
};
