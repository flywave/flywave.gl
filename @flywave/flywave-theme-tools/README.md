...
```
npx flywave-theme-optimizer --help
```
...
To minimize the http requests for a production application the `flywave-theme-optimizer` will combine all included files into one.
Additionaly whitespaces can be removed withe the `--minify` option.

```
npx flywave-theme-optimizer --in 'my-theme-file.json' --minify
```
