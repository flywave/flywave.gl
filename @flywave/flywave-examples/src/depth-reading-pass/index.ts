...
const CONFIG = {
    ...
    INITIAL_COORDINATES: new GeoCoordinates(22.376, 109.0255, 690),
 ...
 * @param useLogarithmicDepth Whether to use logarithmic depth buffer
...
const createSampleObjects = (mapView: MapView): void => {
    ...
    // @ts-ignore
    objectGroup.anchor = new GeoCoordinates(39.7095, -105.21, 2300);
...
class DepthReadingController {
    ...
    private setupGUI(): void {
        const infoFolder = this.gui.addFolder("Depth Buffer Info");
        infoFolder
            .add(this.params, "useLogarithmicDepth")
            .name("Use Logarithmic Depth")
            .onChange((enabled: boolean) => {
                console.log(`Logarithmic depth buffer: ${enabled ? "enabled" : "disabled"}`);
                console.log("Note: Changing this setting requires page reload to take effect");
            })
            .listen();
        ...
        vizFolder
            .add(this.params, "showClickPoint")
            .name("Show Click Point")
            .onChange((enabled: boolean) => {
                if (this.clickPoint) {
        ...
        objectFolder
            .add({ clearObjects: () => this.clearPlacedObjects() }, "clearObjects")
    ...
    private handleMouseMove(event: MouseEvent): void {
        ...
        const worldPosition = this.mapView.getWorldPositionAt(x, y, false);
    ...
    private handleCanvasClick(event: MouseEvent): void {
        ...
        if (worldPosition) {
            console.log(
                `World Position: (${worldPosition.x.toFixed(2)}, ${worldPosition.y.toFixed(
                    2
            ...
            const geoPosition = this.mapView.projection.unprojectPoint(worldPosition);
            console.log(
                `Geo Position: Lat=${geoPosition.latitude.toFixed(
                    6
                )}, Lng=${geoPosition.longitude.toFixed(6)}, Alt=${
                    geoPosition.altitude?.toFixed(2) || "N/A"
                }`
            );
            ...
            this.displayInfo(ndc, worldPosition, geoPosition);
        } else {
            console.log(`No valid position at (${ndc.x.toFixed(3)}, ${ndc.y.toFixed(3)})`);
        }
    }
    ...
    private displayInfo(ndc: THREE.Vector2, worldPos: THREE.Vector3, geoPos: GeoCoordinates): void {
        ...
        infoFolder
            .add({ ndcX: ndc.x.toFixed(3) }, "ndcX")
            .name("NDC X")
            .listen();
        infoFolder
            .add({ ndcY: ndc.y.toFixed(3) }, "ndcY")
            .name("NDC Y")
            .listen();
        infoFolder
            .add({ worldX: worldPos.x.toFixed(2) }, "worldX")
            .name("World X")
            .listen();
        infoFolder
            .add({ worldY: worldPos.y.toFixed(2) }, "worldY")
            .name("World Y")
            .listen();
        infoFolder
            .add({ worldZ: worldPos.z.toFixed(2) }, "worldZ")
            .name("World Z")
            .listen();
        infoFolder
            .add({ lat: geoPos.latitude.toFixed(6) }, "lat")
            .name("Latitude")
            .listen();
        infoFolder
            .add({ lng: geoPos.longitude.toFixed(6) }, "lng")
            .name("Longitude")
            .listen();
        infoFolder
            .add({ alt: geoPos.altitude?.toFixed(2) || "N/A" }, "alt")
            .name("Altitude")
            .listen();
    }
}
...
// Get URL parameters to enable logarithmic depth buffer
const urlParams = new URLSearchParams(window.location.search);
const useLogDepth = true;

try {
    // 1. Get map canvas element
    const canvas = getMapCanvas();

    // 2. Initialize map view with optional logarithmic depth buffer
    const mapView = initializeMapView(canvas, useLogDepth);
    ...
    // Update the parameter to match actual setting
    depthController["params"].useLogarithmicDepth = useLogDepth;
    ...
    console.log(
        `Logarithmic depth buffer: ${
            useLogDepth
                ? "enabled (add ?logdepth=true to URL to use)"
                : "disabled (add ?logdepth=true to URL to enable)"
        }`
    );
} catch (error) {
