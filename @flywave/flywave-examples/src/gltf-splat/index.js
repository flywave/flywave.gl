...
import { MapView, GeoCoordinates, ellipsoidProjection, MapControls, MapControlsUI, CopyrightElementHandler } from "@flywave/flywave.gl";
...
class GLTFSplatController {
    ...
    updateModelProperties() {
        ...
        // Set rotation
        this.model.rotation.set(THREE.MathUtils.degToRad(this.params.rotation.x), THREE.MathUtils.degToRad(this.params.rotation.y), THREE.MathUtils.degToRad(this.params.rotation.z));
        // Set position offset
    ...
    initializeUI() {
        ...
        this.gui = new window.dat.GUI({ name: 'GLTF Splat Controls' });
        // Model controls
        const modelFolder = this.gui.addFolder('Model Settings');
        modelFolder.add(this.params, 'modelUrl').name('Model URL').onChange((value) => {
            this.updateParams({ modelUrl: value });
        });
        modelFolder.add(this.params, 'scale', 0.1, 10).name('Scale').onChange((value) => {
            this.updateParams({ scale: value });
        });
        modelFolder.add(this.params, 'enableSplatting').name('Enable Splatting').onChange((value) => {
            this.updateParams({ enableSplatting: value });
        });
        // Rotation controls
        const rotationFolder = this.gui.addFolder('Rotation Settings');
        rotationFolder.add(this.params.rotation, 'x', -180, 180).name('X-axis Rotation').onChange((value) => {
            this.updateParams({ rotation: { ...this.params.rotation, x: value } });
        });
        rotationFolder.add(this.params.rotation, 'y', -180, 180).name('Y-axis Rotation').onChange((value) => {
            this.updateParams({ rotation: { ...this.params.rotation, y: value } });
        });
        rotationFolder.add(this.params.rotation, 'z', -180, 180).name('Z-axis Rotation').onChange((value) => {
            this.updateParams({ rotation: { ...this.params.rotation, z: value } });
        });
        // Position controls
        const positionFolder = this.gui.addFolder('Position Settings');
        positionFolder.add(this.params.position, 'x', -100, 100).name('X-axis Position').onChange((value) => {
            this.updateParams({ position: { ...this.params.position, x: value } });
        });
        positionFolder.add(this.params.position, 'y', -100, 100).name('Y-axis Position').onChange((value) => {
            this.updateParams({ position: { ...this.params.position, y: value } });
        });
        positionFolder.add(this.params.position, 'z', -100, 100).name('Z-axis Position').onChange((value) => {
            this.updateParams({ position: { ...this.params.position, z: value } });
        });
        // Splat controls
        const splatFolder = this.gui.addFolder('Splat Settings');
        splatFolder.add(this.params, 'pointSize', 0.1, 5).name('Point Size').onChange((value) => {
            // Update point size
            console.log("Point size updated:", value);
        });
        // Render controls
        const renderFolder = this.gui.addFolder('Render Settings');
        renderFolder.add(this.params, 'animationEnabled').name('Enable Animation').onChange((value) => {
            this.updateParams({ animationEnabled: value });
        });
        renderFolder.add(this.params, 'castShadow').name('Cast Shadow').onChange((value) => {
            this.updateParams({ castShadow: value });
        });
        renderFolder.add(this.params, 'receiveShadow').name('Receive Shadow').onChange((value) => {
