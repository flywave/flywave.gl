import { MapView, GeoCoordinates, sphereProjection, MapControls, MapControlsUI, DEMTerrainSource, ArcGISTileProvider, MapViewEventNames, PostProcessingGUIModule } from "@flywave/flywave.gl";
import { GUI } from "dat.gui";
import { Mesh, MeshBasicMaterial, SphereGeometry, BoxGeometry, ConeGeometry, CylinderGeometry, TorusGeometry, Group, Color, } from "three";
/**
 ...
 */
const createGlowingObject = (geometry, color, intensity = 1.0, opacity = 0.7) => {
 ...
 */
const createPulsingGlow = (geometry, baseColor, pulseSpeed = 2) => {
    const material = new MeshBasicMaterial({
        color: baseColor,
        transparent: true,
        opacity: 0.5
 ...
 */
const createRotatingGlowGroup = (mapView) => {
    const group = new Group();
    // Center sphere
    const centerSphere = createGlowingObject(new SphereGeometry(50, 16, 16), 0xff6b6b, // Red
    1.2, 0.6);
    group.add(centerSphere);
    // Surrounding ring
    const ring = createGlowingObject(new TorusGeometry(80, 15, 16, 32), 0x4ecdc4, // Cyan
    0.8, 0.4);
    ...
    for (let i = 0; i < sphereCount; i++) {
        ...
        const sphere = createPulsingGlow(new SphereGeometry(20, 12, 12), 0x45b7d1, // Blue
        3 + i * 0.5);
 ...
 */
const createGlowingStructure = (mapView) => {
    const structure = new Group();
    // Base
    const base = createGlowingObject(new CylinderGeometry(60, 80, 40, 8), 0xffa726, // Orange
    1.0, 0.6);
    structure.add(base);
    mapView.mapRenderingManager.addBloomObject(base);
    // Middle layer
    const middle = createGlowingObject(new CylinderGeometry(40, 60, 60, 8), 0x66bb6a, // Green
    1.1, 0.7);
    middle.position.y = 50;
    structure.add(middle);
    mapView.mapRenderingManager.addBloomObject(middle);
    // Top layer
    const top = createGlowingObject(new ConeGeometry(30, 80, 8), 0xab47bc, // Purple
    1.3, 0.8);
    top.position.y = 110;
    structure.add(top);
    // Top glowing sphere
    const topSphere = createPulsingGlow(new SphereGeometry(25, 16, 16), 0xffeb3b, // Yellow
    4);
 ...
 */
const createFloatingParticles = (mapView, count = 8) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const particle = createPulsingGlow(geometry, new Color().setHSL(Math.random(), 0.8, 0.6).getHex(), 2 + Math.random() * 3);
        // Random initial position
        particle.position.set((Math.random() - 0.5) * 300, (Math.random() - 0.5) * 300, (Math.random() - 0.5) * 200);
 ...
 * @param mapView Map view instance
 */
const addGlowingObjectsToMap = (mapView) => {
 ...
 */
const startAnimationLoop = (objects) => {
    const animate = () => {
...
try {
    ...
    const objectsToAnimate = [
        mapView.mapAnchors.children.find((obj) => obj.userData.update !== undefined)
    ].filter(Boolean);
    ...
    const rotatingGlowGroup = mapView.mapAnchors.children.find((obj) => obj.anchor && obj.anchor.equals(mainCoordinates));
    const glowingStructure = mapView.mapAnchors.children.find((obj) => obj.anchor && obj.anchor.equals(structureCoords));
    const floatingParticles = mapView.mapAnchors.children.find((obj) => obj.anchor && obj.anchor.equals(mainCoordinates) && obj !== rotatingGlowGroup);
    const objectsToAnimateFull = [rotatingGlowGroup, glowingStructure, floatingParticles].filter(Boolean);
