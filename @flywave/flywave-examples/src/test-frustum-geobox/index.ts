...
const worldMatrix = new THREE.Matrix4().fromArray([
    0.5432974135116533, 0.6898197582194543, -0.4785150171536949, 0, -0.6493203781569559,
    0.7065619533364914, 0.2813418785168874, 0, 0.5321756918364856, 0.1578572369812847,
    0.831787308000653, 0, -2425041.7472774643, 4526239.929326517, 3771607.494102686, 1
]);

const projectionMatrix = new THREE.Matrix4().fromArray([
    1.4376083581701098, 0, 0, 0, 0, 2.7474774194546225, 0, 0, 0, 0, -1.6448979591836732, -1, 0, 0,
    -811.4811041403531, 0
]);
...
function extractCameraParams(matrix: THREE.Matrix4): {
...
function createUI() {
    ...
    cameraInfo.innerHTML = `
        ...
        <strong>Position:</strong> [${cameraParams.position.x.toFixed(
            2
        )}, ${cameraParams.position.y.toFixed(2)}, ${cameraParams.position.z.toFixed(2)}]<br>
        <strong>Look At:</strong> [${cameraParams.lookAt.x.toFixed(
            2
        )}, ${cameraParams.lookAt.y.toFixed(2)}, ${cameraParams.lookAt.z.toFixed(2)}]<br>
        <strong>Up Direction:</strong> [${cameraParams.up.x.toFixed(
            2
        )}, ${cameraParams.up.y.toFixed(2)}, ${cameraParams.up.z.toFixed(2)}]<br><br>
        <strong>FOV:</strong> ${(
            (2 * Math.atan(1 / projectionMatrix.elements[5]) * 180) /
            Math.PI
        ).toFixed(2)}°<br>
        <strong>Aspect Ratio:</strong> ${(
            projectionMatrix.elements[5] / projectionMatrix.elements[0]
        ).toFixed(2)}<br>
...
function createGeoBoxWithAltitudes(
    tileKey: TileKey,
    minAltitude: number,
    maxAltitude: number
): GeoBox {
...
function runTestWithTileKeyAndAltitudes(
    tileKeyStr: string,
    minAltitude: number,
    maxAltitude: number
) {
    try {
        ...
        tableHTML +=
            '<tr><th style="border: 1px solid #ddd; padding: 3px;">Level</th><th style="border: 1px solid #ddd; padding: 3px;">TileKey</th><th style="border: 1px solid #ddd; padding: 3px;">Intersection</th><th style="border: 1px solid #ddd; padding: 3px;">Status</th><th style="border: 1px solid #ddd; padding: 3px;">GeoBox [W,S,E,N,MinH,MaxH]</th></tr>';
        ...
        tableHTML += `<tr style="background-color: ${color};">
            ...
            <td style="border: 1px solid #ddd; padding: 3px; font-size: 9px;">[${geoBox.west.toFixed(
                4
            )}, ${geoBox.south.toFixed(4)}, ${geoBox.east.toFixed(4)}, ${geoBox.north.toFixed(
            4
        )}, ${geoBox.minAltitude?.toFixed(2) ?? "0"}, ${geoBox.maxAltitude?.toFixed(2) ?? "0"}]</td>
        </tr>`;
        ...
        detailsDiv.innerHTML = `
            ...
            GeoBox: [${geoBox.west.toFixed(6)}, ${geoBox.south.toFixed(6)}, ${geoBox.east.toFixed(
            6
        )}, ${geoBox.north.toFixed(6)}, ${geoBox.minAltitude?.toFixed(2) ?? "0"}, ${
            geoBox.maxAltitude?.toFixed(2) ?? "0"
        }]<br>
            OrientedBox3 Position: [${orientedBox.position.x.toFixed(
                6
            )}, ${orientedBox.position.y.toFixed(6)}, ${orientedBox.position.z.toFixed(6)}]<br>
            OrientedBox3 Extents: [${orientedBox.extents.x.toFixed(
                6
            )}, ${orientedBox.extents.y.toFixed(6)}, ${orientedBox.extents.z.toFixed(6)}]<br>
            Frustum Culling Result: ${
                isIntersecting
                    ? "Intersecting (Needs Rendering)"
                    : "Not Intersecting (Can Be Culled)"
            }<br>
