...
export class MapThumbnailGenerator {
    ...
    private setupEventListeners(): void {
        ...
        generateCustomBtn.addEventListener("click", () => {
            const width = parseInt(
                (this.container.querySelector("#customWidth") as HTMLInputElement).value
            );
            const height = parseInt(
                (this.container.querySelector("#customHeight") as HTMLInputElement).value
            );
     ...
     * 生成缩略图DataURL
     */
    private async generateThumbnailDataURL(
        width: number,
        height: number,
        quality: number = 0.8
    ): Promise<string> {
    ...
    private updatePresetButtons(): void {
        ...
        presetContainer.innerHTML = this.presetSizes
            .map(
                size => `
            <button class="preset-btn" data-width="${size.width}" data-height="${size.height}" 
                    style="padding: 8px 12px; border: 1px solid #ddd; background: #f8f9fa; 
