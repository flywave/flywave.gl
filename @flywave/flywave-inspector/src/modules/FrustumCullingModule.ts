...
export class FrustumCullingModule {
            ...
            testFrustumCulling: () => {
                try {
                    ...
                    while (currentTileKey.level >= 0) {
                        ...
                        try {
                            const elevationRange = this.elevationRangeSource.getElevationRange(
                                currentTileKey,
                            ...
                            console.warn(
                                `Could not get elevation range for tile ${currentTileKey.toString()}:`,
                                e
                            );
                        }
                        ...
                        const orientedBox = normalizedEquirectangularProjection.projectBox(
                            geoBoxWithElevation,
                            new OrientedBox3()
                        );
    ...
    private displayResults(
        ...
        for (let i = 0; i < projArray.length; i += 4) {
            resultStr += `[${projArray[i].toFixed(8)}, ${projArray[i + 1].toFixed(8)}, ${projArray[
                i + 2
            ].toFixed(8)}, ${projArray[i + 3].toFixed(8)}]\n`;
        }
        resultStr += "\nView Matrix (Camera Position/Orientation):\n";
        const viewArray = viewMatrix.toArray();
        for (let i = 0; i < viewArray.length; i += 4) {
            resultStr += `[${viewArray[i].toFixed(8)}, ${viewArray[i + 1].toFixed(8)}, ${viewArray[
                i + 2
            ].toFixed(8)}, ${viewArray[i + 3].toFixed(8)}]\n`;
        }
        resultStr += "\n";
        ...
        for (const result of results) {
            ...
            const geoBoxStr = `[${result.geoBox.west.toFixed(8)}, ${result.geoBox.south.toFixed(
                8
            )}, ${result.geoBox.east.toFixed(8)}, ${result.geoBox.north.toFixed(8)}, ${
                result.geoBox.minAltitude?.toFixed(6) ?? "0"
            }, ${result.geoBox.maxAltitude?.toFixed(6) ?? "0"}]`;
            resultStr += `${result.level}\t${result.tileKey
                .toArray()
                .join(",")}\t${intersectsStr}\t\t${geoBoxStr}\n`;
        }
