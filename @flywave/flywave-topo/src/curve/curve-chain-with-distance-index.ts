...
import { GeometryHandler, IStrokeHandler } from "../geometry3d/geometry-handler";
...
export class PathFragment {
    ...
    public quickMinDistanceToChildCurve(spacePoint: Point3d): number {
        if (this.range) return this.range.distanceToPoint(spacePoint);
...
export class CurveChainWithDistanceIndex extends CurvePrimitive {
    ...
    public override clonePartialCurve(
        ...
        if (fragmentA.childCurve === fragmentB.childCurve) {
            ...
            } // singleton -- children[] does not need to be reversed.
            return undefined;
        }
    ...
    private static flattenNestedChains(chain: CurveChain): CurveChain {
        if (
            chain.children.findIndex((child: CurvePrimitive) => {
        ...
        const flatChildren = flatChain.children.flatMap((child: CurvePrimitive) => {
            if (child instanceof CurveChainWithDistanceIndex) return child.path.children;
            else return [child];
        });
        ...
        if (numChildren === 1) {
            ...
            for (const sortedFragment of sortedFragments) {
                ...
                detailA = child.closestPoint(
                    spacePoint,
                    sortedFragment === fragment0
                        ? extend0
                        : sortedFragment === fragment1
                        ? extend1
                        : false,
                    detailA
                );
    ...
    public computeChainDetail(childDetail: CurveLocationDetail): CurveLocationDetail | undefined {
        if (!childDetail.curve) return undefined;
    ...
    public static convertChildDetailToChainDetail(
        ...
        for (let i = index0; i < pairs.length; ++i) {
            ...
            if (chainA) {
                ...
                if (chainDetail) childDetailPair.detailA = chainDetail;
            }
            if (chainB) {
                const chainDetail = chainB.computeChainDetail(childDetailPair.detailB);
                if (chainDetail) childDetailPair.detailB = chainDetail;
            }
        }
        if (compressAdjacent) {
