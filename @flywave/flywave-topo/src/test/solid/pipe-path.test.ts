...
describe("PipePath", () => {
  ...
  it("TorusPipeTransformed", () => {
    ...
    // test invalid inputs
    let pipeBad = TorusPipe.createTopoTorusPipe(center, Vector3d.createZero(), vectorY, majorRadius, minorRadius, sweep, capped);
    ck.testDefined(pipeBad, "Zero vectorX nevertheless yields defined TorusPipe");  // default vec is used!
    pipeBad = TorusPipe.createTopoTorusPipe(center, vectorX, Vector3d.createZero(), majorRadius, minorRadius, sweep, capped);
    ck.testDefined(pipeBad, "Zero vectorY nevertheless yields defined TorusPipe");  // default vec is used!
    pipeBad = TorusPipe.createTopoTorusPipe(center, vectorX, vectorY, 0, minorRadius, sweep, capped);
    ck.testUndefined(pipeBad, "Zero majorRadius yields undefined TorusPipe");
    pipeBad = TorusPipe.createTopoTorusPipe(center, vectorX, vectorY, majorRadius, 0, sweep, capped);
    ck.testUndefined(pipeBad, "Zero minorRadius yields undefined TorusPipe");
    pipeBad = TorusPipe.createTopoTorusPipe(center, vectorX, vectorY, minorRadius, majorRadius, sweep, capped);
    ck.testUndefined(pipeBad, "Swapped radii yields undefined TorusPipe");
    pipeBad = TorusPipe.createTopoTorusPipe(center, vectorX, vectorY, majorRadius, minorRadius, Angle.zero(), capped);
    ck.testUndefined(pipeBad, "Zero sweep yields undefined TorusPipe");

    let pipe0 = TorusPipe.createTopoTorusPipe(center, vectorX, vectorY, majorRadius, minorRadius, sweep, capped)!;
    ...
    for (const xAxis of [vectorX, vectorX.negate(), Vector3d.create(0, 0, 1), Vector3d.create(1, 0, 1), Vector3d.create(1, 1, 1)]) {
      ...
      for (const yAxis of [vectorY, vectorY.negate(), Vector3d.create(1, 1, 0)]) {
        x0 = 0;
        pipe0 = TorusPipe.createTopoTorusPipe(center, xAxis, yAxis, majorRadius, minorRadius, sweep, capped)!;
        ...
        for (const scale of [Point2d.create(10, 10), Point2d.create(5, 10), Point2d.create(10, 5)]) {
          ...
          const pipeScaledRadii = TorusPipe.createTopoTorusPipe(center, xAxis, yAxis, majorRadius * scale.x, minorRadius * scale.y, sweep, capped);
