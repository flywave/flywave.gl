...
import { CurveFactory, MiteredSweepOutputSelect } from "../../curve/curve-factory";
...
describe("PipeConnections", () => {
    ...
    it("createMiteredPipeSections", () => {
        ...
        for (const path of allPaths) {
            ...
            const startFrame = Matrix3d.createRotationAroundVector(
                startTangent,
                v0Angle
            )!.multiplyMatrixMatrix(Matrix3d.createRigidHeadsUp(startTangent, AxisOrder.ZXY));
    ...
    it("createRectangleXY", () => {
        ...
        const radii: Array<number | undefined> = [undefined, 0, 1, 2, 3];
        let yOut = 0.0;
        for (const yB of [4, -4]) {
            let xOut = 0.0;
            for (const xB of [6, -6]) {
    ...
    it("createMiteredSweep", () => {
        ...
        centerlines.push([
            Point3d.create(0, 0, 0),
            Point3d.create(0, 0, 10),
            Point3d.create(0, 12, 10)
        ]);
        // closed planar
        centerlines.push([
            Point3d.create(0, 0, 0),
        ...
        for (const centerline of centerlines) {
            ...
            for (let i = 0; i + 1 < centerline.length; i++) {
                if (Geometry.isSamePoint3d(centerline[i], centerline[i + 1])) n++;
            }
        ...
        for (const radiusA of [undefined, 0.0, 3.0]) {
            ...
            for (let centerlineIndex = 0; centerlineIndex < centerlines.length; centerlineIndex++) {
                ...
                const rectangleA = CurveFactory.createRectangleXY(x0, y0, x1, y1, 0, radiusA);
                const sweeps = CurveFactory.createMiteredSweepSections(centerline, rectangleA, {
                    wrapIfPhysicallyClosed: wrap
                });
                ...
                if (sweeps !== undefined) {
                    if (
                        ck.testDefined(sweeps.sections) &&
                        sweeps.sections !== undefined &&
                        ck.testDefined(sweeps.planes) &&
                        sweeps.planes !== undefined &&
                        ck.testExactNumber(
                            sweeps.planes.length,
                            sweeps.sections.length,
                            "Same number of planes and sections"
                        )
                    ) {
                        GeometryCoreTestIO.captureCloneGeometry(
                            allGeometry,
                        ...
                        ck.testExactNumber(
                            centerline.length - numDuplicates[centerlineIndex],
                            sweeps.sections.length,
                        ...
                        for (const plane of sweeps.planes) {
                            ...
                            GeometryCoreTestIO.captureCloneGeometry(
                                ...
                                [
                                    plane.getOriginRef(),
                                    plane.getOriginRef().plus(plane.getNormalRef())
                                ],
                                x0Out,
                                y0Out
                            );
                        ...
                        for (let i = 0; i < sweeps.sections.length; i++) {
                            ck.testTrue(
                                isGeometryInPlane(sweeps.sections[i], sweeps.planes[i]),
                                "geometry in plane"
                            );
                        }
                    }
    ...
    it("createMiteredSweepStadiumExample", () => {
        ...
        const zigZag = Sample.createZigZag(
            sweepOrigin,
            [Vector3d.create(0, stepSize, 0), Vector3d.create(0, 0, stepSize)],
            numZigZagEdge
        );
            ...
            for (const section of [sectionA, sectionB, ovalSection]) {
                ...
                if (
                    ...
                    GeometryCoreTestIO.captureCloneGeometry(
                        ...
                        section,
                        x0,
                        yMesh - ay * sweepShiftFraction
                    );
                    GeometryCoreTestIO.captureCloneGeometry(
                        allGeometry,
                        section,
                        x0,
                        ySurface - ay * sweepShiftFraction
                    );
        ...
        GeometryCoreTestIO.saveGeometry(
            allGeometry,
            "CurveFactory",
            "createMiteredSweepStadiumExample"
        );
        expect(ck.getNumErrors()).equals(0);
    });
});
