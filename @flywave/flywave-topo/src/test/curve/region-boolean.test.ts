...
import { CurveCurve } from "../../curve/curve-curve";
...
import { AnyCurve, AnyRegion } from "../../curve/curve-types";
...
import { DuplicateFacetClusterSelector, PolyfaceQuery } from "../../polyface/polyface-query";
...
describe("RegionBoolean", () => {
    ...
    it("ParityRegionWithCoincidentBoundary", () => {
        ...
        const expected: SignedLoopCounts[] = [
            { numPositiveAreaLoops: 2, numNegativeAreaLoops: 1, numSlivers: 2, numEdges: 8 }
        ];
    ...
    it("BananaRegion", () => {
        ...
        const expected: SignedLoopCounts[] = [
            { numPositiveAreaLoops: 1, numNegativeAreaLoops: 1, numSlivers: 0, numEdges: 2 }
        ];
    ...
    it("BridgeEdgesAndDegenerateLoops", () => {
        ...
        const testCases: TestInput[] = [
            ...
            {
                jsonFilePath: "./src/test/testInputs/curve/laurynasRegion9.imjs",
                expectedNumComponents: 2
            },
            {
            ...
            {
                jsonFilePath: "./src/test/testInputs/curve/laurynasCircularHole.imjs",
                expectedNumComponents: 1
            },
            {
                jsonFilePath: "./src/test/testInputs/curve/laurynasCircularHole2.imjs",
                expectedNumComponents: 4,
                skipBoolean: true
            }, // without merge, 4 separate loops
            {
                jsonFilePath: "./src/test/testInputs/curve/dovydasLoops.imjs",
                expectedNumComponents: 1
            } // union makes bridges to three holes along the bridge ray
        ];
                ...
                if (merged) {
                    ...
                    for (const signedLoop of signedLoops) {
                        ...
                        ck.testExactNumber(
                            ...
                            `SignedLoop has one outer loop: ${testCase.jsonFilePath}`
                        );
                    }
                }
            }
    ...
    it("OverlappingArcs", () => {
        ...
        const testCases = [
            // numOverlap is number of non-trivial coincident intervals
            {
                sweepStartA: 90,
                sweepEndA: 0,
                sweepStartB: 90,
            ...
            },
            {
                ...
                numLoop: 1,
                numOverlap: 0
            }, // Laurynas' original 2-arc circle
            {
                sweepStartA: 190,
                sweepEndA: -10,
                sweepStartB: 0,
        ...
        for (const testCase of testCases) {
            ...
            arcPair.push(
                Arc3d.create(
                    ...
                    vector0,
                    vector90,
                    AngleSweep.createStartEndDegrees(testCase.sweepStartA, testCase.sweepEndA)
                )
            );
            arcPair.push(
                Arc3d.create(
                    ...
                    vector0,
                    vector90B,
                    AngleSweep.createStartEndDegrees(testCase.sweepStartB, testCase.sweepEndB)
                )
            );
            x0 = 0;
            ...
            if (
                ck.testExactNumber(
                    1,
                    signedLoops.length,
                    "Overlapping arcs yield one connected component."
                )
            ) {
                ck.testExactNumber(
                    testCase.numLoop,
                    signedLoops[0].positiveAreaLoops.length,
                    "SignedLoop has expected number of inner loops."
                );
                ck.testExactNumber(
                    testCase.numLoop,
                    signedLoops[0].negativeAreaLoops.length,
                    "SignedLoop has expected number of outer loops."
                );
                ...
                if (testCase.numLoop === 0) ++expectedNumSliver;
                ck.testExactNumber(
                    expectedNumSliver,
                    signedLoops[0].slivers.length,
                    "SignedLoop has expected number of sliver faces."
                );
...
describe("PlaneAltitudeRangeContext", () => {
        ...
        for (const geom of [
            ...
            for (const rayParam of rayParams) {
                if (geom instanceof CurveCollection)
                    ...
                    lowHigh = PlaneAltitudeRangeContext.findExtremeFractionsAlongDirection(
                        geom,
                        rayParam.ray,
                        lowHigh
                    );
    ...
    it("ClosedCurve", () => {
        ...
        const fitCurveLoop = IModelJson.Reader.parse(
            JSON.parse(
                fs.readFileSync("./src/test/testInputs/curve/interpolationCurveLoop.imjs", "utf8")
            )
        ) as Loop;
        ...
        for (const param of params) {
            GeometryCoreTestIO.captureCloneGeometry(
                ...
                [
                    param.geom,
                    LineSegment3d.create(param.ray.origin, param.ray.fractionToPoint(1.0))
                ],
                x0
            );
            x0 += 10;
    ...
    if (Checker.noisy.parityRegionAnalysis) {
        RegionOps.setCheckPointFunction(
            (name: string, graph: HalfEdgeGraph, _properties: string, _extraData?: any) => {
                ...
                const euler =
                    graph.countVertexLoops() - graph.countNodes() / 2.0 + graph.countFaceLoops();
