...
describe("CurveChainWithDistanceIndex", () => {
    ...
    it("closestApproachChainLoops", () => {
        const ck = new Checker();
        const allGeometry: GeometryQuery[] = [];
        const semicircleCW = Arc3d.createXY(
            Point3d.createZero(),
            3,
            AngleSweep.createStartEndDegrees(-45, -225)
        );
        const diameter = LineSegment3d.createCapture(
            semicircleCW.endPoint(),
            semicircleCW.startPoint()
        );
        const chainA = CurveChainWithDistanceIndex.createCapture(
            Loop.createArray([semicircleCW, diameter])
        );
        const lowestPolygonPoint = Point3d.create(-4, -4, 2);
        const skewPlanePolygon = [
            Point3d.create(4, 4, 8),
            Point3d.create(-4, 4, 5),
            lowestPolygonPoint,
            Point3d.create(4, -4, 5)
        ];
        const chainB = CurveChainWithDistanceIndex.createCapture(
            Loop.createPolygon(skewPlanePolygon)
        );
        ...
        if (ck.testExactNumber(pairs.length, 2, "found two close approaches")) {
            ...
            ck.testCoordinate(
                xyApproachDistance0,
                xyApproachDistance1,
                "close approaches are the same distance"
            );
            ...
            if (
                ck.testType(
                    xyzApproachDistance,
                    CurveLocationDetail,
                    "found 3D closest approach between chains"
                )
            ) {
                ck.testLT(
                    xyzApproachDistance.a,
                    xyApproachDistance3D,
                    "3D closest approach is actually closer"
                );
            }
            for (const pair of pairs) {
                if (Geometry.isSmallRelative(pair.detailA.fraction - xyApproachFractionA0)) {
                    ck.testTrue(
                        Geometry.isSmallRelative(pair.detailB.fraction - xyApproachFractionB0),
                        "has expected close approach 0"
                    );
                } else if (Geometry.isSmallRelative(pair.detailA.fraction - xyApproachFractionA1)) {
                    ck.testTrue(
                        Geometry.isSmallRelative(pair.detailB.fraction - xyApproachFractionB1),
                        "has expected close approach 1"
                    );
                } else ck.announceError("unexpected close approach");
            }
    ...
    it("intersectXYZChainSegment", () => {
        ...
        GeometryCoreTestIO.captureCloneGeometry(allGeometry, geometryB);
        // test intersectionXY global fractions
        const intersectionXYAB = CurveCurve.intersectionXYZPairs(
            geometryA,
            false,
            geometryB,
            false
        );
        const intersectionXYBA = CurveCurve.intersectionXYZPairs(
            geometryB,
            false,
            geometryA,
            false
    ...
    it("intersectXYZChainString", () => {
        ...
        const geometryB = LineString3d.create(
            [-3, 0, -1],
            [-3, 0, 1],
            [2, 0, 1],
            [2, 0, -1],
    ...
    it("intersectXYZChainChain", () => {
        ...
        GeometryCoreTestIO.saveGeometry(
            allGeometry,
            "CurveChainWithDistanceIndex",
            "intersectXYZChainChain"
        );
        expect(ck.getNumErrors()).equals(0);
    });
    it("recursionAvoidance", () => {
        const ck = new Checker();
        const pts: Point3d[] = Array(7)
            .fill(null)
            .map((_v: Point3d, i: number) => {
                return Point3d.create(i, i, i);
            });
        const childChain0 = CurveChainWithDistanceIndex.createCapture(
            Path.create([pts[1], pts[2], pts[3]])
        );
        const childChain1 = CurveChainWithDistanceIndex.createCapture(
            Path.create([pts[4], pts[5], pts[6]])
        );
        const path = Path.create(
            LineSegment3d.create(pts[0], pts[1]),
            childChain0,
            LineSegment3d.create(pts[3], pts[4]),
            childChain1
        );
