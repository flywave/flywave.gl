...
import { AnyCurve } from "../../curve/curve-types";
    ...
    for (const f0 of fractions) {
        ...
        for (const f1 of fractions) {
            ...
            if (approaches.length > 0) {
                ...
                if (!(geometryA instanceof Arc3d)) {
                    // due to the "NO NO NO" bug in testAndRecordProjection
                    _ck.testCoordinate(
                        0,
            ...
            if (f0 !== f1) {
                ...
                if (approaches.length > 0) {
                    ...
                    if (!(geometryA instanceof Arc3d)) {
                        ...
                        _ck.testCoordinate(
                            0,
                            CurveCurve.intersectionXYPairs(circleB0, false, geometryA, false)
                                .length,
                            "expect no intersection"
                        );
                        _ck.testCoordinate(
                            0,
                            CurveCurve.intersectionXYPairs(circleB1, false, geometryA, false)
                                .length,
                            "expect no intersection"
                        );
                    }
...
describe("CurveCurveCloseApproachXY", () => {
    ...
    it("SingleLineLine1", () => {
        ...
        const minLenSqr = closestApproach!.detailA.point.distanceSquaredXY(
            closestApproach!.detailB.point
        );
    ...
    it("SingleLineLine2", () => {
        ...
        ck.testCoordinate(
            approaches[0].detailA.fraction,
            1.0,
            "closest approach has expected fraction on curveA"
        );
        ck.testCoordinate(
            approaches[0].detailB.fraction,
            0.1,
            "closest approach has expected fraction on curveB"
        );
        GeometryCoreTestIO.captureGeometry(allGeometry, approachSegment);
        // test the convenience method
        const closestApproach = CurveCurve.closestApproachProjectedXYPair(geometryA, geometryB);
    ...
    it("LinePath2", () => {
        ...
        const approaches1 = CurveCurve.closeApproachProjectedXYPairs(
            geometryA,
            geometryB1,
            maxDistance
        );
        ...
        const lineSegment5 = LineSegment3d.create(
            Point3d.create(5, 0, 1),
            Point3d.create(6, 3, -2)
        );
        const lineSegment6 = LineSegment3d.create(
            Point3d.create(6, 3, -2),
            Point3d.create(7, 0, 1)
        );
        ...
        // find approaches
        const approaches2 = CurveCurve.closeApproachProjectedXYPairs(
            geometryA,
            geometryB2,
            maxDistance
        );
        ...
        for (let i = 0; i < approach1Len; i++) {
            ck.testPoint3d(approaches1[i].detailA.point, approaches2[i * 2].detailA.point, [
                "failed for approach1 index: ",
                i
            ]);
            ck.testPoint3d(approaches1[i].detailB.point, approaches2[i * 2].detailB.point, [
                "failed for approach2 index: ",
                i
            ]);
        }
        // test the convenience method
    ...
    it("LineLoop2", () => {
        ...
        ck.testDefined(closestApproach1);
        ck.testDefined(closestApproach2);
        const minLenSqr1 = closestApproach1!.detailA.point.distanceSquaredXY(
            closestApproach1!.detailB.point
        );
        const minLenSqr2 = closestApproach2!.detailA.point.distanceSquaredXY(
            closestApproach2!.detailB.point
        );
        const expectedMinLenSqr = 4;
        ck.testLE(
            Math.sqrt(minLenSqr1),
    ...
    it("SingleLineStringLineString", () => {
        ...
        const allGeometry: GeometryQuery[] = [];
        const maxDistance = 2;
        const geometryA = LineString3d.create(
            [-1, 1],
            [0, 0],
            [1, 1],
            [2, 0],
