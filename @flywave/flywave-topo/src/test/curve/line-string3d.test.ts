...
function exerciseClonePartialLineString3d(
    ...
    for (const extendFraction of [0.05, 0.721, 1.4, 3.96]) {
        ...
        const postLocalFractionHalf =
            lsA.globalFractionToSegmentIndexAndLocalFraction(1 + extendFraction / 2).fraction - 1;
        ...
        if (ck.testPointer(c1e) && expectValidResults) {
            ...
            if (!lsA.isPhysicallyClosed)
                ck.testCoordinate(
                    c1e.curveLength(),
                    postLocalFraction *
                        lsA.points[lsA.numPoints() - 2].distance(lsA.points[lsA.numPoints() - 1]),
...
describe("LineString3dOps", () => {
    it("CollectChainsAsLineString3d", () => {
        ...
        const createFragments = (perturbClosure: boolean, includeDangler: boolean): AnyCurve[] => {
            ...
            if (includeDangler)
                fragments.push(
                    LineSegment3d.create(Point3d.create(4, 4, 0), Point3d.create(3, 2, 0))
                );
            fragments.push(
                Arc3d.create(
                    Point3d.create(1, 0, 0),
            ...
            fragments.push(
                LineSegment3d.create(
                    Point3d.create(0, -1, 0),
                    Point3d.create(1, perturbClosure ? -2 * Geometry.smallMetricDistance : 0, 0)
                )
            );
            return fragments;
        };
        ...
        for (const select of [true, false]) {
            ...
            CurveOps.collectChainsAsLineString3d(frags, (pts: LineString3d) => {
                if (pts.isPhysicallyClosed) loops.push(Loop.create(pts));
                else paths.push(Path.create(pts));
            });
            if (select) {
                ck.testExactNumber(
                    loops.length,
                    0,
                    "CurveOps.collectChainsAsLineString3d forms no loops as expected"
                );
                ck.testExactNumber(
                    paths.length,
                    2,
                    "CurveOps.collectChainsAsLineString3d forms 2 paths as expected"
                );
            } else {
                ck.testExactNumber(
                    loops.length,
                    2,
                    "CurveOps.collectChainsAsLineString3d forms 2 loops as expected"
                );
                ck.testExactNumber(
                    paths.length,
                    0,
                    "CurveOps.collectChainsAsLineString3d forms no paths as expected"
                );
            }
