...
import { AustralianRailCorpXYEvaluator } from "../../curve/spiral/australian-rail-corp-xy-evaluator";
import { ClothoidSeriesRLEvaluator } from "../../curve/spiral/clothoid-series";
import { CzechSpiralEvaluator } from "../../curve/spiral/czech-spiral-evaluator";
import { DirectHalfCosineSpiralEvaluator } from "../../curve/spiral/direct-half-cosine-spiral-evaluator";
...
} from "../../curve/spiral/normalized-transition";
import { PolishCubicEvaluator } from "../../curve/spiral/polish-cubic-spiral-evaluator";
import { TransitionConditionalProperties } from "../../curve/spiral/transition-conditional-properties";
...
describe("TransitionSpiral3d", () => {
    ...
    it("ClothoidTerms", () => {
        ...
        for (const radius1 of [200, 400, 1000]) {
            ...
            for (const d of [
                0, 10, 20, 30, 40, 50, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 80, 90,
                100
            ]) {
                distances.push(d);
                const f = d / distance1;
    ...
    it("NamedApproximations", () => {
        ...
        GeometryCoreTestIO.consoleLog(
            `Czech gamma factor ${CzechSpiralEvaluator.gammaConstant(nominalL1, nominalR1)}`
        );
        for (const spiral of [
            mxCubicAlongArc,
            ...
            GeometryCoreTestIO.consoleLog(
                ` ${spiral.spiralType}  Y1  / Y4  ${spiral.evaluator.fractionToY(1) / y4}`
            );
    ...
    it("AlexGStroking", () => {
        ...
        const alignment = IModelJson.Reader.parse(
            JSON.parse(
                fs.readFileSync("./src/test/testInputs/curve/AlexGSpiral/AlexGSpiral.imjs", "utf8")
            )
        );
    ...
    it("AlexGSamples", () => {
        ...
        if (fileList) {
            for (const fileName of fileList) {
                ...
                const allGeometry: GeometryQuery[] = [];
                if (
                    ck.testTrue(alignment instanceof Path, "expect one alignment in test file") &&
    ...
    it("test TransitionSpiral3d clonePartialCurve", () => {
        ...
        for (const spiral of [
            ...
            if (spiral instanceof IntegratedSpiral3d) {
                ...
                ck.testTrue(
                    (partial as IntegratedSpiral3d).bearing01.isAlmostEqual(spiral.bearing01)
                );
                ck.testTrue(
                    (partial as IntegratedSpiral3d).radius01.isAlmostEqual(spiral.radius01)
                );
...
it("AlexGProjectPointToChain", () => {
    ...
    const alignment = IModelJson.Reader.parse(
        JSON.parse(
            fs.readFileSync("./src/test/testInputs/curve/AlexGSpiral/pathWithSpirals.imjs", "utf8")
        )
    );
    captureStroked(allGeometry, alignment);
    if (alignment instanceof Path) {
        ...
        GeometryCoreTestIO.captureCloneGeometry(allGeometry, alignment);
        const fractions = [
            -0.2, -0.1, 0.0, 0.1, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.8,
            0.9, 1.0, 1.1, 1.2
        ];
