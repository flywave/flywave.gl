...
import { DeepCompare } from "../../serialization/deep-compare";
...
const allJsonSamples: { [key: string]: any } = {};
...
function exerciseIModelJSon(ck: Checker, g: any, doParse: boolean = false, noisy: boolean = false) {
  ...
  if (g instanceof GeometryQuery) {
    ...
    saveJson(imData, allJsonSamples);
    if (noisy)
...
//
// JsonSamples workflow:
// * Each execution of it("CreateIModelJsCreateJsonSamplesonSamples") constructs GeometryQuery objects and saves them in the test output
//            path     test/output/JsonSamples
// * a copy of those is saved in path     test/JsonSamples
// * Each execution of it("ReadIModelJson") reads tht saved files in test/JsonSamples, converts to GeometryQuery, converts that back to
//     json and does a deep compare of the before/after json
//

describe("CreateJsonSamples", () => {
  it("GeometryQueryToIModelJS", () => {
    ...
    GeometryCoreTestIO.savePropertiesAsSeparateFiles(iModelJsonOutputSubFolder, allJsonSamples);
    exerciseIModelJSonArray(ck, Sample.createSmoothCurvePrimitives(numSample), true, false);

    // GeometryCoreTestIO.consoleLog(allJsonSamples);
  ...
  it("ReadIModelJson", () => {
    ...
    for (const sourceDirectory of [JsonSamplesDirectory, iModelJsonNativeSamplesDirectory]) {
