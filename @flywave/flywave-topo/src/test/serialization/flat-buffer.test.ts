...
import { AuxChannelDataType } from "../../polyface/aux-data";
import { TaggedNumericData } from "../../polyface/tagged-numeric-data";
import { GeometryFlatBuffer } from "../../serialization/geometry-flat-buffer";
import { BGFBAccessors } from "../../serialization/bgfb-accessors";
...
function testGeometryQueryRoundTripGo(ck: Checker, g: GeometryQuery | GeometryQuery[] | undefined) {
  ...
  if (g instanceof GeometryQuery) {
    const justTheBytes = GeometryFlatBuffer.geometryToBytes(g);

    if (ck.testType(justTheBytes, Uint8Array)) {
      const g1 = GeometryFlatBuffer.bytesToGeometry(justTheBytes);
      if (Array.isArray(g1)) {
        ...
        if (!ck.testTrue(g.isAlmostEqual(g1 as GeometryQuery))) {
          ...
          const g2 = GeometryFlatBuffer.bytesToGeometry(justTheBytes);
          if (g2 instanceof GeometryQuery)
            ck.testTrue(g.isAlmostEqual(g2));
        }
      }
    }
    const bytesWithSignature = GeometryFlatBuffer.geometryToBytes(g, true);
    if (justTheBytes && bytesWithSignature) {
      ...
      const g3 = GeometryFlatBuffer.bytesToGeometry(bytesWithSignature, true);
      ck.testTrue(g3 instanceof GeometryQuery && g.isAlmostEqual(g3), "round trip with signature");
      ck.testUndefined(GeometryFlatBuffer.bytesToGeometry(justTheBytes, true), "signature state mismatch A");
      ck.testUndefined(GeometryFlatBuffer.bytesToGeometry(bytesWithSignature, false), "signature state mismatch B");
    }
...
it("HelloNativeBytes", () => {
  ...
  for (const nativeBytes of [sphereBytes, singleSegmentBytes, singleArcBytes, arcBytes]) {
    const g0 = GeometryFlatBuffer.bytesToGeometry(nativeBytes, true);
    if (Checker.noisy.flatBuffer)
    ...
    if (ck.testDefined(g0, "native bytes to geometry") && g0) {
      ...
      const jsBytes = GeometryFlatBuffer.geometryToBytes(g0, true);
      if (Checker.noisy.flatBuffer) {
      ...
      if (ck.testDefined(jsBytes, "geometry to bytes") && jsBytes) {
        const g1 = GeometryFlatBuffer.bytesToGeometry(jsBytes, true);
