...
describe("constructorsAndImodelJson", () => {
  ...
  it("constructorsAndImodelJson.SolidPrimitives", () => {
...

    {
      ...
      const bx = 4.0;
      const by = 2.0;
      emitIModelJson("Box.createTopoBox(cornerA, xVector, yVector, baseX, baseY, topX, topY, capped)",
        "box with sides slanting inward",
        Box.createTopoBox(cornerA, Vector3d.unitX(), Vector3d.unitY(),
  ...
  it("MeshWithTag", () => {
    ...
    const surface = TorusPipe.createTopoTorusPipe(Point3d.create(0, 0, 0), Vector3d.create(1, 0, 0), Vector3d.create(0, 1, 0), 4, 1, Angle.createDegrees(90), true)!;
