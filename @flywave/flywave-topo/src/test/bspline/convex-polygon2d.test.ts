...
import { ConvexPolygon2d } from "../../numerics/convex-polygon2d";
...
describe("ConvexPolygon2d", () => {
    ...
    it("Ray2d", () => {
        ...
        const checkResultArgIsUsed = (
            functionName: string,
            resultArg: Ray2d,
            returnVal: Ray2d,
            expectedOrigin: Point2d,
            expectedDirection: Vector2d
        ) => {
            ck.testPoint2d(
                expectedOrigin,
                resultArg.origin,
                `Ray2d.${functionName} sets expected origin`
            );
            ck.testVector2d(
                expectedDirection,
                resultArg.direction,
                `Ray2d.${functionName} sets expected direction`
            );
            ck.testTrue(
                returnVal.origin === resultArg.origin,
                `Ray2d.${functionName} uses result.origin`
            );
            ck.testTrue(
                returnVal.direction === resultArg.direction,
                `Ray2d.${functionName} uses result.direction`
            );
        };
        let ray3 = Ray2d.createOriginAndTarget(ray0.origin, pointA, ray1);
        checkResultArgIsUsed(
            "createOriginAndTarget",
            ray1,
            ray3,
            ray0.origin,
            Vector2d.createStartEnd(ray0.origin, pointA)
        );
        ...
        ck.testTrue(
            vec === ray3.direction,
            "Ray2d.createOriginAndDirectionCapture captures direction"
        );
