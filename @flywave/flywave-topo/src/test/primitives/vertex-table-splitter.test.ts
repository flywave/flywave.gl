...
import { Id64 } from "../../utils";
import { Point2d, Range2d } from "../..//core-geometry";
import {
...
import { EdgeParams, SegmentEdgeParams } from "../../common/render/primitives/edge-params";
...
} from "../../common/render/primitives/vertex-table-splitter";
...
function makeMeshParams(mesh: TriMesh): MeshParams {
  ...
  return createMeshParams(args, renderSystem!.maxTextureSize);
}
...
describe("VertexTableSplitter", () => {
  ...
  function testMeshParams(adjustPt?: (pt: TriMeshPoint) => TriMeshPoint): void {
    ...
    const split = splitMeshParams({
      params, featureTable, maxDimension: 2048,
      computeNodeId: makeComputeNodeId(featureTable, (id) => id.lower),
      createMaterial: (args) => renderSystem?.createRenderMaterial(args),
    });
