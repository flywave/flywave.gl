...
describe("Ellipsoid", () => {
    ...
    it("Singular", () => {
        ...
        ck.testUndefined(
            patch.anglesToUnitNormalRay(LongitudeLatitudeNumber.createDegrees(0, 0, 0))
        );
        ...
        ck.testExactNumber(
            0,
            ellipsoid.intersectRay(Ray3d.createZAxis(), undefined, undefined, undefined)
        );
            ...
            for (const e of [1.0, 2.0, 0.5]) {
                ...
                for (const phiDegrees of degrees) {
                    for (const thetaDegreesA of degrees) {
                        ...
                        const myAngles = ellipsoid.otherEllipsoidAnglesToThisEllipsoidAngles(
                            unitSphere,
                            anglesOnUnitSphere
                        )!;
                        ...
                        const myAngles1 = ellipsoid.otherEllipsoidAnglesToThisEllipsoidAngles(
                            undefined,
                            anglesOnUnitSphere
                        )!;
    ...
    it("SphereOffsetPatchRange", () => {
        ...
        for (const offsetDistance of [0.2, 0.0, -0.3]) {
            ...
            for (const patchB of [
                ...
                const diff10 =
                    rangeA10.low.distance(rangeB.low) + rangeA10.high.distance(rangeB.high);
...
function testEllipsoidPaths(
    ...
    for (let i = 1; i < anglePoints.length; i++) {
        ...
        if (ck.testDefined(arc) && arc) {
            ...
            for (const angle of [
                /* Angle.createDegrees(10), Angle.createDegrees(5), */ Angle.createDegrees(2)
            ]) {
