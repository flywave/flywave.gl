...
describe("AnalyticRoots.SolveQuadric", () => {
    ...
    it("PowerPolynomial", () => {
        ...
        ck.testTrue(
            TrigPolynomial.solveAngles(new Float64Array([0, 1, 1, 0, 0, 0]), 4, 100, radians)
        );
    ...
    it("SolveCubic.Cubic3", () => {
        ...
        for (let e = 1.0; e > 2.0e-8; e *= 0.1) {
            for (let x0 = 0.0; x0 < Math.max(rangeScale * e, 20.0); x0 = 10.0 * (x0 + 1.0)) {
                ...
                const u2 = x0 + 1;
                if (Checker.noisy.cubicRoots) {
                    GeometryCoreTestIO.consoleLog(`Cubic Roots for [${u0}, ${u1}, ${u2}]`, {
                        ee: e,
                        x00: x0
                    });
                }
    ...
    for (let step = 0; step < 10 && eMax > 1.0e-14; step++) {
        if (!ok || printAll) {
            GeometryCoreTestIO.consoleLog(
                ...
                )} ${newtonStep4(coffs, actual.atUncheckedIndex(2))} ${newtonStep4(
                    coffs,
            ...
            for (let k = 0; k < actual.length; k++) {
                actual.reassign(
                    k,
                    actual.atUncheckedIndex(k) - newtonStep4(coffs, actual.atUncheckedIndex(k))
                );
            }
...
export class AnalyticRoots1 extends AnalyticRoots {
    public static appendQuarticRoots1(c: Float64Array, roots: GrowableFloat64Array) {
        ...
        const r =
            (-3.0 / 256) * squareA * squareA + (1.0 / 16) * squareA * B - (1.0 / 4) * A * C + D;
