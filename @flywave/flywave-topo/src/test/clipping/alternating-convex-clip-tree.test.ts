...
import { AlternatingCCTreeNode } from "../../clipping/alternating-convex-clip-tree";
    ...
    for (const fx of fractions) {
        for (const fy of fractions) {
            ...
            const doOutput =
                outputLevel === 2 ||
                (outputLevel === 1 &&
                    Geometry.isAlmostEqualNumber(Math.abs(fx - 0.5), Math.abs(fy - 0.5))) ||
    ...
    it("Test2", () => {
        ...
        for (const perpendicularFactor of [-1.0, 1.0]) {
            for (const generatorFunction of [
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalDiamondConvexPattern(numRecursion, _perpendicularFactor),
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalSquareReversingPattern(numRecursion, _perpendicularFactor),
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalLReversingPattern(numRecursion, _perpendicularFactor),
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalLMildConcavePatter(numRecursion, _perpendicularFactor)
            ]) {
        ...
        for (const perpendicularFactor of [-1.0, 1.0]) {
            for (const generatorFunction of [
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalSquareReversingPattern(numRecursion, _perpendicularFactor),
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.nonConvexQuadSimpleFractal(numRecursion, _perpendicularFactor),
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalDiamondConvexPattern(numRecursion, _perpendicularFactor),
                (numRecursion: number, _perpendicularFactor: number) =>
                    Sample.createFractalSquareReversingPattern(numRecursion, _perpendicularFactor),
