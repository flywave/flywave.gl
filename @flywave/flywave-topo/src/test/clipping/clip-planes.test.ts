        ...
        for (const transform of transforms) {
            ...
            for (const hide of [-1, 0, 2, 3, 4]) {
                ...
                if (hide < 0) {
                    ...
                    const rangeA = ClipUtilities.rangeOfConvexClipPlaneSetIntersectionWithRange(
                        convexSetB,
                        range
                    );
        ...
        for (const u of fractions) {
            for (const v of fractions) {
                for (const w of fractions) {
                    ...
                    const clip = IndexedXYZCollectionPolygonOps.intersectRangeConvexPolygonInPlace(
                        range,
                        polygon1
                    );
