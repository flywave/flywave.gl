...
import {
    ...
    ClipShape
} from "../../clipping/clip-primitive";
import { ClipUtilities } from "../../clipping/clip-utils";
import { ClipVector, StringifiedClipVector } from "../../clipping/clip-vector";
...
import { clipPrimitivesAreEqual } from "./clip-primitives.test"; // External test functions
...
describe("ClipVector", () => {
    ...
    it("converts to compact string representation", () => {
        ...
        const planes = [plane];
        planes.push(
            ClipPlane.createNormalAndDistance(new Vector3d(0, 0, -1), 0.00000000005, true, true)!
        );
    ...
    it("OuterAndMask", () => {
        ...
        for (const isMask of [true, false]) {
            ...
            const jsonTwoInletsReversed = [
                { shape: { points: outer } },
                { shape: { points: twoInletsWithWhisker.slice().reverse(), mask: isMask } }
            ];
...
export function exerciseClipPrimitive(
    ...
    if (primitive instanceof ClipShape) {
        if (primitive.transformIsValid) {
            const transformedPolygon = primitive.transformFromClip!.multiplyPoint3dArray(
                primitive.polygon
            );
