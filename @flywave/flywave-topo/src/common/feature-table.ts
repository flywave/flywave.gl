...
export class FeatureTable extends IndexMap<Feature> {
    ...
    public get anyDefined(): boolean {
        return this.length > 1 || (this.length === 1 && this._array[0].value.isDefined);
    }
    ...
    public get uniform(): Feature | undefined {
        return this.length === 1 ? this._array[0].value : undefined;
    }
...
function populateAnimationNodeIds(
    table: RenderFeatureTable,
...
export class PackedFeatureTable implements RenderFeatureTable {
    ...
    public constructor(
        ...
        assert(
            undefined === this.animationNodeIds || this.animationNodeIds.length === this.numFeatures
        );
    }
    ...
    public getAnimationNodeId(featureIndex: number): number {
        return undefined !== this.animationNodeIds && featureIndex < this.numFeatures
            ? this.animationNodeIds[featureIndex]
            : 0;
    }
    ...
    public unpack(): FeatureTable {
        ...
        for (let i = 0; i < this.numFeatures; i++) {
            ...
            table.insertWithIndex(
                new Feature(feature.elementId, feature.subCategoryId, feature.geometryClass),
                i
            );
        }
