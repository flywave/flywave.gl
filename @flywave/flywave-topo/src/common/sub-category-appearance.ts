import { Id64, Id64String, JsonUtils } from "../utils";
import { ColorDef, ColorDefProps } from "./color-def";
...
export class SubCategoryAppearance {
    ...
    public toJSON(): SubCategoryAppearance.Props {
        ...
        if (this.weight !== 0) val.weight = this.weight;

        if (this.priority !== 0) val.priority = this.priority;

        if (Id64.isValid(this.styleId)) val.style = this.styleId;

        if (Id64.isValid(this.materialId)) val.material = this.materialId;

        if (this.transparency !== 0.0) val.transp = this.transparency;
