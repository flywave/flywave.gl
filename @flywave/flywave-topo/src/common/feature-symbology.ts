import { assert, Id64, Id64String } from "../utils";
import { ColorDef } from "./color-def";
import { BatchType, Feature } from "./feature-table";
import { GeometryClass } from "./geometry-class";
import { LinePixels } from "./line-pixels";
import { RgbColor, RgbColorProps } from "./rgb-color";
import { SubCategoryOverride } from "./sub-category-override";
...
export class FeatureAppearance {
    ...
    public static fromSubCategoryOverride(ovr: SubCategoryOverride): FeatureAppearance {
        ...
        const ignoresMaterial =
            undefined !== ovr.material && Id64.isValid(ovr.material) ? true : undefined;
        return this.fromJSON({
            rgb,
            transparency,
            weight,
    ...
    public toJSON(): FeatureAppearanceProps {
        ...
        if (this.ignoresMaterial === true) props.ignoresMaterial = true;

        if (this.nonLocatable === true) props.nonLocatable = true;

        if (this.emphasized === true) props.emphasized = true;
...
export class FeatureOverrides implements FeatureAppearanceSource {
    ...
    private getElementAnimationOverrides(
        ...
        if (!app || animationNodeId === 0 || this._ignoreAnimationOverrides.length === 0) {
            return app;
        }
    ...
    public override(args: OverrideFeatureAppearanceArgs): void {
        ...
        const replace = args.onConflict === "replace";
        const existing = replace ? undefined : map.get(idLo, idHi);
        if (existing) {
            assert(args.onConflict !== "replace");
    ...
    public isFeatureVisible(feature: Feature): boolean {
        ...
        const alwaysDrawn =
            undefined !== elemIdParts && this.isAlwaysDrawn(elemIdParts.lower, elemIdParts.upper);
