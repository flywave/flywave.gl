...
import {
    ...
    transform,
    vogelDisk
} from "../three-geospatial/shaders";

import { AtmosphereParameters, type AtmosphereParametersUniform } from "./AtmosphereParameters";
import {
...
import fragmentShader from "./shaders/aerialPerspectiveEffect_frag";
import vertexShader from "./shaders/aerialPerspectiveEffect_vert";
...
import skyShader from "./sky_glsl";
...
export interface AerialPerspectiveEffectUniforms {
    ...
    _atmProjMatrix: Uniform<Matrix4>;
    _atmViewMatrix: Uniform<Matrix4>;
    inverseProjectionMatrix: Uniform<Matrix4>;
    inverseViewMatrix: Uniform<Matrix4>;
    _atmCamPos: Uniform<Vector3>;
...
export class AerialPerspectiveEffect extends Effect {
    ...
    constructor(
        ...
        super(
          ...
          Object.entries({
            ...
            _atmProjMatrix: new Uniform(new Matrix4()),
            _atmViewMatrix: new Uniform(new Matrix4()),
            inverseProjectionMatrix: new Uniform(new Matrix4()),
            inverseViewMatrix: new Uniform(new Matrix4()),
            _atmCamPos: new Uniform(new Vector3()),
    ...
    private copyCameraSettings(camera: Camera): void {
        const { projectionMatrix, matrixWorldInverse, projectionMatrixInverse, matrixWorld } =
            camera;
        const uniforms = this.uniforms;
        uniforms.get("_atmProjMatrix").value.copy(projectionMatrix);
        uniforms.get("_atmViewMatrix").value.copy(matrixWorldInverse);
        ...
        const cameraPosition = camera.getWorldPosition(uniforms.get("_atmCamPos").value as Vector3);
    ...
    private _octEncodedNormal: boolean = false;

    get octEncodedNormal(): boolean {
        return this._octEncodedNormal;
    }

    set octEncodedNormal(value: boolean) {
        if (value !== this._octEncodedNormal) {
            this._octEncodedNormal = value;
            if (value) {
    ...
    private _reconstructNormal: boolean = false;

    get reconstructNormal(): boolean {
        return this._reconstructNormal;
    }

    set reconstructNormal(value: boolean) {
        if (value !== this._reconstructNormal) {
            this._reconstructNormal = value;
            if (value) {
    ...
    set hasNormals(value: boolean) {
        if (value !== this._hasNormals) {
            this._hasNormals = value;
            if (value) {
                this.defines.set("HAS_NORMALS", "1");
            } else {
                this.defines.delete("HAS_NORMALS");
            }
            this.setChanged();
        }
    }
    ...
    /** @private */
    private _combinedScatteringTextures: boolean = false;

    get combinedScatteringTextures(): boolean {
        return this._combinedScatteringTextures;
    }

    set combinedScatteringTextures(value: boolean) {
        if (value !== this._combinedScatteringTextures) {
            this._combinedScatteringTextures = value;
            if (value) {
                this.defines.set("COMBINED_SCATTERING_TEXTURES", "1");
            } else {
                this.defines.delete("COMBINED_SCATTERING_TEXTURES");
            }
            this.setChanged();
        }
    }
    ...
    /** @private */
    private _hasHigherOrderScatteringTexture: boolean = false;

    get hasHigherOrderScatteringTexture(): boolean {
        return this._hasHigherOrderScatteringTexture;
    }

    set hasHigherOrderScatteringTexture(value: boolean) {
        if (value !== this._hasHigherOrderScatteringTexture) {
            this._hasHigherOrderScatteringTexture = value;
            if (value) {
                this.defines.set("HAS_HIGHER_ORDER_SCATTERING_TEXTURE", "1");
            } else {
                this.defines.delete("HAS_HIGHER_ORDER_SCATTERING_TEXTURE");
            }
            this.setChanged();
        }
    }
    ...
    private _correctGeometricError: boolean = false;

    get correctGeometricError(): boolean {
        return this._correctGeometricError;
    }

    set correctGeometricError(value: boolean) {
        if (value !== this._correctGeometricError) {
            this._correctGeometricError = value;
            if (value) {
    ...
    private _sunLight: boolean = false;
    ...
    set sunLight(value: boolean) {
        if (value !== this._sunLight) {
            this._sunLight = value;
            if (value) {
                this.defines.set("SUN_LIGHT", "1");
            } else {
                this.defines.delete("SUN_LIGHT");
            }
            this.setChanged();
        }
    }

    private _skyLight: boolean = false;

    get skyLight(): boolean {
        return this._skyLight;
    }

    set skyLight(value: boolean) {
        if (value !== this._skyLight) {
            this._skyLight = value;
            if (value) {
                this.defines.set("SKY_LIGHT", "1");
            } else {
                this.defines.delete("SKY_LIGHT");
            }
            this.setChanged();
        }
    }

    private _transmittance: boolean = false;
    ...
    private _inscatter: boolean = false;

    get inscatter(): boolean {
        return this._inscatter;
    }

    set inscatter(value: boolean) {
        if (value !== this._inscatter) {
            this._inscatter = value;
            if (value) {
    ...
    set ground(value: boolean) {
        if (value !== this._ground) {
            this._ground = value;
            if (value) {
                this.defines.set("GROUND", "1");
            } else {
                this.defines.delete("GROUND");
            }
            this.setChanged();
        }
    }
    ...
    private _shadowSampleCount: number = 8;

    get shadowSampleCount(): number {
        return this._shadowSampleCount;
    }

    set shadowSampleCount(value: number) {
        if (value !== this._shadowSampleCount) {
            this._shadowSampleCount = value;
            this.defines.set("SHADOW_SAMPLE_COUNT", String(value));
            this.setChanged();
        }
    }
}
