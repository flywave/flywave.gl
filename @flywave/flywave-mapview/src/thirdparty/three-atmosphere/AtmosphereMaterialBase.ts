...
import { Ellipsoid } from "../three-geospatial";
...
export abstract class AtmosphereMaterialBase extends RawShaderMaterial {
    ...
    copyCameraSettings(camera: Camera): void {
        ...
        const cameraPosition = camera.getWorldPosition(uniforms.cameraPosition.value);
    ...
    private _combinedScatteringTextures = false;
    ...
    set combinedScatteringTextures(value: boolean) {
        if (value !== this._combinedScatteringTextures) {
            ...
            if (value) {
                this.defines ??= {};
                this.defines.COMBINED_SCATTERING_TEXTURES = "1";
            } else {
                delete this.defines?.COMBINED_SCATTERING_TEXTURES;
            }
    ...
    private _hasHigherOrderScatteringTexture = false;
    ...
    set hasHigherOrderScatteringTexture(value: boolean) {
        if (value !== this._hasHigherOrderScatteringTexture) {
            ...
            if (value) {
                this.defines ??= {};
                this.defines.HAS_HIGHER_ORDER_SCATTERING_TEXTURE = "1";
            } else {
                delete this.defines?.HAS_HIGHER_ORDER_SCATTERING_TEXTURE;
            }
            this.needsUpdate = true;
        }
    }
