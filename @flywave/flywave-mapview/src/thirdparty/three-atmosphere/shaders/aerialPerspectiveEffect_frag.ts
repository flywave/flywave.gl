const shader: string = `precision highp sampler2DArray;
...
uniform mat4 _atmProjMatrix;
uniform mat4 _atmViewMatrix;
...
float getShadowRadius(const vec3 worldPosition) {
  ...
  mat4 viewProjectionMatrix = _atmProjMatrix * _atmViewMatrix;
...
void mainImage(const vec4 inputColor, const vec2 uv, out vec4 outputColor) {
  ...
  if (depth >= 1.0 - 1e-8) {
    ...
    // DEBUG: magnify sky radiance so we can see it
    outputColor.rgb *= 100.0;
    outputColor.a = 1.0;
    return;
    #else // SKY
