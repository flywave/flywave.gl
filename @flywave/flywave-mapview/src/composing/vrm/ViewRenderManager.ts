...
            // Throttle the center-depth readback to ~10Hz while flying: each
            // 1x1 readback is a separate GPU submit+map, which is a hard cost
            // in a CPU-submission-bound frame. Consumers (camera collision)
            // tolerate a 100ms-stale depth.
            const now = performance.now();
            if (moved && now - this._lastCenterReadMs > 100) {
                this._lastCenterReadMs = now;
...
        this.taaNode = undefined;
        ...
        try {
            this.m_pickReadBuffer?.destroy();
        } catch {
            // ignore — buffer may already be gone
        }
        this.m_pickReadBuffer = null;
...
        if (x < 0 || x >= rt.width || y < 0 || y >= rt.height) return null;
        ...
        try {
            const data = await this.readPickDepthPixel(x, y);
...
            return ndcZ * 0.5 + 0.5;
    ...
    /**
     * Persistent-staging readback of one pickDepth texel (1x1 rgba16float).
     *
     * Replaces renderer.readRenderTargetPixelsAsync for the per-frame depth
     * slots: three r185 allocates a fresh MAP_READ|COPY_DST GPUBuffer on every
     * call (copyTextureToBuffer → device.createBuffer → mapAsync → destroy),
     * which showed up as a per-frame GPU allocation under the alloc probe.
     * A persistent buffer can be mapped/unmapped repeatedly, so we own one
     * 8-byte buffer for the whole session and just copy → map → read → unmap.
     * All callers are funneled through one serial queue because a GPUBuffer
     * cannot be mapped twice concurrently.
     * Falls back to readRenderTargetPixelsAsync on non-WebGPU backends
     * (forceWebGL) or when the texture has no GPU counterpart yet.
     */
    private readPickDepthPixel(x: number, y: number): Promise<Uint16Array | null> {
        const run = async (): Promise<Uint16Array | null> => {
            const rt = this.passNode?.renderTarget;
            if (rt == null || this.pickDepthTexIndex < 0) return null;

            const backend = (this.renderer as any)?.backend;
            const device = backend?.device;
            const textureData = device ? backend.get(rt.textures[this.pickDepthTexIndex]) : null;
            if (device == null || textureData?.texture == null) {
                // WebGL fallback / texture not yet uploaded — three's generic path
                return (await this.renderer.readRenderTargetPixelsAsync(
                    rt,
                    x,
                    y,
                    1,
            ...
            try {
                if (this.m_pickReadBuffer == null) {
                    // 1 texel rgba16float = 8 bytes. WebGPU requires bytesPerRow to
                    // be 256-aligned, but for height=1 the buffer only needs to
                    // hold the single row payload (same sizing as three itself).
                    this.m_pickReadBuffer = device.createBuffer({
                        size: 8,
                        usage: 0x1 | 0x8 // GPUBufferUsage.MAP_READ | COPY_DST
                    });
                }
                const encoder = device.createCommandEncoder();
                encoder.copyTextureToBuffer(
                    { texture: textureData.texture, origin: { x, y, z: 0 } },
                    { buffer: this.m_pickReadBuffer, bytesPerRow: 256, rowsPerImage: 1 },
                    { width: 1, height: 1, depthOrArrayLayers: 1 }
                );
                device.queue.submit([encoder.finish()]);

                await this.m_pickReadBuffer.mapAsync(0x1); // GPUMapMode.READ
                const out = new Uint16Array(4);
                out.set(new Uint16Array(this.m_pickReadBuffer.getMappedRange(0, 8)));
                this.m_pickReadBuffer.unmap();
                return out;
            } catch {
                // Buffer may be dead (device lost / destroyed): drop it so the
                // next call recreates instead of failing forever.
                try {
                    this.m_pickReadBuffer?.destroy();
                } catch {
                    // ignore
        ...
        const result = this.m_pickReadQueue.then(run, run);
        this.m_pickReadQueue = result.then(
            () => undefined,
            () => undefined
        );
        return result;
    ...
    /** Persistent 8-byte staging buffer for pickDepth readbacks (WebGPU only). */
    private m_pickReadBuffer: any = null;

    /** Serializes readbacks — one persistent buffer, one map at a time. */
    private m_pickReadQueue: Promise<void> = Promise.resolve();
