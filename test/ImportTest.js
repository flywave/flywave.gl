...
function checkImports() {
    ...
    const packageJsonFiles = [...glob.sync(__dirname + "/../@flywave/*/package.json")];
    for (const packageJsonFile of packageJsonFiles) {
        const moduleName = "@flywave/" + packageJsonFile.split(path.sep).slice(-2, -1)[0];
    ...
    function getDependencies(moduleName, deps) {
        if (deps[moduleName]) {
            return;
        }
    ...
    const sourceFiles = glob
        .sync(__dirname + "/../@flywave/**/*.ts")
        .filter(sourcePath => !sourcePath.includes("node_modules") && !sourcePath.includes("generator-flywave.gl"));
    // regular expression catching all imported modules
    const importRE = /(.*)?import\s*{[\s\S]*?}\s*from\s+["'](.*)["']/gi;
    const environmentRE = /@flywave:check-imports:environment:(.*)/gi;
    const errors = new Array();
    for (const sourceFile of sourceFiles) {
        const contents = fs.readFileSync(sourceFile, "utf-8");
        let env = "browser";
        const relativePath = path.relative(__dirname + "/../@flywave", sourceFile);
        const moduleName = "@flywave/" + relativePath.split(path.sep)[0];
        ...
        if (environmentMatch) {
            ...
            if (env !== "browser" && env !== "node") {
                errors.push(`Error: ${relativePath} unknown '@flywave:environment:' type: ${env}. Supported types: 'node', 'browser'`);
                env = "browser";
            }
        }
        ...
        while ((matches = importRE.exec(contents)) != null) {
            ...
            if (importedModule === moduleName) {
                errors.push(`Error: ${relativePath} contains wrong module import "${importedModule}", change it to a relative import`);
            }
            ...
            if (!localModule && !allowedNodeModule) {
                ...
                if (packageDependencies[moduleName] !== undefined &&
                    packageDependencies[moduleName][importedModuleName] === undefined) {
                    errors.push(`Error: unknown module ${importedModuleName} used in ${moduleName}: ` +
                        `${relativePath}`);
                }
            }
            // 3) Make sure we don't include other @flywave modules via relative paths
            if (importedModule.startsWith("..")) {
                const resolvedImportedModule = path.resolve(path.dirname(sourceFile), importedModule);
                if (!path.dirname(resolvedImportedModule).startsWith(modulePath)) {
                    errors.push(`Error: the module ${importedModule} is imported via a relative path in ${sourceFile}`);
                }
            }
        }
    }
    ...
    function detectCircularDependencies() {
        ...
        for (const moduleName in packageConfigs) {
            const dependencies = Object.keys(packageDependencies[moduleName]).filter(dependencyName => dependencyName.startsWith("@flywave/") && dependencyName !== moduleName);
            const module = {
                name: moduleName,
                index: undefined,
                lowLink: undefined,
