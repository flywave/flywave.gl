...
function checkImports() {
    ...
    const sourceFiles = glob
        ...
        .filter(
            sourcePath =>
                !sourcePath.includes("node_modules") && !sourcePath.includes("generator-flywave.gl")
        );
    ...
    for (const sourceFile of sourceFiles) {
        ...
        if (environmentMatch) {
            ...
            if (env !== "browser" && env !== "node") {
                errors.push(
                    `Error: ${relativePath} unknown '@flywave:environment:' type: ${env}. Supported types: 'node', 'browser'`
                );
                env = "browser";
            }
        }
    ...
    function detectCircularDependencies() {
        ...
        for (const moduleName in packageConfigs) {
            const dependencies = Object.keys(packageDependencies[moduleName]).filter(
                dependencyName =>
                    dependencyName.startsWith("@flywave/") && dependencyName !== moduleName
            );
