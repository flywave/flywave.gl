...
import { EarthConstants, GeoBox, GeoCoordinates, webMercatorProjection, webMercatorTilingScheme } from "@flywave/flywave-geoutils";
...
describe("MapView + OmvDataSource + GeoJsonDataProvider rendering test", function () {
    ...
    it("renders extruded polygons with height, with lineWidth Expression," + " resulting in 0", async function () {
        this.timeout(5000);
        const ourStyle = [
            {
                when: ["==", ["geometry-type"], "Polygon"],
    ...
    it("renders extruded polygons with height, with lineWidth Expression," + " resulting in 1", async function () {
        this.timeout(5000);
        const ourStyle = [
            {
                when: ["==", ["geometry-type"], "Polygon"],
    ...
    describe("wrap polygon crossing antimeridian", async function () {
        ...
        const coordinates = polygon_crossing_antimeridian.features[0].geometry.coordinates[0].map(([lng, lat]) => GeoCoordinates.fromGeoPoint([lng, lat]));
        ...
        const toGeoPolygonFeature = (part) => {
            const coordinates = wrappedPolygon[part];
        ...
        it(`polygon to wrap`, async function () {
            ...
            await geoJsonTest.run({
                ...
                theme: { lights, styles: { geojson: ourStyle } },
                geoJson: polygon_crossing_antimeridian
            });
        });
    ...
    describe("wrap linestring crossing antimeridian", async function () {
        ...
        // Extracts a part (left, middle or right) from the wrapped polygon
        // and convert it to a GeoJson feature.
        const toGeoJsonMultiLineString = (part) => {
            const coordinates = wrappedLineString[part];
            // Converts an Array of GeoCoordinates to GeoJSON coordinates.
            const toGeoJsonCoordinates = (coordinates) => {
    ...
    describe("clip lines against bounds", async function () {
        it("clip long line string", async function () {
            ...
            const geoBox = new GeoBox(GeoCoordinates.fromGeoPoint(bounds[0]), GeoCoordinates.fromGeoPoint(bounds[0]));
            ...
            const points = inputLineString.map(geoPos => {
                const { x, y } = webMercatorProjection.projectPoint(GeoCoordinates.fromGeoPoint(geoPos));
                return new Vector3(x, y, 0);
            });
            const lineStrings = clipLineString(points, min.x, min.y, max.x, max.y);
            const coordinates = lineStrings.map(lineString => lineString.map(({ x, y }) => webMercatorProjection.unprojectPoint(new Vector3(x, y, 0)).toGeoPoint()));
            const geoJson = {
                type: "FeatureCollection",
                features: [
                    {
    ...
    describe("Tiled GeoJson", async function () {
        it("Circle with an hole with same winding as the outer ring", async function () {
            ...
            await geoJsonTest.run({
                ...
                lookAt: {
                    zoomLevel: 14.0,
                    target: webMercatorTilingScheme.getGeoBox(webMercatorTilingScheme.getTileKey(GeoCoordinates.fromGeoPoint([-90, -42.52556]), 13)).center
                }
            });
        });
    });
...
//# sourceMappingURL=GeoJsonDataRendering.js.map