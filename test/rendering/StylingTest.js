...
import { TextureCoordinateType } from "@flywave/flywave-datasource-protocol";
import { FeaturesDataSource } from "@flywave/flywave-features-datasource";
import { OrientedBox3, ProjectionType } from "@flywave/flywave-geoutils";
...
import { getPlatform, RenderingTestHelper, waitForEvent } from "@flywave/flywave-test-utils";
import { getReferenceImageUrl } from "@flywave/flywave-test-utils/lib/rendering/ReferenceImageLocator";
...
function baseRenderingTest(name, options, testFun, cleanupFun) {
    const commonTestOptions = { module: "flywave.gl" };
...
function mapViewFitGeoBox(mapView, geoBox, margin = 0.1) {
...
function mapViewFeaturesRenderingTest(name, options, testFun) {
    let mapView;
    baseRenderingTest(name, options, async function (canvas) {
        ...
        const defaultLookAt = mapViewFitGeoBox(mapView, geoBox, getOptionValue(options.margin, 0.15));
...
describe("MapView Styling Test", function () {
    ...
    describe("point features", function () {
        ...
        function makePointTextTestCases(testCases, options) {
        ...
        function makePointPoiTestCases(testCases, options) {
            for (const testCase in testCases) {
                const attr = testCases[testCase];
        ...
        function makePointTestCases(testCases, options) {
            for (const testCase in testCases) {
                const attr = testCases[testCase];
                mapViewFeaturesRenderingTest(`solid-line-styling-${testCase}`, {
                    geoJson: {
    ...
    describe("line features", function () {
        ...
        function makeLineTestCase(testCases, lineGeometry = straightLine) {
            for (const testCase in testCases) {
                for (const clipping of [true, false]) {
                    const postfix = clipping === true ? "" : "-no-clipping";
                    const attr = JSON.parse(JSON.stringify(testCases[testCase]));
    ...
    describe("polygon features", function () {
        ...
        function makePolygonTestCases(technique, testCases, options) {
            let extraFeatures = [];
        ...
        describe("standard technique", function () {
            ...
            mapViewFeaturesRenderingTest(`polygon-standard-texture`, {
                geoJson: {
                    type: "FeatureCollection",
                    features: [rectangle1, rectangle2, referenceBackground]
                },
            ...
            mapViewFeaturesRenderingTest(`polygon-standard-texture-transparent`, {
                geoJson: {
                    type: "FeatureCollection",
        ...
        describe("extruded-polygon technique", function () {
            ...
            describe("flat", function () {
                makePolygonTestCases("extruded-polygon", {
