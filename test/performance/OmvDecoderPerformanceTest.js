...
import { mercatorProjection, sphereProjection, TileKey, webMercatorProjection } from "@flywave/flywave-geoutils";
...
import { APIFormat, AuthenticationMethod, OmvRestClient } from "@flywave/flywave-vectortile-datasource";
...
if (typeof window === "undefined") {
    ...
    global.PerformanceObserver = perfHooks.PerformanceObserver;
    global.PerformanceEntry = perfHooks.PerformanceEntry;
}
/**
 ...
 */
export function createOMVDecoderPerformanceTest(name, options) {
    ...
    describe(`OMVDecoderPerformanceTest - ${name}`, function () {
        ...
        before(async function () {
            ...
            omvTiles = await Promise.all(options.tiles.map(async (mortonCode) => {
        ...
        it(`measure feature matching time`, async () => {
            ...
            const geometryProcessor = {
                processPointFeature(layerName, tileExtents, geometry, properties) {
                    const env = new MapEnv(properties);
                    styleSetEvaluator.getMatchingTechniques(env, layerName, "point");
                },
                processLineFeature(layerName, tileExtents, geometry, properties) {
                    const env = new MapEnv(properties);
                    styleSetEvaluator.getMatchingTechniques(env, layerName, "line");
                },
                processPolygonFeature(layerName, tileExtents, geometry, properties) {
                    const env = new MapEnv(properties);
        ...
        it(`measure decode time - webMercator`, async () => {
            ...
            await measurePerformanceSync(counterName, repeats, function () {
                for (const [tileKey, tileData] of omvTiles) {
                    const decoder = new VectorTileDataProcessor(tileKey, projection, styleSetEvaluator, new OmvDataAdapter());
...
//# sourceMappingURL=OmvDecoderPerformanceTest.js.map