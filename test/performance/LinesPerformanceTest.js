    ...
    before(function () {
        ...
        tests.forEach(test => {
            ...
            for (let i = 0; i < segments; i++) {
                ...
                test.points.push(Math.cos(THREE.MathUtils.degToRad(angle) * radius), Math.cos(THREE.MathUtils.degToRad(angle) * radius), 0);
            }
        });
    });
    tests.forEach(test => {
        it(`createLineGeometry segments=${test.segments}`, async function () {
            this.timeout(0);
            await measureThroughputSync(`createLineGeometry segments=${test.segments}`, 1000, function () {
                createLineGeometry(center, test.points, mercatorProjection);
            });
        });
    });
});
//# sourceMappingURL=LinesPerformanceTest.js.map