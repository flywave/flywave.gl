...
### §885 g77: 跨 cell 片段合并帧差假设证伪——建筑几何逐片段精确正确（2026-09-22）

**① 逐片段对拍（g76③ 计划执行）**：[MBBldgRing] 扩展 cell key + center + 全角点，5 片段（z19/465685-87 × 206485-86，src=primary——extras 合并后 _sourceId 丢失，仅标记问题）逐片段与 geojson 裁剪期望（标准 mercator 离线计算）对拍：**全部非钉扎角点精确命中**（B=(−33.0,−4.4)↔(−33.1,−4.4)；C=(−18.7,13.6)↔(−18.8,13.6)；A=(27.0,19.9)↔(27.0,19.9)；D=(−37.3,−37.3)↔(−37.3,−37.2)），钉扎点均为 cell 边界 ±38.2 裁剪伪影。**g74"建筑旋转14°"与 g76"合并帧差"两条假设链全部证伪——建筑世界几何（底环）完全正确。**

**② 差带残余的重新解释（开放）**：几何正确而影子掩膜左边界斜率差 14° 仍待解释。候选重排：(a) 掩膜分析混入了**建筑本体剪影**（暗色立面 <140 计入"影子"）——若两引擎建筑可见高度/剪影不同（§294 高度 sec(lat) 双乘补偿嫌疑），左边界差即剪影差而非影子差；(b) 顶点转储走 project() 逐点，绕过了对象级 matrix——渲染期对象变换仍可能与转储不一致（同管线道路像素级吻合，此可能性低但未封死）。

**③ 战略建议**：该残余（esl ~14k px 中的主导结构）已四轮取证仍未闭合，建议下轮先做 5 分钟廉价验证——no-shadow 态直接测量两引擎建筑剪影边界（y=90 行采样立面色 vs 背景色分界），若剪影一致则残余归影子链 caster 高度（§294 sec² 补偿），若不一致则建筑渲染高度差定案；之后再决定继续深挖或转战 S8/underpass 等更大开放项。

**④ 状态**：esl 22,372 持平；tsc 26；单测 310；[MBBldgRing] 转储定型入库。

### §885 g78: 剪影验证 + 高度补偿假设上位（2026-09-22）

**① 剪影边界测量（g77③ 计划执行）**：no-shadow 态建筑剪影左缘 ours(366/300/234/169/101/39) vs exp(324/287/222/165/100/36)（y=70..170）——**基本吻合**（差 3-43px 向下收敛，expected 含影边界有污染），g77②(a) 剪影混合假设弱化。
...
### §885 g82: 大夹具 shadowdisable 判别筛查——影子相关/无关定量分离，优先级重排（2026-09-22）

**① 判别矩阵（无影 vs 有影，全 mtime 新鲜）**：
| 夹具 | 有影 | 无影 | 结论 |
|---|---|---|---|
| ortho-camera | 80,516 | 58,127 | 影子链 22k + 正交固有 58k |
| elevated-wireframe | 77,467 | 77,467 | **影子零相关** |
| elevated-symbols-viewport-aligned(+text/+terrain) | 62,762/63,287/50,084 | **逐位同值** | **影子零相关** |
| guard-rail-color(+fd) | 62,159/59,379 | **逐位同值** | **影子零相关** |

**② 结构判定（viewport-aligned 无影差分析）**：89k 差异全域均匀散布、双向（暗/亮各半），采样见"路↔背景互换"型（(100,100) ours 路/exp 背景；(400,400) 反向）——**局部路段几何差异**（互相关证伪整体位移：最佳平移(11,8)仅 27.6%→26.3%）——属遗留道路渲染域（路面段位置/宽度/覆盖的长期家族差），非 3d-intersections 阴影专项。

**③ 优先级重排结论**：
- **影子链可修总量其实很小**：ortho 22k + underpass 局部 ~28k + esl 差带 ~14k（挂 S10）≈ 64k/188 万（3.4%）——S10/S8/S12 的天花板有限。
- **真正大头=非影域**：elevated-symbols 家族（~20 夹具 × 24-63k，viewport-aligned 族已证零影相关）+ guard-rail 62k + wireframe 77k——公共根因是道路/护栏/符号渲染的局部几何差（legacy 域，历轮已多次推进）。
...
### §885 g96: uMBPatternScale mgl 字面化落地（patMul 2→1）——no-cross-beams −4.2k、其余噪声级（2026-09-22 第八轮）

**① 公式实现（fill_pattern.vertex + pattern.ts 字面）**：mgl period = display_size = sprite cell px/pixelRatio（**屏幕常量**——u_tile_units_to_pixels = tileSize×2^(tileZoom−overscaledZ)/EXTENT 的 overscale 因子与瓦片放大相互抵消，pattern.ts:35-52 + pixels_to_tile_units.ts）。我方现行 period_px = texWidth/(patMul×spritePr)——hatch sprite 40×40@2x 下 patMul=2 → 10px vs mgl 20px（细一倍）。落地：patMul 默认 2→1（公式结构不变，`patternmul=<f>` 旋钮保留）。
...
### §885 g102: g82③ 非高度域首层归因——白标线网络拓扑同构、无全局平移、逐段 1-2px 局部位移定量（2026-09-22 第十四轮）

**① 结构分类（mgl oracle vs ours-legacy，va，阈 12）**：66,870 diff px 中 mglOnlyWhite 22,693 + oursOnlyWhite 17,856（**白标线差 ≈60% 主源**）+ oursOnlyColor 13,202（背景/色互换）+ bothDiff 10,966；径向集中中环（64-256px 半径带）。

**② 白掩膜量化**：mgl 白 104,429 / ours 100,888 / 交集 81,406 → **IoU=0.657**；XOR 42,505 px 成 1,608 个沿路网走廊蔓延的蛇形 blob（非局部孤岛）。

**③ 平移假设否定**：白掩膜 (dx,dy)∈±6 扫描——最优 (−3,−2)=0.7956 vs (0,0)=0.7938（**平坦响应，无全局平移**）；线宽 run-length 分布双侧同形（1-12px 计数同衰减曲线，我方约 −30% 计数=覆盖略少）。

**④ 桥面带结构对拍（y96..335，512 列全量）**：每列白 run 数 7.38(mgl) vs 7.51(ours)、每列白像素 68.4 vs 67.7、mglOnly/oursOnly 列数均为 **0**——**线网拓扑完全同构，XOR 全部来自同一线段的局部位移（~1-2px per-segment offset）**，非缺失/复制/宽度差（g83 箭头复制、g84 宽度假说在 oracle 对拍下双双排除）。

**⑤ 定性**：残差主体 = **逐段投影几何差**（候选：线锚点/offset 方向基准、逐顶点投影取整、dash 相位锚、曲线细分采样点选择）。与 g72/g73 的互相关结论（最佳平移不收敛）一致——需 MVT 直读逐段对拍（同段两端点在两引擎的屏幕坐标差向量统计）定位到管线级。

**⑥ 状态**：零代码改动；oracle 基建（mgl-shot + 白掩膜/XOR/平移扫描脚本模式）复用成立。**下轮首案：选 3-5 条 dash 走廊，MVT 直读线几何 → 双引擎各自投影 → 逐端点屏幕坐标差向量统计（dx,dy 分布若呈系统方向=锚点/帧差；随机=细分/相位差），据此定管线修复点。**

### §885 g103: g102⑥ 执行——埋没机制实锤 + deckstrip 旋钮 + 桥面结构密度/屏位双差发现（2026-09-22 第十五轮）

**① 逐段位移测量（g102⑥ 计划执行）**：行/列 run 配对（489/1500 对）——**中心偏移 med≈0（0.00/−0.50）**=线位正确；宽度差 med +1/+3px（run 合并伪影混入）。linewscale 0.7 扫描 **vs expected 与 vs oracle 双平坦**（42,414/42,835 vs 42,390/42,792）——g87 后宽度旋钮惰性，宽度非杠杆。

**② 走廊直读（dash run 并排）**：y=101-107 处 mgl 在 x≈64-235 有完整 dash 链（4-6px 短 run），**我方整走廊缺失**；我方 x≈9-47 有 mgl 没有的多余白——**整走廊级可见性差**（非相位/宽度）。

**③ 埋没机制闭环**：markupbias=0.5 → 缺失走廊白 0→622（mgl 373，**过冲**）+ 全局变差（45.5k）；deckstrip=1（新旋钮：仅 deck 发射项剥 zoff，markup fills/rails 保持 legacy）→ 走廊 759（同样过冲）+ 全局 65,837≈全量 strip。**机制=deck fills 的 curve+level 超过 markup 线自身堆叠曲线（13.00 vs 11.05）压埋线标线**；但单纯解埋即过冲——mgl 走廊只有 373 白，我方解埋后 622-759，说明我方线标线在解埋后仍比 mgl 密/宽。
...
### §885 g132: g131③ 执行——第五路尝试(结构链 uniform 逐帧刷新)仍 inert; deck 漏照亮项正式降级为"渲染级 WebGL 拦截"专案（2026-09-23 第四十四轮）

**① 尝试与结果**：g131 假设"首编译时 lighting3DState 未就绪→uMB3DAmb 烘焙 [1,1,1] 永不刷新"——在 shadow 刷新环内补 uMB3DAmb/Dir/DirColor 逐帧刷新——**ortho 逐数 71,858 不变**（刷新环的 uniform 发现集不含结构链 uniforms 或 deck 材质不在 refreshTargets）。
...
### §885 g149: g148④ 执行——**转换式测定（u_color 覆写四点标定）**: **out = 0.9098·v 线性（v=0.25/0.5/0.9176 三点逐数精确）, v=1.0 白点例外（215, 再除 1.0794=radiance, 疑饱和/白裁剪条件路径）**（2026-09-24 第六十一轮）

**① 覆写实验落地**：mirror background_program u_color 钩（bgcol=r,g,b,a 查询参, Color-like 对象——首版数组致 uniform 拒收=背景不画, 修正后生效）+ mgl-shot 透传。

**② 四点标定**：1.0→**215**（ratio 0.8431）/0.9176(style bg)→213 **逐数=exp**/0.5→116 **精确=0.9098·v**/0.25→58 **精确**。最小二乘幂拟合 a=0.9102, b=1.0004 ⇒ **线性暗化 k=0.9098=1/1.0992**（物理含义未定: 非 radiance(1/1.0794)/非 opacity）; v=1.0 白点独外（215=232/1.0794, 疑 fog_dither 或白饱和条件路径, 挂号）。
...
### §885 g151: g150③ 执行——背景列扫描第三层结构: **振荡型（121-138 摆动, 均值≈未照亮 0.5）非平滑梯度**; mgl 顶带均值 (212,221,230)=exp 逐数同/我方 (217,225,234) 偏亮 +5——四层结构定性收敛为"投影 moiré/tile-quad 深度交织", 挂号防止继续反复（2026-09-24 第六十三轮）

**① 列 run-length 扫描**：bgcol=0.5 注入下 x=71 列 512 行 = 133 段振荡（136,138,135,132,130,128,126,125,130,134…）——**非单调梯度**（g150"垂直梯度"再修正）, 摆动域 121–138, 均值≈128=未照亮 0.5·255; 底部区稳 138(=0.541 照亮)。mgl rmstyle background → 全白（确认背景在画）。结构=**投影 moiré/逐 tile-quad 深度交织**（pitch0 ortho 下的亚像素 quad 边界采样振荡——SwiftShader 光栅化特征疑）。

**② 三方顶带均值（收敛对账）**：mgl (212,221,230) = exp **逐数同**（oracle 再证）/ 我方 (217,225,234) **均匀偏亮 +5/+4/+4** —— 残差的宏观可落地部分=**整带 +5 偏亮**（恒定照亮 vs 振荡结构的均值差）, 微观振荡部分为 SwiftShader 光栅化域（不可复刻/不应追逐）。
...
### §885 g159: g158④ 绘制裁决——**BLUE DRAWN**（隐藏 3 蓝墙 mesh → draw calls 365→362 / tris −160 精确差分）⇒ 在画但零像素 ⇒ **全片元离屏或深度全拒**; 结合 g158 投影恒离屏 ⇒ 强指向 **实例世界矩阵恒等（绘于世界原点=墨卡托百万坐标外）**（2026-09-24 第七十一轮）

**① 差分裁决**：帧40 标记 3 蓝墙 → 帧45 隐藏 → 帧47 对比: calls −3 / triangles −160（与 idx=246+186+? ≈160 tri 全量提交一致）⇒ **mesh 每帧在画、几何全量提交**; 零蓝色像素 ⇒ 片元层全灭（离屏或深度全拒）。

**② 三线合流**：g157/g158 投影恒 (-209,-187,z=1) 离屏（曾判伪影）+ 本轮"在画零像素" ⇒ **伪影定性反转: 投影读数可能是真的**（蓝墙绘于世界原点）; junction 同读数但其像素来自其他路径（其可见墙疑非这批实例——待验）。**根因候选=实例未挂 tile 组变换（恒等 matrixWorld）→ 绘于原点被裁**。

**③ 下轮首案（修复向）**: ①dump 蓝墙 mesh 的 matrixWorld 元素（恒等与否一锤）+ parent 链; ②若恒等→为何（TileObjectsRenderer 挂载路径 vs tile 组）→ 修挂载或烘变换; 修后 tunnel-fd 111k 应大幅收敛。

**④ 状态**：探针弃置树净; 单测 310; 零代码改动。**会话终局**: 55 轮台账 g106-g159。

### §885 g160: g159⑤ matrixWorld 一锤——**非恒等但 z=−29**: 结构 mesh 挂 Scene 直下 position=(81,−10,−29)/(-72,143,-29), 几何局部 z=+0..17 ⇒ **世界 z≈−12..−29 深埋地下** ⇒ "在画零像素"= 全深度拒于路面/地面之下（elevation 语义正确形态=需 mask 孔透出）（2026-09-24 第七十二轮）

**① [MBMw] 一锤**：蓝墙实例 mW 平移 (81,−10,−29)/(−72,143,−29)（非恒等, g159 恒等假设证伪修正）; parent=Scene 直下（**无 tile 组**）; 局部几何 z=+0..17 ⇒ 世界 z≈−12..−29 **地下**——与相机(+28)间被 z=0 地面/路面全遮 ⇒ 深度全拒=零像素机制闭环（g159"离屏或深度拒"二择裁决=**深度拒**）。

**② 根因候选（双重偏移）**：我方 emitter 对结构几何已发 **世界系坐标**（project() 直出, g52 系）, 而引擎挂载又加了 tile 中心平移（−29 即该瓦 z 偏移）⇒ **z 双重变换**; junction 的可见墙疑走 tile.objects 路径（无此双移）故像素正常——两夹具差异的统一解释。

**③ 修复规格（下轮首案, 预期 111k 大幅收敛）**: 结构几何改 **tile 局部系**（emit 时减 tile 中心——与 fill 一致的约定）或挂载时对 __mb-elevated 组清零平移; 二选一需对照 fill 的实际约定（fill v0 亦局部长度级 −967=junction fill 同域 ⇒ fill=局部系+组平移 ✓, 结构=世界系+组平移 ✗ = 不一致即根因）。

**④ 状态**：探针弃置树净; 单测 310; 零代码改动。**会话终局: 56 轮台账 g106-g160, tunnel-fd 根因闭环至"结构世界系 vs 挂载局部系双重变换"。**
