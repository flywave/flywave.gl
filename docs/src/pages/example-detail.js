...
const updateReducer = (state) => state + 1;
export default function ExampleDetail() {
    const location = useLocation();
    const { siteConfig } = useDocusaurusContext();
    const previewRef = useRef(null);
    const [exampleId, setExampleId] = useState("hello-world");
    ...
    useEffect(() => {
        ...
        if (config?.customFields?.copyright) {
            ...
            setCopyrightText(`Copyright © ${new Date().getFullYear()} flywave.gl. Built with Docusaurus.`);
        }
    }, [siteConfig]);
    // 初始化和处理URL参数
    useEffect(() => {
    ...
    const handleExampleSelect = (selectedExampleId) => {
        // 确保示例存在
    ...
    const runCodeWithContent = (codeContent) => {
        if (!previewRef.current || !codeContent)
            return;
        setIsRunning(true);
        // 清除之前的预览内容
    ...
    const categoriesWithExamples = sortedCategories.filter(category => examplesByCategory[category.code] && examplesByCategory[category.code].length > 0);
    const definedCategoryCodes = new Set(EXAMPLE_CATEGORIES.map(cat => cat.code));
    const undefinedCategories = Object.keys(examplesByCategory).filter(categoryCode => !definedCategoryCodes.has(categoryCode));
    ...
    <Layout wrapperClassName={styles.layoutWrapper} noFooter={true}>
                    ...
                    <div className={styles.sidebar}>
                        <div className={styles.sidebarHeader}>
                            ...
                            <div className={styles.sidebarActions}>
                                <button className={styles.sidebarBackButton} onClick={handleBackToHome}>
            ...
            return (<div key={category.code} className={styles.exampleCategory}>
                                        <div className={styles.categoryTitle}>
                                            {currentLocale === 'zh' && category.nameZh ? category.nameZh : category.name}
                                        </div>
                                        {examples.map(ex => (<div key={ex.id} className={`${styles.exampleItem} ${ex.id === exampleId ? styles.active : ""}`} onClick={() => handleExampleSelect(ex.id)}>
                                                <div className={styles.exampleImage}>
                                                    {ex.image ? (<img src={ex.image} alt={ex.title} className={styles.exampleThumbnail}/>) : (<div className={styles.placeholderImage}>
                                                            {ex.title}
                                                        </div>)}
                                                </div>
            ...
            return (<div key={categoryCode} className={styles.exampleCategory}>
                        ...
                        <div className={styles.header}>
                            <div className={styles.headerContent}>
                                ...
                                <div className={styles.headerActions}>
                                    <button className={`${styles.actionButton} ${styles.primary}`} onClick={handleRunCode} disabled={isRunning}>
                                        {isRunning ?
            (currentLocale === 'zh' ? "运行中..." : "Running...") :
            (currentLocale === 'zh' ? "运行代码" : "Run Code")}
                                    </button>
                                    <button className={`${styles.actionButton} ${styles.secondary}`} onClick={handleResetCode} title={currentLocale === 'zh' ? "重置代码到初始状态" : "Reset code to initial state"}>
                                        {currentLocale === 'zh' ? "重置代码" : "Reset Code"}
                                    </button>
                                    <button className={`${styles.actionButton} ${styles.secondary}`} onClick={handleFullscreen}>
                        ...
                        <div className={styles.panelsContainer}>
                            <div className={styles.editorPanel} style={{ width: `${editorWidth}%` }}>
                                ...
                                <div className={styles.editorWrapper}>
                                    <Editor language={currentExample.language} value={code} onChange={value => setCode(value || "")} theme={isDarkTheme ? "vs-dark" : "light"} options={{
            ...
            scrollbar: {
                ...
                horizontal: "auto"
                                ...
                                <div className={styles.resizer} onMouseDown={() => setIsResizing(true)}/>
                            </div>
                            <div className={styles.previewPanel} style={{ width: `${100 - editorWidth}%` }}>
                                <div className={styles.panelHeader}>
