...
harp.gl supports labeling as part of the [[flywave-mapview]] package, including features such as:

* Signed distance field text rendering provided by [[flywave-text-canvas]].
* Text only, icon & text and icon only labels.
* Placement on anchor point or along a path.
* Rendering and layout styling using techniques provided by [[flywave-datasource-protocol]].
...
* [[flywave-datasource-protocol]]: Provides the [[ITileDecoder]] interface that has to be implemented
...
* [[flywave-mapview]]: The main classes involved in labeling are:
...
* [[flywave-text-canvas]]: Text rendering package that generates, places and renders glyphs:
