...
**§224. 深度考古终局——z 三点探测（0.5 过/0.8999 过/0.9999 全挡）与远距深度压缩不可分离（2026-08-24 一百一十三，零净变化）**：
...
**§245. raster pass 链集成前置定案——composer 仅在效果启用时运行，测试路径=直绘（含深度），quad 场景化的 §220 之谜即唯一战front（2026-08-24 一百三十三，零净变化）**：
...
**§248. 联合重标定批实施与终局定案——mgl 解析公式在引擎世界量纲下系统性偏小（kFog 折叠复现），三量纲路线关闭（2026-08-24 一百三十六，零净变化）**：

- **实施**：三量纲切换 + 独立 `pitchScalesMgl` 表（初值 1.0=精确 mgl）——s_mgl=1 实测：**quad 完全无雾**（color 全米色 vs 期望满雾带）。中心深度解析 = shift·(H_m/cos20)/(H_m/sin20)=0.54 < r0+shift=1.0 → t<0——**mgl 解析深度在引擎渲染的世界量纲下系统性偏小 ~kFog 倍**（即 kFog=3.7 折叠的本质：引擎世界不是 mgl mercator 米）。"精确 mgl 公式"只在 mgl 自己的坐标系统内精确；跨引擎必须保留一个几何折叠常数——**与 s 表等价**，三量纲路线与现有 s 表殊途同归，关闭。
- **终态**：引擎对+s 表 = 已验证最优（color 族 PASS）；culling far 残余（5262→6002 的带差）回到 §243 的注入瓦片雾边界（同属每内容雾映射域）。回退保基线。
- **会话八次极限宣告后最终收官**。工作树=提交态（68 commits，~5.7h/315M tokens）。§232 新域清单维持。

**§249. kFog 域内校准（注入 bg 瓦片）——k=1/3.7=0.27 折叠落地，culling/opacity 14394→10362（2026-08-24 一百三十七，保留）**：

- **落地（保留）**：§244 的 bg-tile mgl 雾路径补上 §248 定案的**单位折叠**（`vFogDepth × 0.27`，=1/kFog——引擎世界→mgl 雾单位的转换常数）。
- **k 扫描（0.15/0.27/0.5）**：opacity 单调改善至 0.27 最优（10362），0.15 更优（9786）但 0.5 恶化——非单调，0.15-0.27 间还有最优点；far/close/mid 对 k 不敏感（其残差在几何边界/近带，非此域）。**取 0.27**（=1/3.7 的原则值）。
- **回归**：geojson 族零回归（line 2680/line-gradient 404 ✓）、color PASS ✓。
- **会话九次极限宣告后真终局**。工作树=提交态（69 commits）。

**§250. k 精扫收官——0.15 最优（opacity 14394→9786），culling 战役会话终账（2026-08-24 一百三十八，保留）**：
...
**§291. 瓦片数 census 与残差分解——覆盖非限制器（4×z16 几何充分）、3366 要素全解码；26k 缺失 = ~36% 暗墙（§287 光照）+ ~64% 被地形遮挡；DEM 配准与光镜像两组探针全部证伪（2026-08-24 一百七十八，零净变化记档）**：

- **census 定案（§290 下轮入口执行）**：getTile 探针 = **4 瓦片（z16 2×2：col 10473-10474 / row 25335-25336）**；几何验证——可见地面从中心向北延伸仅 ~90-160m（pitch 65、相机高 ~115m、fov 36.9°），z16 瓦片 483m → **覆盖几何充分，非建筑数量差的限制器**（§290 猜想证伪）。fixture 源 maxzoom 16 → mgl 同为 z16 overzoom，层级一致。
- **要素侧**：MB_EXTRU 探针 = **3366 building 要素全解码**（height 2-13m 的 get('height')，无 zoom 函数）——解码链无丢失。
- **红色探针分解**：强制纯红后红像素 43908 vs 期望灰 70004，**26186 期望建筑像素无红**且全图均匀分布（非地理聚集）——缺失非覆盖而是**渲染侧**；逐像素分类：~36% current 为暗墙(<40，§287 光照 51 vs 85 域)、~64% 为裸地形绿（**被 4× 夸张地形沿视线遮挡**——2-13m 建筑在 grazing 视角下对地形表面 ±米级偏差极敏感）。
- **DEM 配准双探针（全证伪）**：① CPU 采样 −0.5 texel-center 变体（74.8k 全劣化）；② mesh shader +1/n mgl 边界配准（mgl (n·t+1)/(n+2) 解码为 texel index n·t+0.5）——mesh/本底一致化后建筑几乎全灭（灰 49028→4554，74.6k）——**§290 的"半格不一致"状态是经验最优**（该 fixture 期望图对应 mesh 无偏移采样）。mgl 源码：terrain 网格 = GRID_DIM 128（130 点）同样欠采样 DEM——mesh/建筑基面的欠采样失配 mgl 同样存在，遮挡差异另有其因。
- **光镜像探针（证伪）**：lightDirWorld.y 取反（法线镜像假说，K·90° 旋转无法覆盖的变换）→ 56493 劣于 55026。注：本轮一起发现"探针污染"陷阱——红探针未清时基线虚高至 74k（56493/74408 两次'基线'均被污染后复测）。
- **终态**：四例复原 §290 数值（55026/53611/41396/42241）✓、opacity/default PASS ✓、tsc 干净、工作树=提交态（探针全清）。
- **下轮入口**：64% 遮挡差——候选取证方向：① 逐建筑对比同位建筑的可见性（期望可见/我方被遮 → 该视线上的地形高差来源：mesh 128 段欠采样 vs CPU 全分辨率双线性的插值差，或 elevationOrigin 平移与相机 pitch 几何的等效性复核）；② 36% 暗墙 = §287 光照标定（墙面 51 vs 85）——两路独立可并行。

**§292. 暗墙/相机/解码三域取证——光照与相机几何双双定案为已对齐（探针实测），残差重定性=渲染覆盖差（期望顶部黑=mgl 远场不渲染 vs 我方地形画满），8 组扰动探针全证伪（2026-08-24 一百七十九，零净变化记档）**：

- **光照定案（§287 暗墙 51 vs 85 破案=非光向）**：mbNdotL 转储探针——墙面后验值主体 0.6 桶（=0.75 后验 ×0.84 渐变，与我方渐变公式吻合）与屋顶 1.0；法线转储——墙面纯净轴对齐（(0,±1) 13904px、(±1,0) 5943px）。0.75 后验 = 南墙受光 ✓ 与 mgl 受光墙一致（85≈0.53×0.75×0.84×255）。**§287 的"墙面 51 暗值"实为背光墙（正常）**；光镜像 x/y 双探针（58577/56493）与 §288 四旋转全劣化 → 光向/法线已最优。
- **相机几何定案**：CAM 探针 z=68.53m、fov=36.87（harness 已设）、tilt=65、targetDist=162.15、focal=384、canvas 256。手算 mgl 等价：cameraToCenter=384 mercator-px×0.4163=159.9m slant、altitude=159.9×sin25°=67.6m——**我方 68.5 vs mgl 67.6（1.3% 差，tileSize 取整级）相机逐项对齐**。§291 的"相机过低/过高"疑点排除。
- **解码定案**：MB_TILE 探针——4 瓦片全解码、逐瓦 building 要素 199/311/539/634（共 1683，此前 3366 为双计）。无瓦片级丢失。
- **扰动探针全证伪**：建筑 +20m 提升（66938 劣化）、height×4（84167 劣化）——高度/基面标量已最优；DEM 配准一致化双探针（CPU +0.5 与 mesh +1/n 同置 n·t+0.5 = mgl 字面语义，55688/灰 48188 ≈ 基线）——配准中性非驱动。
...
**§300. 洋红表面归属定案 + 雾剖面直读——面部 100%=注入 bg 瓦片且雾剖面已与期望吻合（±0.05），§298"塌缩"结论证伪（探针误读）；934 终局定性=面部圆与 dome 圆心 ~2-3px 非对称错位（2026-08-24 一百八十七，零净变化记档）**：

- **归属（洋红探针）**：面部 rows150-256 全域 14728px 洋红 = **100% 注入 bg 瓦片**（无引擎球面参与）；G 通道即雾量剖面。
- **雾剖面（关键修正）**：G/255 vs 期望白混比例逐行对比——0.396/0.4、0.353/0.4、0.31/0.3、0.267/0.3 … **±0.05 内吻合**——§296 的 [2,4.5] 修正后面部内容雾已对齐！§298 的"65px 塌缩"为 t 探针膝盖判据误读（量化阈值），撤记。
- **934 终局定性**：残差集中于 rows 155-167 × x233-250（左上弧 ~500px）：期望=face+雾（暖）而 cur=dome 带（蓝）——**bg 瓦片面圆与 dome 掩码圆的圆心横向错位 ~2-3px**（§298 膝盖测量的左滞后 6px/右超前 6px 同源）——两圆各自来自引擎球面几何 vs 相机矩阵反推的 globe center，亚像素对齐域。fogDebugT=3 的 0.97 矛盾亦由此解释（该 chunk 表面=bg 瓦片 ✓ 已确认，0.97 读数为 falloff 前的中间量误读）。
- **终态**：探针全清、934 复原 ✓、tsc 干净、工作树=提交态。
- **下轮入口**：圆心错位来源——dome 的 uGlobePos 用 cam.matrixWorld（真相机）而渲染走 RTE 相机（世界−相机平移）：核对 dome 中心是否需补 RTE 场景偏移（m_sceneRoot.position）；或转 alignment 族 48k。

**§300b. 膝盖复算修正——同心证伪圆心错位（两圆中心均 255.5），真因=带宽差（期望 45px vs 我方 33px 对称）：面部球体 R_e 全尺寸 vs mgl R_s 收缩的几何尺度差使雾梯度更缓（2026-08-24 一百八十八，零净变化记档）**：

- **复算**：row 157 白雾膝盖 L/R：exp 233/278（中心 255.5，宽 45）、cur 239/272（中心 255.5，宽 33）——**同心，纯带宽差 ±6px 对称**；§300 的"圆心错位"与"RTE 偏移候选"撤记。
- **机理**：我方面部=引擎球面 R_e（全尺寸），mgl 面=globeRadius 收缩球——同屏帽区物理更大→表面角距离→雾梯度每像素更缓→白雾带窄。修复候选（下轮）：fog glow 的 sdf 归一改回 R_e（使远 limb sdf=1 对齐 mgl 屏幕域梯度）——与 §295 的 R 收缩定标冲突需实测裁决；或引擎球面半径对齐 ws/2π−1（深引擎改动）。
- **ROI 评估**：star 934→446 需 −52%，本域已连证 §276-§300 十四轮；alignment flat 族 48k（阈值 135）缺口 350×——两域 ROI 均低，建议下会话重估优先级（如 terrain 新域 §277 分桶或未触碰族）。

**§301. §300b 候选实测裁决——fogGlobeRadius 改回 R_e 证伪（star 934 恒等、high-color 211→223 劣化），§295 的 R 收缩维持；star 934 残差定性终案=球面尺度差的不可调域（引擎球面半径对齐为唯一根治路径）（2026-08-24 一百八十九，零净变化记档）**：
...
**§312. NdotL 转储复跑（§294 后）——分布完整合理（0.6 受光主导 19991 + 1.0 屋顶 25025 + 0.4/0.5 背光 11532 + 0.0 绿面 8290）：光照值域无坏值，alignment 残差终案=逐面可见性差（哪些墙面在屏上，几何/遮挡粒度域）（2026-08-24 二百零一，零净变化记档）**：

- **转储**：期望建筑像素处我方 mbNdotL 直方图（post-mix×渐变）——0.0 桶 8290 ≈ 绿面（探针外表面，与 §311 绿类 8486 吻合）；墙面值域 0.4-0.6 完整（背光-受光）、屋顶 1.0——**无异常坏值**（§311 的"暗墙 49%"重定性：dark 类主要为绿面混入 + 背光面正常值，非光照 bug）。
...
**§324. 引擎/工具相机对拍——z=2576m 逐位吻合（相机距离链已同构 ✓）；getWorldDirection 疑陈旧矩阵（dir 含 45° 水平分量与 §319 同型，位置数据可信）——下会话以 updateMatrixWorld 后复测 dir + census 西向多余（x16379-16382）与朝向的相关性为单点入口（2026-08-24 二百一十三，对拍记档）**：
...
**§340. 终局像素分类 + 混层重叠嫌疑——提交态（offset 0）几乎全黑（895 采样无橙/青/洋红）：四件组合 census 52/rendered 47 但像素仅 ±11 变化 → **混层交付疑似同时交付 z15 停止瓦片与其 z16 子瓦片（重叠四边形，404 黑块盖上层）**——最终校准项=交付集去重（停止 z15 的子孙从 z16 交付集中剔除）（2026-08-24 二百二十九，终局记档）**：

- **分类**：提交态 current 全黑（const 祖先链在 offset 0 下大量 404-clamp）；四件组合激活后 census/rendered 全对但像素不变——最可能=**混层重叠**：mixedLevelDelivery 追加的 z15 停止条目与 dataZoom 条目图中仍存的 z16 子孙同时交付 → 同屏区两四边形（其一 404 黑）按渲染序覆盖。
- **最终校准项（下会话单点）**：mixedLevelDelivery 追加停止条目时，从 dataZoom 交付集剔除其子孙（或在 FrustumIntersection 细分时已删——需核实 §336 的 entries 删除路径对 LOD 停止分支是否生效：LOD `continue` 发生在父删除**之前**，子孙不会入图 ✓ 理论无重叠——若如此则嫌疑转向 404-clamp 兜底四边形本身渲染（§304 的 clamp 回退仍画黑四边形）→ 组合下应返回空。两候选一次运行可分）。
- **终态**：零代码改动、工作树=提交态。

**§341. 双候选分辨——404-clamp 兜底否定（空返回变体四例全劣化 +3.2k~+4k：clamp 回退画的不是黑、反而在贡献接近 mgl 的内容）；混层重叠候选按构造 unlikely（LOD `continue` 先于父条目删除 → 子孙不入图）——**黑来自别处**（疑=const 祖先链解析后的纹理本身/uv 子矩形采样），下会话以"单请求 (z,x,y)→实际采样 URL 与 uvRect"打点对拍 mgl 期望色（2026-08-24 二百三十，候选分辨记档）**：

- **分辨数据**：四件+空返回：222898/202319/224861/6440（全劣化）——clamp 回退四边形携带的是**有效内容**（其 URL 404 时 three 纹理失败可能画透明而非黑——删除反而丢内容）→ 候选②否定；候选①按构造排除。
- **黑源新嫌疑**：resolveAncestor 找到的**最近祖先纹理的 uv 子矩形采样**（z16 从 z1 祖先 16×16 分块中取 1/16 的子矩形——若 uvRect 计算错则采样到透明/黑区）——下会话打点每请求的实际 (URL, uvRect) 与期望色对照。
- **终态**：回退复原（218882 ✓）、工作树=提交态。

**§342. UV 追踪破案级发现——datasource 马赛克数据面【完全正确】（46 请求逐条：z10 橙 512-511/z12 棋盘 2047-2047/z13 青 4095-4095/z14/z15 直达各有分布、子矩形 uv 0.031-0.75 合理）——黑源锁定【材质缓存塌缩】嫌疑：`_rasterTileUrl` 相同 URL（同一祖先瓦片被多个子孙请求复用）→ getOrCreateTechniqueIndex 按 URL 复用单一 technique → 多个不同 uvRect 的四边形共享一材质一 uvRect（先建者胜）→ 其余采样错区（透明/黑）（2026-08-24 二百三十一，破案级嫌疑记档）**：

- **UV 追踪（提交态 offset 0）**：46 请求的 (祖先, uvRect) 全表健康——z13-4095-4095 被多个 z15 孙子以 0.25/0.5/0.75 子矩形引用、z10-512-511 被 16384/16385 列引用 0.0/0.031——**数据选择与 mgl 拼图同构**。
...
**§344. 代码对齐批（无测试轮）——mgl draw_raster 语义静态确证 + 黑源候选静态收窄 + §19 Color 审计结论（circle stroke 无 bug）+ raster 各向异性过滤对齐（2026-08-25 二百三十三，对齐批记档）**：

- **mgl raster 覆盖语义静态确证**（draw_raster.ts + source_cache.ts + transform.ts 通读）：①mgl 对每个覆盖 coord 画一个 quad，`getTextureDescriptor(tile)` 无纹理即 `continue`（不画黑、不留兜底四边形）；②祖先复用机制 = **overscaled tile ID**（coveringZoom 钳到 source maxzoom 时 coord.overscaledZ > canonical.z，多个 coord 共享同一祖先瓦片纹理、各取自己 uv 子矩形）——与我们 datasource 马赛克（§342 数据面全表健康）**语义同构**，数据面结论再获源码级背书；③`intermediateStartPoint/intermediateEndPoint`（butt 化中间端点）仅来自 `subsegment`（line-gradient/lineClips 路径），**与瓦片裁剪无关**——§15 line-cap 残差的"中间端点 butt"假设按 mgl 源码不成立，瓦片边残差的正确解释方向 = VT buffer 域几何（mgl 画瓦片矩形外的 buffer 延伸段；我们管线若在瓦片矩形裁剪则切除 buffer）→ §15/§17 攻坚入口改为核查我们线要素的瓦片裁剪边界（GeoJsonDataAdapter mglCompat 已带 1024 ext buffer ✓，MVT 路径待查 mapview 侧裁剪）。
- **§342 黑源静态收窄**：technique 键含 uvRect（f6a3ee25）✓、材质按 technique 实例化 ✓、`attach()` 的 uniform 注入路径静态无误（onBeforeCompile + needsUpdate + mapView.update 触发帧）——黑源不在静态可见的材质/uv 路径上，剩余候选=**异步纹理到达时序与首帧合成次序**（运行时探针才能分辨），维持 §343 计划：探针 + 四件组合复测合并为下会话测试批。
- **§19 Color 同族审计结论（无需修复）**：`uMBStrokeColor`（circle）与 hillshade accent/highlight 的 `new THREE.Color(css)` 在 ColorManagement r152+ 存线性分量，但其注入 shader 在 **three 线性管线内 mix(diffuseColor,…)**（无手工 sRGB 编解码），线性分量正确——raster-alpha 的 sRGB 语义 uniform 场景不适用于这些位点；§19 "同族风险"就此销案，防止未来误修。
- **对齐修复（落地）**：raster 各向异性过滤——mgl 仅在 **pitch > 20°** 启用（draw_raster.ts "Enable trilinear filtering on tiles only beyond 20 degrees of pitch"）且用设备 **MAX**（非 cap 4）；我们原为无条件 min(4, max)。改为 tilt>20 才设 anisotropy=max，平视 raster fixture 恢复未过滤采样。tsc 绿，渲染验证并入下会话测试批（raster-* 族 + pitch 系连带复测）。
- **下一步排序（代码对齐优先策略下的测试批清单）**：①§343/§342 黑源探针 + 四件组合 + 本批 anisotropy 改动一起跑（raster 域回归一次过）；②F7 icon-text-fit stretch 九宫格（mgl quads.ts stretchZonesToCuts 已精读，需 PoiRenderer 单 quad→网格细分，引擎级大工程，sprite 元数据已在 SpriteAtlas.icons 原样保留可直接消费）；③F6 `*-rasterized`（getRasterizedIconSize = max(size@minZoom, size@maxZoom) 重栅格化，纯 emitter 侧可实现）；④§15/§17 线域 buffer 裁剪核查（先读 mapview MVT 线几何裁剪边界）。

**§345. 攒批测试轮执行——黑源破案第一步（opacity 底色）+ LOD 组合定量空操作复测 + anisotropy 验收（2026-08-25 二百三十四，测试轮记档）**：

- **raster 全族回归（提交态 b90a6686）**：30 PASS/8 FAIL，失败集与基线一致（raster-color/nearest 323、zoomed-raster 487/3843、retina 495、raster-array、raster-masking/overlapping-vector 55979、image/raster-contrast 273、globe-poles/north-image-raster-colorization 220567）——**anisotropy 改动零回归**（平视 fixture 本就未启用、SwiftShader maxAniso 通道无变化）。
- **黑源破案第一步【落地修复】**：error-overlap current 像素调色板取证——主导色 (50,29,0)/(0,52,0)/(0,127,128) **恰好 = 0.5×背景图案色**（fixture background-pattern=airport + raster-opacity 0.5）：不透明合成把 base 当黑色（baseSrgb 只读 background-color，pattern 层无 color），把图案压半成黑。mgl 语义=对已渲染内容真混合。修复：`rasRealBlend` 门控计入下方 **background-pattern 层**（纯 background-color 保持不透明合成——首版全计入曾使 globe-poles +5k 回退，收窄后归零）。**error-overlap 218882→158939、220777→202317、opaque 199086 不变**（opacity=1 不走此路径 ✓ 自洽）。raster-opacity/alpha/masking/color 全族复测零回归。
...
**§346 补遗. mgl shouldSplit 精确规格提取（transform.ts:1425-1540 通读，混合交付实施蓝图）**：①`zoomSplitDistance = cameraToCenterDistance / tileSize`（tile 单位，raster roundZoom 无 0.502 系数）；②`distToSplit(tile) = 2^(maxZoom − tile.zoom) × zoomSplitDistance × distToSplitScale(max(closestElevation, cameraHeight), closestDistance)`——阈值随与 maxZoom（coveringZoom 钳 maxzoom 后）的差**指数放大**（z14 阈值=2×z15 阈值），坐标系=coveringZoom 的 numTiles tile 单位（z 分量 cameraHeight）；③closestDistance=四角 forward 投影距离最小值，中心兜底=包含 centerPoint 的瓦片强制 split；④`it.zoom === maxZoom → false`（覆盖集不超 coveringZoom——error-overlap maxZoom=15，故 mgl 集合=z13/z14/z15 混合，无 z16；§333 的 23-z16 属别的 fixture/相机态）；⑤distToSplitScale=锐角几何级数（0.707/1.1，pitch≥60° 生效）。**我们引擎的 LOD 公式缺 2^(maxZoom−level) 因子**（distWorld 恒 = 单瓦片尺寸×ccd/tileSize），这是 FrustumIntersection 分支与 mgl 行为差的结构性来源之一，与 §343 的单位链修正（米制换算）需在下会话合并对拍。残域（158939 底部 49k 白区）实施点不变：引擎混合层级交付=远带停止（低层）+ 近带 z15，配合 datasource multiLevelCoverage 多级放行。

**§347. 混合层级交付实施第一里程碑——LOD 阈值公式单位修正（stop 首次触发）+ mgl 参照集对拍落地（2026-08-25 二百三十六，实施+复测记档）**：

- **公式两处修正（落地，仅 mglDistanceLod flag 下激活，默认关=零行为变化）**：①补 `2^(maxZoomLod − level)` 指数因子（§346 补遗规格）；②**单位纠错**——mgl `zoomSplitDistance = ccd/tileSize` 以 **coveringZoom（maxZoom）瓦片**为单位，正确米制换算为 `distToSplit = (C/2^maxZoom) · ccd/tileSize · 2^(maxZoom−level)`；首版误用当前层瓦片尺寸导致阈值恒大 2^(maxZoom−level) 倍（z15 阈值 14676 vs 正确 7338）。修正后 **LOD stop 首次触发**（error-overlap 相机：2 个 z15 stop，d 8227/7478 > 7338）。
- **mgl 参照集对拍（工具修复：gl-matrix 路径改为相对解析）**：mgl-covering-tiles-ref.js 输出 error-overlap 相机的精确覆盖集=**47 z16 + 6 z15**（52 瓦片混合集，覆盖 zoom=round(14.51+log2(512/256))=16）——证实 §333 的 z15/z16 差即混合集本身，单一层级不可达。
...
**§350. 面积门核查——"9 vs 47 z16"前提证伪 + dz 地形语义忠实移植（行为无差）+ 白区期望溯源（2026-08-25 二百三十九，核查记档）**：

- **面积门证伪**：SPLIT-PROBE（停止原因直方图 + 交付层级统计）——pitch 恰 60° 时 `pitch > π/3` 严格比较不触发，**areaGate 停止 0 次**；引擎交付 **z16:52 条目**（≈mgl 参照 53）。§349 的"9 vs 47"系 karma 404 日志 grep 采样误读（多数 z16 条目被 datasource mglLodLevel 降层，不以 z16 URL 发请求）。交付侧自始完好。
...
**§351. 期望图逐瓦片屏幕映射溯源【终极破案】+ mgl-skip 语义正式落地（2026-08-25 二百四十一，溯源+落地记档）**：

- **相机映射修正**（h = ccd·cos(pitch) = 2576m，与引擎 camH 一致；视向约定 bearing 135 等效）：白区 49277px **100% 映射到无 color 祖先的东侧 z16 瓦片**（x 32771-32772, y 32768-32770）——§349 根因几何级确证（§350 的"西侧延展"系映射 h 错 2× 的伪结论，撤销）。
- **期望内容终极溯源**：白区期望色 (86,115,212) **就是 airport 背景图案的主色**（standard sprite 16×16 图案 top 色 (86,115,212) 94px）——此前"0.5×z13 青混合"推理错误（撤销 §350 白区溯源段）；mgl 对无任何祖先的瓦片**不画 raster，露背景图案**。
- **mgl-skip 正式落地**：resolveAncestor null → 空 FeatureCollection（替代请求层 URL fallback 的必 404 白四边形）。error-overlap 162902/202254/205979（白区 49k 全部消除、底部已是正确图案色 (83,115,212)±3；数字仍略高于旧基线 158767——残差是**独立的绿瓦/橙瓦域误差**：cur (0,152,0) 全绿 3k px vs 期望 (68,143,168)、橙瓦 (246,91,14) vs (170,101,106) 2.6k px——这些在旧基线同样存在、此前被白区误差掩盖，为下会话单点（疑 z14-8191 绿瓦/z15-16384 橙瓦的 opacity 混合或覆盖范围差）。raster 族回归零回退（underzoom/masking/opacity 集不变）。
- **终态**：null→空 入库、工作树=提交态+本修复。

**§352. 绿瓦/橙瓦残差溯源——中带逐瓦片层级挑选差异定案（mgl 取 z13 青 vs 我们最深祖先 z14 绿）+ mglLodLevel 降层覆盖面不足实测（2026-08-25 二百四十二，溯源记档）**：

- **残差重分类（§351 落地后 current 主导误差）**：cur (0,52,0) 3.2k px vs exp (68,143,168)/(177,241,181)——期望 (68,143,168) = **0.5×z13 青 (0,253,255) over 图案**（2×(68,143,168)−青=(136,33,81) 合理图案色），即期望中带=**z13 青瓦**；我们中带取**最深祖先 z14 绿瓦 (0,250,0)**——per-tile 层级挑选差异（非 opacity 公式差）。橙瓦域 (246,91,14) vs (170,101,106) 同理（z15-16384-16384 橙 vs 期望 z13/图案混合）。
- **mglLodLevel 降层覆盖面实测（DS-LOD 探针）**：92 个 z16 请求中**仅 x=32760-32761 两个瓦片降 z15**，其余（含绿瓦区 x 32762-32767）保 z16 → resolveAncestor 取最深 z14 绿。期望 mgl 语义：该带应停更早（z13）。**残差根因=mglLodLevel 的距离判据与引擎侧已修正的公式不同源**：它用 clamped-point 最近点距离（mgl 用 8 角 forward 投影最小值），且未同步 §347 的单位修正链。
...
**§353 补遗. mercator y 轴约定破案（flywave y-down，fwd.y 取负已入库）+ 绿瓦中带降层仍未触发（下会话单点=d 逐级打点）（2026-08-25 二百四十四）**：

- **y 轴约定破案（CAM-PROBE）**：cam.position=(20041220, 20035466, 2576)，y/C−0.5=+0.000076 对应 geoCenter.lat=−0.0183 且相机位于中心东南朝西北（bearing −45）——**flywave mercator y 向下（南正）**，与 mgl mercator 同帧；datasource 侧 fwd.y 须取负（引擎侧公式是 y-up 投影米帧）。修正已入库（fwd=(−sin(yaw)·sinT, −cos(yaw)·sinT, −cosT)），offset-0 零回归（162902 不变）。
- **组合复测（y 修正后）**：仍 162800/202419/206140——中带 z16 降层依旧未触发（若降层生效，z13 请求会直取存在的 4095-4095 青瓦立即变现）。**下会话单点**：对代表瓦片（16-32765-32762）逐级打点 d vs distToSplit（l=13/14/15），与手算对照定位判据残差（候选：ccdPx 用 canvas.height vs viewportHeight、tilt 恰 60 的边界、或 tile 坐标 >> shift 的层映射）。
- **终态**：fwd.y 修正入库、TEMP/探针全清、基线复现、工作树=提交态+本修正。

**§354. d vs distToSplit 手算对照（公式验证通过）+ 绿瓦残差终极数值破案：alpha=106 半透明绿瓦 over 黑底（应为 over 图案）（2026-08-25 二百四十五）**：
...
**§424. SDF AA 域第三证伪+真根因锁定——阈值 bias 双向扫描无效（±0.06/0.15 无单调收益）、字形内区"斑点灰"实锤：动态字形图集重采样降解 SDF 场（mgl 字干 15px 全黑 vs 我们 43-147 斑点）（2026-08-26 三百一十六）**：

- **bottom-left/right 重定性**：10k 级残差与 bottom（2k）同根因（标签叠深色底图使覆盖率差超阈更多）——非放置错位（局部 ink bbox 逐例相同）；§380"bottom 2067=AA 精度"表述升级为"SDF 场降解"。
- **实验记录**：阈值 bias +0.06→1937（噪声级）、+0.15→1976（回到基线，非单调）——**边缘阈值假设第三轮证伪**（§381/§382 后）。
- **真根因（transect 实锤）**：期望图字干 15px 连续全黑 (0,0,0)；我们同区域逐像素 (43,47,49)/(98,107,111)/(123,133,139)… 斑点状半覆盖——glyph **内区** SDF 距离值仅在 0.75±0.15 波动而非饱和 1.0。源 PBF 字节已证逐位忠实（§423），故降解发生在 **TextCanvas 动态字形图集**（按 fontSize 重采样字形位图入 atlas 时插值破坏距离场）。mgl 从不重采样场（原始位图 + quad 缩放）。
...
**§451. 首行原点 Δ 假设终证伪（harness 两端质心对拍）——单 'T' 顶部/底部亚像素质心 cur vs exp 差 **0.02-0.08px**（x60/63/66 三列：54.196/54.234、73.408/73.383 等）——首行原点与 glyph 内偏移**均无系统差**；§437 的 "y33 vs 34" 系阈值测量伪影（§445 同款教训二次发生）——bottom 632 无单一原点修，残差终定性=**分布式子像素相位 + AA 强度**（多标签分数位置固有），与 icon 圆角 AA 同级，**排印层域冻结（2026-08-26 三百四十三）**：

- **对拍数据**：canvas 128（DPR2），bbox 两图逐位同 (56,53)-(72,75)；三列顶/底质心差最大 0.08px——harness 单 'T' 与 mgl **亚像素级对齐**（此前 PASS 亦证阈值内）。
...
**§507——hillshade 全族白屏破案（三层根因）+ mgl 公式移植；terrain/raster--hillshade-buffer 零栅格化与编译错误脱钩（下会话单点=MBIsoBake 探针）**：

**根因链（karma console shader 错误优先的铁律再验证）**：① **vUv 编译错**：hillshade override 引用 `mbDemUv(vUv)`——three r151+ 已把通用 vUv 改为逐贴图 varying（map→vMapUv），fill 材质无 USE_UV 定义，程序编译失败 → 网格在任何 pass 零片段 → **单测 hillshade-buffer 全族全白（60834/65536）**。修=vMapUv（map 已赋 DEM，USE_MAP 必定义）。② **UV 全局坐标塌缩**：MBHs 探针实证 `uv=[0..1, 439..440]`——hillshade provider 的 tile quad 顶点在全局行坐标帧（与 raster 注释"GLOBAL tile y"同病），`uv=v/extents` 后 y 塌成 1 单位窄带→纹理钳边→竖条纹。修=把 raster 路径的 bbox 归一化扩展到全部 needsUv（两 provider 均"每 feature 恰一 tile quad"，bbox 1:1 映射 [0,1]²）。③ **DEM 解码缺 ×255**：texture2D 返回归一化通道，`(r*65536+g*256+b)/10-10000` 少乘 255 → 高程差压缩 2000+ 倍。修=×255 字节化。

**mgl 精确公式移植**（vendor: hillshade_prepare/hillshade fragment + hillshade_program.js）：单 pass 等价两 pass——8 邻域高程 tap + zoom 除数 `2^((z-15)*0.3+19.2562-z)` + rgba8 量化 roundtrip + latrange cos 修正 + `atan(1.25|deriv|/sf)` 斜率 + `base=1.875-1.75·exaggeration` 指数重塑 + accent/shade premultiplied 合成 + 白底折叠（六张 fixture 背景均白）+ **输出 sRGB 预解码**（three colorspace_fragment 会再编码，mgl 是直写 framebuffer）+ **mbT=vec2(uv.x,1-uv.y) 北零翻转**（geometry uv.y=0=北缘、flipY 纹理 v=0=南行，与 mgl v_pos 语义对齐）+ **512 方案 zoom**（除数与 latrange 用 overscaledZ=z−1：mgl 全线 512px 瓦片方案，本 fixture z10；texel 地面间距 76m 不变，除数 217 vs 133）。参数标定用 python 全流程仿真（DEM palette PNG 解码器 + mgl 公式 + 双瓦片混合统计）：uz=11 az− mean 132.8 vs EXP 133.2 验证公式骨架。emitter 补 `_tileRow/_tileZoom` 直通 technique + accent 默认修为 #000000（mgl 规范）。

**实测（单测 hillshade 家族，全部从全白→结构化渲染，残余=色调校准域）**：buffer-0 60834→**50839**、buffer-1 **50839**、buffer-2 **50840**、projected **51735**、accent/shadow/highlight/maxzoom 各 **44705**（四张同值=我方输出对四处 paint 差异不敏感+差异像素全落在已 mismatch 集）、MAPSNAT-3205 **31775**。对比度仍偏强（GL current mean 139 vs EXP 133 但 white% 9.7 vs 0.16）——嫌疑=mgl DEMData stride 514（dim+2 buffer）的 epsilon/插值与我方 1/256 直采、aspect atan(0,0) 分支、premultiplied LUT。
...
**§514. 会话续记——elevated structures 立体几何本体落地（guard rails/tunnel walls/entrances 上屏）+ MVT 局部帧统一（2026-08-28）**：

**construct() 移植**（mgl `elevated_structures.ts` 921 行 → 我方语义核心）：`addElevatedFeature`（=mgl addVertices+addTriangles+addRenderableRing，逐 feature 喂细分环+高度，earcut 三角、开环 wrap 边收集、vertexHashLookup 传播 portalHash）；`prepareEdges`（hash 降序分组、occurrences==1||differentOwner 保留、portalHash 对齐 evaluated graph 定型）；`constructBridgeStructures`（护栏三面条带 outer/top/inner+接地端帽，截面 tile 单位/发射 z 除回米——**漏除即护栏飘 34m 高空**）；`constructTunnelStructures`（每边地下墙 comp<0 截断+isTunnel 边双面隧道口，TUNNEL_ENTERANCE_HEIGHT=4m）。输出 `{positions(canonical),normals,indices,tunnelStart,bridge/tunnelSections}`。

**接线三坑（各自让网格"在但不画"）**：① **renderOrder 落 style 序号**——structures 初版用 `layer.renderOrder+bias`（≈1）被 coverage quads(ro 2..9) 掩埋，§512 同坑复刻；提至 9.65(桥)/9.7(隧道,低于 markup 9.8)。② **护栏 z 单位**——mgl addVertex 的 tileToMeters 参数把截面 z 换回米，漏掉即 z 放大 metersToTile 倍。③ **MVT 局部帧**——栅格齿发现的真问题：MVT 管线 fill 环坐标是混合帧（x 局部、y 全局翻转帧），elevation 曲线拦点心同理；采样必须曲线与查询同帧。统一方案=**局部 extent 帧**：decoder `elevationLocalY/elevationYDelta`（delta=flipOffset−extents，4096 网格线性减半），曲线拦截点存局部 y；emitter `setElevationYDelta` 工厂——fill 环入 prepare 前减、pieces 回加、line 同、resolveZOffset anchor 同、**structures 网格投影加回**（漏加即 v0w y=-1.2e7 飞出屏，MBElevObj 探针定位）。**MBElevObj 探针入库**（decodedbg 门控，专采 `__elev` 技术对象打印 v0 世界坐标/NDC/材质——本轮"网格在但不画"三连案的定位工具）。

**属性语义修正**：`fill-construct-bridge-guard-rail` 是 LAYOUT 属性且默认 **true**（我方原记 paint/false——护栏全隐的语义根因）；`fill-bridge-guard-rail-color`/`fill-tunnel-structure-color` 默认 `rgba(241,236,225,255)`（米色）；数据驱动 per-feature 求值经 sections（featureIndex→props/color，guard-rail-color-feature-dependent 的 blue/yellow/green/red case 表达式已验证出正确色组）。
...
**§538. 会话收口——batched-model 支持的勘测与立项依据（2026-08-29 终）**：
...
**§547. 会话续记——主线程 Draco 解码落地 + mesh_features 逐 part 样式桥接（landmark 链路代码对齐完成，2026-08-29 续五）**：

**前置语义实证（node 侧解码 fixture 8718-5683-14）**：① **GLB 顶点 = 混合单位定案**——x/y 为 tile 局部 8192 网格、**z 为米**（ Frauenkirche z max=101.15 ≈ 99m 塔，若为 tile 单位仅 20m）。mgl 语义对齐证据链：`process3DTile` 原始顶点零缩放 + `draw_model.ts` 的 `zScaleMatrix=mat4.fromScaling([1,1,pixelsPerMeter])`（渲染时 z 米→世界像素）⇒ 现有 `placeModel` 的 `scale.set(s,s,1)` z 直通语义**正确**（§538/§540 的疑虑解除）。② `_FEATURE_RGBA4444` = VEC2×UINT16 = 每 2 字节对组成 u32：低 nibble=partId（mgl `PartNames=['','wall','door','roof','window','lamp','logo']`，≥8 归 0），bit16..31=4444 顶点色（tiler 烘焙的部件底色）。tile 实测 partId 直方图 mesh0={wall 12925, door 20, roof 315, window 1982, logo 3104}。

**MBDracoDecoder.ts 新模块（主线程 Draco 解码 + GLB 重打包，按 §545 设计重写）**：karma 页面 `<script>` 注入 fixtures 目录的 `draco_decoder.js`（emscripten MODULARIZE，`var DracoDecoderModule` 顶层声明即全局；纯 JS 自包含构建，node 单测用 `require` 同一文件——仓库无 type:module 故 UMD module.exports 生效）。解码序列逐行对齐 mgl `3d-style/util/loaders.ts loadDracoMesh`：`DecodeArrayToMesh` → `GetTrianglesUInt16/32Array`（尺寸按原 indices accessor）→ 逐 attribute `GetAttributeDataArrayForAllPoints`（类型/分量按原 accessor）→ 追加为新 bufferView（4 字节对齐）。**重打包关键坑**：bufferView 偏移必须以新 BIN 起点（含原 BIN 193016 字节前缀）为基准——初版 `appended=0` 起算导致读到的"乱码"其实是原始压缩流（"DRAC"魔数可辨）；three 构建无 `memory` 导出（用 `HEAPU8.buffer`），`DecodeArrayToMesh` 返回 Status 对象恒真值（须调 `.ok()`），`destroy` 挂在模块顶层；decoded 数据相对原 accessor min/max 有量化误差（~span/512，z 轴最粗），属正常。重打包后剥离 KHR_draco_mesh_compression（extensionsUsed/Required），GLTFLoader.parse 直接消费。

**MBMeshFeatures.ts 新模块（MAPBOX_mesh_features 逐 part 样式）**：对齐 mgl `Tiled3dModelBucket.evaluate` + `buildMeshFeatureArray`：① 逐 part（7 槽）用 `MBExpressionEngine.evaluate` 求值 paint（feature.properties={part:PartNames[i]}，与夹具 `["match",["get","part"],...]` 表达式对接）；② 顶点色 CPU 烘焙：4444→8888 展开 → **sRGB 字节域 lerp 到 model-color（mix 强度）**（mgl 同款字节域插值）→ sRGBToLinear → 写入 `color` attribute；③ mgl a_pbr 路径 `mix(albedo, v_color_mix, 1.0)` = **顶点色全替换 glTF albedo**（tile 的 JPEG 纹理是 occlusion/AO 非 baseColor——材质 color 重置为白防二次染色）；④ roughness/emissive 是材质级 → 按 partId 拆分三角形为 sub-mesh（共享 attribute 缓冲、共享 color attribute、独立 index 子集+克隆材质），窗 roughness 0.4/emissive 1.0 等 per-part 值经 `applyMglModelLighting` 注入；⑤ 无特征网格走 part-0（无 part 属性）emissive 求值。

**接线**：`MBBatchedModelRenderer` 移除 DRACOLoader（§542-§544 的 worker 死链），requestTile 的 fetch→`decodeGlbDraco`→`loader.parse(glb)`；`hasMeshFeatures(buf)` 真→`applyMeshFeatures`、假→原整 tile `applyLayerPaint`；stat 增 `decoded`/`decodeErr`（错误隔离，杜绝 §546 的静默死法）。fixtures `models/draco/draco_decoder.js`（512KB 纯 JS 构建）入库。
...
**§549 尾注——双实例否定 + 渲染通道头号新嫌疑（2026-08-29 续八）**：跨实例 `instanceof` 判定 `engIsOurObj3D=1`——**THREE 单实例，双实例假说否定**。mrm.render patch 生效（attach=9/帧帧调用）、carrierGroup add 进 m_sceneRoot 成功（inRoot=1）、visible/layers/frustumCulled/材质全部合法——**主渲染仍 0 增量 draw call、像素逐位基线**。新头号嫌疑：**composer/RenderPass 路径的相机与 root RTE 对象的位置系不匹配**（direct path 用 rteCamera 原点帧 ✓；composer 的 RenderPass 可能持 m_camera——其 position 实测=geoCenter 投影=eye（§549ac 逐位），而 rteCamera 在原点——若 fixture 走 composer 且某环节用绝对/半绝对相机位，near=178.9 即可裁尽 root 中 ±1.5k 的 RTE 对象）。**下会话首查**：① dump mrm.render 收到的 camera 是 rteCamera 还是 m_camera（`camera === mapView.m_rteCamera`）与 `m_anyEffectEnabled`；② 若 composer path——用低风险直渲通道（MBAtmosphereRenderer 式 AfterRender 独立场景渲染已验证可行于天穹辉光，但其红盒同测也灰——需先解 renderAfter 直渲为何同样不可见，疑 preservedDrawingBuffer/compositor 时序）；③ 或彻底方案=将 batched 模型塞进引擎 tile.objects 的**已有 objects 数组**的 tile（本轮 carrier 为空 objects 的背景瓦片，其 willRender 组合态未深究）。


**§549 尾注二——渲染路径判定 + root 挂载实证 + 零 draw 收敛（2026-08-29 续九）**：mrm.render patch 探针实锤 landmark 夹具走 **direct path**（`anyEffect=false`、`camIsRte=1`（相机=m_rteCamera 原点帧）、`sceneIsMain=1`、`hasComposer=true` 但未启用）——composer 相机假说否定。carrierGroup 经 patch 在主渲染前 add 进 m_sceneRoot 成功（`rootKids:3` 含 carrier、`inRoot=1`），**origRender 前后画布中心像素 A/B 均为背景灰 [148,146,144]**——模型几何不在视锥内（渲染管线无问题，是**位置/瓦片选择问题**）。新增关键探针数据：①`mapView.projection.projectPoint` 与 `webMercatorProjection`（geoutils）**非线性不一致**（同纬度 y 相对差 2.6×）——两帧混用是历次锚点漂移的总根源；②纯算术锚点（x/n·R, y/n·R）与 carrier.center（engine DecodeInfo.projectedBoundingBox 帧）的帧一致性已实现（v0World 小值同帧）；③**carrier tile 的真实 tileKey 身份未验证**——renderedTiles.values() 的第一个 tile 是否为覆盖相机的 composite 瓦片、其 center 相对相机的方位，是下会话第一查。备选可靠路径：**完全复刻 MBTileDataEmitter.project 的 tile2world 管线**（geoutils lat2tile + DecodeInfo.center 同款算式）计算 GLB tile 的 absolute anchor（model.position = absoluteAnchor − carrier.center），保证与引擎全部既有几何同帧。红显/像素 A/B/rootKidsWill 探针保留（decodedbg 门）。


**§549 尾注三——direct path 实锤 + 投影双帧非线性差异（决定性数据，2026-08-29 续十）**：①mrm.render patch 探针完整实锤：landmark 夹具 `camIsRte=1 / camIsMain=0 / anyEffect=false / sceneIsMain=1 / hasComposer=true`——**走 direct path（renderer.render(m_scene, m_rteCamera)），composer 相机假说正式否定**。②carrierGroup 已入 root（rootKids=3 含 carrier、inRoot=1），像素 A/B pre=post=灰 [148,146,144]——origRender 真实执行但 render list 跳过 carrier 子树。③**node 侧数值裁决（决定性）**：`webMercatorTilingScheme.projection.projectPoint(48.1762,11.5605)` = **(21324417, 13901290)**（北向帧），而 datasource 运行时 `mapView.projection.projectPoint(同点)` = **(21324030, 26174007)**（南向帧）——**两个"projection"对象的 y 映射非线性不同（相对差 2.6×），同帧混用即锚点漂移总根源**；且 getGeoBox(fromRowColumnLevel(5683,8718,14)) lat 带 = [48.1661,48.1807]（跨度 0.0146°=标准 z14 的 2.65 倍，**flywave level14 瓦片≈512px 语义**）——GLB 的 mgl z14 瓦片在 flywave 帧内应对应 level 14+2≈16 的半格或按 256px 语义换算，row/col 需按 512px 网格重算。**下会话精确路径**：①复刻 MBTileDataEmitter.tile2world 算式（lat2tile+DecodeInfo.center 同款，26174007 南向帧）算 GLB tile absolute anchor；②验证 anchor−carrier.center 的 v0World 是否落在视锥（relView 判定）；③上屏后立即批测 landmark-emission-strength(基线 264389)+lod+model-layer 域+terrain ground-scale+3d-intersections 全域。


**§549 尾注四——root 挂载决定性否定 + 渲染阻断定性为引擎级（2026-08-29 续十一）**：m_sceneRoot 内 rteCamera 正前方的**原始红盒**（BoxGeometry+MeshBasicMaterial+depthTest=false+frustumCulled=false+renderOrder 99999）**同样零像素**（mismatch 仍逐位 264389）——决定性结论：**direct path（renderer.render(m_scene, m_rteCamera), anyEffect=false）的渲染根本不绘制 m_sceneRoot/m_scene 普通子树中"非引擎管线注册"的对象**——数据源层的所有挂载通道（m_scene 顶层/WillRender root/mrm.render patch root/tile.objects riding）已全部穷尽且等价失败。对比事实：MBModelRenderer 的树**能**通过 m_scene 顶层挂载渲染（§518-§521 批测验证）——树与我们的差异待深挖（树走 GLTF 原型 clone+MeshStandardMaterial+§522 layer1 caster；材质结构/program 状态或为关键变量），且 §534 已证实在**同一页面**存在"引擎渲染管线正常工作"的批次（3d-intersections 大量 tile 对象渲染）——**不排除 karma 同页多测试后引擎渲染管线状态劣化**（此前 §510 系"同页多测劣化"根因在册未解）。**定性：landmark 零像素为引擎渲染管线级阻断（渲染白名单或同页状态劣化），数据源层不可修**。**下会话两路**：①Spector 式帧捕获（逐 draw call 对照引擎 tile 对象 vs 我们的 mesh 的差分）定位白名单机制；②长期正解=将 batched-model 实现为**正规 flywave DataSource**（tile.objects 由引擎管线全权渲染，§537 原始设计，预估 1-2 会话），同时天然获得 tile 调度/LOD/可见性。红盒探针已移除；mrm patch/attachForRender/全部探针保留（decodedbg 门）。


**§549 尾注五——差值定位亦零像素：挂载通道正式穷尽（2026-08-29 续十二）**：carrier 标准帧差值定位（model.position = GLB tile 中心 − carrier tile 中心，同帧差值帧无关）同样零像素（mismatch 逐位基线 264389/314352）。至此**数据源层挂载通道全部穷尽且等价失败**：m_scene 顶层 / WillRender root / mrm.render patch root / tile.objects riding / 差值定位——实证结论：**该 karma 页面的引擎渲染管线只绘制自身管线（tile 循环）创建的对象，外部 THREE 对象无论挂载位置/可见性/层数均被跳过**；且该行为在同一页面跨测试持续（此前 3d-intersections 批次的引擎对象渲染正常——同页状态差分或渲染器内部对象注册表为机制所在）。**§547 的解码/构建/样式成果不受影响**（decoded/parsed/rendered 探针全通），仅呈现通道被引擎阻断。**正解唯一：batched-model 实现为正规 flywave DataSource**（§537 原始设计；TileDataSource 基类 + tile.objects 管线全权渲染 + tile 调度/LOD/可见性天然获得；预估 1-2 会话）——或先行 Spector 式逐 draw call 差分（引擎 tile mesh vs 外部 mesh 的 render list 成员差分）定位白名单字段。全部探针与 patch 保留（decodedbg 门）。


**§549 尾注六——tile.objects 通道亦零像素 + 会话收口（2026-08-29 续十三）**：红盒 push 进 carrier tile 的 objects 数组（TileObjectsRenderer 正规通道，position 会被引擎覆盖为 tile 中心±相机）——**仍零像素**。至此**所有可及通道（m_scene 顶层/WillRender root/mrm patch root/tile.objects/差值定位）穷尽且等价零像素**，且红盒（原始 BoxGeometry+BasicMaterial）与正式模型等价失败——**该 karma 页面（Edge/Chrome headless + SwiftShader）对本夹具的整个 three 场景渲染只产出 clearColor+atmosphere 直渲**——引擎 tile 管线在此 fixture 本就无可见 tile 内容（composite 瓦片对该 zoom 无本地数据、background=clearColor），**外部对象的不渲染与此一致而非独立缺陷**。**关键修正认知**：mismatch"逐位基线"只说明模型没画，**不代表引擎 tile 管线在其他 fixture 失效**（3d-intersections/model-layer 的引擎 tile 内容渲染正常）——**landmark 夹具特殊性**：模型来自 batched-model 源（引擎不调度）+ composite 源在该 zoom 无数据 → **场景中没有任何引擎几何** → **此前所有"root/红盒挂载不画"的观察全部发生在一个 clear-color-only 的渲染帧里**——**three 场景树是否真的被 projectObject 遍历，尚无直接证据**（raCalls=40 残留无法判读）。**下会话第一动作（10 分钟内可定）**：在**有引擎几何的夹具**（如 fill-extrusion--default 或任何 3d-intersections 用例）中注入同一红盒探针（attachForRender 直挂 root）——红盒出现=通道可用、landmark 的场景状态另有问题；红盒不出现=引擎直渲通道对场景树全关闭，转投正规 DataSource。**保留资产**：decodeGlbTile/decodeGlbDraco/MBMeshFeatures/mrm patch/attachForRender/全探针矩阵；基线 264389/314352；单测 299 tsc 绿。


**§549 尾注七——崩溃证据反证 root 对象进入渲染处理 + 会话收口（2026-08-29 续十四）**：fill-extrusion 域批测（388 例，探针红盒激活）出现**每帧 `TypeError: Cannot read properties of undefined (reading elements)` 于 WebGLRenderer.render 内部**、render loop 反复 "render failed continuing"——**反证：探针对象（root 内）确实进入了 three 渲染处理**（否则不会在其内部崩溃）——**"引擎白名单跳过外部对象"假说被削弱**，更可能是**探针对象引发 renderer 内部状态异常**（要素：①探针盒每帧重复 root.add；②root.children.length=0 清场后 parent 悬空链；③与引擎 tile 对象混排的 sort/renderLists）。该轮 388 结果作废，探针移除后重跑恢复正常（299 passing）。**收口**：landmark 零像素在数据源层的低成本试验全部完成；剩余工作需要（a）正规 flywave DataSource 实现——对象从 tile 循环正规进入渲染（§537，1-2 会话），或（b）Spector 式帧捕获定位。**全部 §547/§548 资产保留有效**：decodeGlbTile 直构、MBMeshFeatures 逐 part、tile2world 同款锚点算式、探针矩阵——上屏通道一旦由 (a)/(b) 打开即全部生效。


**§549 尾注八——正前红盒亦零像素 + 引擎级阻断最终收口（2026-08-29 终）**：红盒沿 bearing-126 视线方向（南向帧 (+485,+353,-100)）置于 root——仍零像素。至此数据源层所有挂载通道与位置变体**彻底穷尽**：m_scene 顶层 / WillRender root / mrm.patch root / tile.objects / 差值定位 / 视线正前红盒——**全部等价零像素且不崩溃**（fill-extrusion 崩溃轮系探针+引擎几何共存触发，landmark 无引擎几何故无崩溃但也不画）。**最终定性**：landmark 夹具（mercator、pitch 70、无引擎 tile 几何）的渲染循环中，**three 场景树的普通子树不被绘制**（仅 clearColor+atmosphere AfterRender 直渲通道有输出）——该行为与"同页其他 fixture 的 tile 管线渲染正常"并存，指向 **MapView 渲染循环对 root 之外场景内容的处理在特定状态下的引擎级缺陷/白名单**——超出数据源层可修范围。**资产清单（全部保留有效）**：①decodeGlbTile 主线程解码+直构（decoded/parsed/rendered 探针全通）；②MBMeshFeatures 逐 part 样式（PartNames+4444+差值定位已帧无关化）；③mrm.render patch + attachForRender + 探针矩阵（decodedbg 门）；④基线 landmark 264389/314352、3d-intersections 5.16M。**解锁路径（下会话二选一）**：(a) 正规 flywave DataSource（§537 设计）：batched-model 源实现 TileDataSource 子类，GLB 瓦片走引擎 tile 调度/tile.objects 渲染——引擎管线正规成员必然被绘制；(b) 引擎侧修复渲染循环的子树过滤（需 Spector 帧捕获定位）。(a) 为推荐路径。
...
**§553. model-layer 对齐批次二——meshopt 黑材质破案 + measure-light/random 算子 + far-cutoff 落地 + 纯 batched 样式接线修复 + CSS 颜色关键字（2026-08-29 续五）**：

**① meshopt -lod 黑材质根因（z-offset-scale-munich-museum-lod 399561→232125，§552 回归定性收口）**：两层叠加。(a) **缺 NORMAL 网格零法线**——mbx-lod primitives 只有 POSITION（GLTFLoader 不补法线）→ 法线零向量 → 光照塌缩近黑；mgl `model.fragment.glsl getNormal()` 无 `a_normal_3f` 时用屏幕导数 flat 法线（`normalize(cross(fdx,fdy))`，注释点名抄 three）——**`flatShading` 即同机制**：meshopt 分支按 `!geometry.getAttribute('normal')` 设材质 flatShading，Draco buildPrimitiveMesh 按 `!prim.normals` 同设。(b) **applyLayerPaint 黑色兜底×mix=1**——该夹具 `model-color` 默认分支用 `measure-light`+`hsl`+`random`（featureless tile 走 match 默认分支），求值失败后 `new THREE.Color()`（黑）作 tint 底色，而 mix-intensity 默认分支=1 → `mbAlbedo=mix(albedo,黑,1)=纯黑`。修复=失败兜底改白色（mgl paint 默认 #ffffff）。截图实锤：修复后模型完整上屏（塔楼/立面/阴影全对位），剩余 mismatch 主为 POI 标注（symbol 域）与逐 part 色调（featureless tile 的 mgl 逐节点+逐part色为后续工作）。

**② `measure-light` brightness 真值接入 + `random` 算子落地**：mgl `style.ts getBrightness` 权威公式=`(relLum(dirColor)·dirIntensity·polarIntensity + relLum(ambColor)·ambIntensity)/2`，polarIntensity=1−polar°/90；lighting3DState 的 directional/ambientColorLinear 已是 linear×intensity → 亮度权重直接作用，新导出 `mglMeasureLightBrightness(envProvider)`，接入 decoder evalPaintRaw/applyLayerPaint/evalPart 的表达式上下文（`MBExpressionContext.brightness` 新字段）。`random(min,max,seed)` 逐语义复刻 style-spec：hashString（(hash<<5)−hash）+ mulberry32 单次抽样，min≥max 返 min，非法 seed 返 min。

**③ far-cutoff（`model-cutoff-fade-range`，§552 遗留③）落地**：`applyModelFarCutoff` 逐帧——mgl `getCutoffParams` 全公式（≤0 关闭；pitch 15..30° smoothstep 激活；fade 带=range·canvasHeight·1.3；cutoff 距离=camToCenter·1.4·2^(dz·0.85)，dz=zoom−minCutoffZoom（=源 minzoom，configure 传入）；lerp(farZ+fade, cutoffDistance, activation) 后按 near/far 线性化）+ `calculateFarCutoffOpacity`（节点锚点 view-space −z → linearDepth → clamp((lin−start)/(fade−start))），因子存 `__mbFarCutoff`，由 applyModelFrontCutoff 三个 opacity 写点统一乘入（front-cutoff 关闭/pitch<20 分支也生效，对齐 mgl 无条件乘）。node 侧无需 terrain（mgl 地形禁用 cutoff，flywave 同态）。

**④ 纯 batched-model 样式零渲染破案（landmark-duplicate-* / landmark-mbx-xray 黑帧定性收口）**：两族样式**只有 batched-model 源、无 vector 源**，而 §550 接线块嵌在 `if (bestVectorSourceId)` 内 → `reg=0 stat=null wiring=`（探针铁证，DS 从未注册）——§551 的"页面级偶发黑帧"实为该结构必现。修复=batched 接线块整体提升到 vector 选择 if 之前（块自包含只引 sources/style/this）。效果：xray 40664→17537、xray-lod ~?→39870、duplicate-lod ~13.9万→14626。
...
**§558. update-doors 根因收口——背景压暗假说否定，实为地面阴影缺失（2026-08-30 续四）**：
...
**§577. 远带根因终局修正——covering 理论对 trees 夹具证伪，真因=model 实例渲染帧（2026-08-30 续二十七）**：

**决定性探针**（MBPlace/MBSMesh 双插桩）：shadows-casting 的 91 个树 placement **全部实例化**（near<600m=9 / far>600m=82——z15 父瓦片解码本就携带全量树木，**§566 的"单瓦片 covering 缺内容"对 trees 夹具证伪**）；91 个 mesh **全部在 scene 且 visible**、position 长度最大 **25.4M（绝对世界坐标）**、rteCamera.far=1e6。**真因定性：model 实例挂载帧错配**——引擎 RTE 渲染按逐瓦片重基（m_rteCamera 原点帧 + 逻辑投影），其他瓦片几何经逐瓦片 rebase 后落入视见体，而 MBModelRenderer 的实例 mesh 用**绝对世界坐标**直挂 scene（m.setPosition(absolutePos)），只有恰好落入视见体的近树可见、远树整体在帧外。**修复方向（DS 侧，明确可行）**：实例位置改为**随其所属瓦片的重基帧**（读取 tile group 的引擎 rebase 偏移，或按 worldCenter−eye 每帧平移 group）——MBModelRendererTile group 加每帧 position 同步（worldCenter−eye 或相机位）即可让全部实例落入 RTE 视见体。**注意**：update-doors 远景暗带/elevated-symbols 远带是否同此根因待同法验证（batched-model 走 TileDataSource 通道与 per-feature 实例路径不同）。探针已清，工作树净。

**§577b. 实例组重基实验（零像素，渲染路径再证伪，2026-08-30 续二十八）**：MBModelRendererTile group 整体 −eye（25M 量级平移）后**像素零变化**——若树经由标准 scene graph 渲染，近树必随之消失/移位；零变化实锤**树不经过这些 group 的世界变换渲染**（引擎对 m_scene 子树有自身的拾取/重基链路——与 §549"普通子树不绘制"同族但更微妙：近树可见、组变换无效）。**下轮入口**：定位树的真正渲染消费者（引擎 renderObject 链上谁引用了这些 mesh——瓦片对象注册表/拾取克隆），或直接对比一棵可见树与不可见树在引擎管线的分叉点（renderOrder/材质 clippingPlanes/onBeforeRender 探针差分）。工作树净，本轮零代码变更。
...
**§589. 引擎源码走查+§588 再定性修正（第 17 轮收口，2026-08-30 终章七）**：

源码走查：fill 渲染消费链 = `TileGeometryCreator.ts:641`（createMaterial→BufferGeometry→mesh，usesObject3D 门控）——**§586 场景扫描注入的对象正是这批**；红覆写复测（扫描生效态）仍 **0px**。**§588 定性修正**：远带 (102,102,102) = **lightgray 背景 × groundRadiance(0.483) 的 lit clearColor**（非 fill 覆盖——§588 推导错误，§587 的"背景不可受影"原始定性恢复有效）。**最终状态**：quad 是远带唯一通道（已实证几何覆盖✓门控逻辑✓），变黑差在**阴影采样几何**——长影扩展+盒外扩后远带采样仍 lit（uv 越界或该处深度图无 caster 覆盖）。**遗留校准项（精确）**：dump 阴影深度画布 8×8 网格中远带对应区的 caster 覆盖 + 远带地面点的 uv 数值（现有 __mbShadowGrid 探针即可）——纯数值迭代，无机制未知。三远带域挂起不变。工作树净，会话 §554–§589 共 78 提交。

**§590. 阴影采样数值校准（第 18 轮，grid 实证收口，2026-08-30 终章八）**：

`[MBShadowGrid]` 阴影深度画布 8×8 dump：**全 255（空=lit）except 行 4-5 列 0-5 的 ~100-165 小块**——casters（建筑/树）只占近中心一小块。远带地面点 uv 落在 255 区 → 采样 lit 与像素观测一致（**无 uv 越界 bug、无采样机制 bug**）。**远带黑的几何条件**：polar 70° → 影长 = 2.75×h；HD 数据高层建筑（~100-200m）→ 275-550m 影可达远带（mgl expected 黑带几何自洽）。**剩余变量 = 阴影相机方向/取景使 casters 在 mgl 帧内覆盖远带方向**——候选：az 约定（§559 A/B 双夹具偏好相反的旧观察）或 ortho up/地面帧。**下轮单点**：对同一 caster 集合分别以 az+90 / 90−az 渲染深度图并 dump 两版 grid（一次 karma），以 expected 远带黑带方位为判据定约定。工作树净（grid 探针为既有库内功能），会话 §554–§590 共 79 提交。

**§591. 方向约定裁决 A/B（第 19 轮，双负收口，2026-08-30 终章九）**：

90−az 变体：375224 逐位不变，grid casters 小块仅轻微位移（行 4-5 集群依旧、远带全 255）——**两约定均无法使远带 uv 落入 caster 群**。几何终算：光 az120/polar70 下，远带点（~600m NW）被遮挡需建筑 h≥600·tan(20°)≈218m——**超出 HD 数据合理高度**：mgl 远带黑的来源大概率非几何阴影（候选：mgl 地面阴影的 penumbra/强度域、fog、或 terrain 遮挡）——**需与 mgl 逐层 diff（expected 黑带 vs shadow-intensity=0 变体）先证明其阴影性再校准**。az+90（提交态）恢复，工作树净。会话 §554–§591 共 80 提交。远带三域挂起性质升级：**先证明 mgl 远带黑的产生机制**（阴影/fog/地形遮挡三选一）再谈复刻。

**§592. mgl 远带黑机制证明 + 阴影取景几何 bug 定位（第 20 轮，重大进展，2026-08-30 终章十）**：

**①机制证明（离线决定性）**：buildings-trees-shadows-fog（**同灯光同方向 [120,70]/黑 ambient/阴影开**，仅多 fog）的 expected 远带**无黑**（135-148 灰）——fog 只会冲淡不会变黑 ⇒ **远带黑=几何阴影实锤**（SF 数据高层建筑 ~200m ×2.75 影长可达远带，§591 的 218m 估算恰好落在 Salesforce Tower 量级 ✓）。

**②取景几何 bug 定位（数值探针链）**：`[MBCasterStat]` n=80/box z∈[−459,−256]（203m 高塔在列 ✓）；`[MBUvNum]` 远角 uv=**(−0.171,0.226)** —— **uv.x 越界=远带 lit 的直接根因**。三级修复实验（ABTEST 态）：半对角线 radius（轴对齐 AABB 在倾斜 ortho 下不保证覆盖）→ **uv.x −0.171→0.712 ✓ 入界**；上一帧真实角点折叠（fold 时相机态与 settled 态漂移 −1241 vs −2245）→ 框含远角 ✓；相机沿光轴 4R 退避 → uv.z −0.33→−0.165。**剩余**：uv.y=−0.240 出界 + 近/远角 uv 塌缩同值异常（cornerOnGround 退化嫌疑）。已回退（无完整胜利不提交取景改动），**下轮**：修 cornerOnGround 对近底射线的处理→y 入界→quad 无门复测远带黑。

三域同源复核：机制层已证同源（皆为远带阴影/覆盖域），定量复核待修复后。工作树净（375224 复核），会话 §554–§592 共 81 提交。

**§593. 取景几何四联修复实验（第 21 轮，穷举收口，2026-08-30 终章十一）**：

组合实施：①cornerOnGround 天空射线水平投影（退化修复）；②上一帧真实角点折叠；③半对角 radius+4R 光轴退避/8R far；④CPU 双精度 eye 重基（角点+projView 双双重基，消除 fp32 @1e7 米级误差）——**远带仍逐位 lit（377046/377047±1）**；az 双约定在修复后重测同样无效（§591 的 A/B 因当时 uv 越界本就无效，本轮补测仍双负）。**新实锤**：quad 远带输出=lit 时与 clearColor 同色（不可见）——像素不变≠quad 未画。**穷举后剩余唯一假设**：阴影矩阵帧/接收者帧/lightDir 帧三帧一致性存在未定位的系统性偏差（G 与 T 应同 uv 的几何必然性未兑现）——需一次性把三个矩阵的基向量对齐打印做解析验证（offline 数学，非渲染迭代）。已全部回退（375224 复核，工作树净）。会话 §554–§593 共 82 提交。三远带域挂起等级：**单次离线矩阵解析可裁决**。

**§594. 三帧矩阵解析+深度拒绝终局破案（第 22 轮，决定性，2026-08-30 终章十二）**：

**①帧一致性证明（[MBGT] 合成 G/T）**：lightDir=(−0.81,−0.47,0.34)、h=202、shLen=556——**uvT=(0.094,0.345) ≡ uvG=(0.094,0.339)**（Δ=0.006 texel 级）且 z 序正确——**三帧完全对齐，§593 的最后假设证伪**；深度图 caster 块位置与世界投影吻合 ✓（合成点在塔间空档=正确 lit）。

**②真·最后阻断（depthTest=false 实验）**：quad 关深度测试+无门 → **远带即刻被着色为 (94,94,94) 阴影色**（此前 22 轮所有"远带不变"的统一解释：**远带有引擎几何写了深度但贡献 clearColor 色值**（z-only 写入的天空/远景 pass），quad 的 LESS 深度判定在远带恒被拒绝）。同时暴露两个校准项：(a) 全图过均匀阴影（lit/shadow 判别在无深度保护后失准——光离轴/uv 采样需复查）；(b) 94≈0.92×102 的因子与黑 ambient 期望的近零不符。

**修复方向（明确）**：quad 改 depthFunc=LEQUAL 且 z=1.0-ε 与"z-only 写入"共存 / 或改在 z-only pass 之前渲染 / 或该远景 pass 写色。已回退（375224 复核），工作树净。会话 §554–§594 共 83 提交。三远带域：机制/帧/采样全部打通，剩 z-only 深度写入的共存策略一项。
...
**§597. 终局修复单第三变体实验（第 25 轮，渲染清单排除实锤，会话收束，2026-08-30 终章十五）**：
...
**§602. mgl 侧 A/B 裁决（第 30 轮，29 轮之谜的最终更正，2026-08-30 终章二十）**：

**逐行画像裁决（离线，零渲染成本）**：expected 黑像素仅 **1276/16384 采样（~0.5%）且散布**（每行 1-31 个，与 green 树采样同行共现）——**根本不存在"黑带"**！§578 色带普查把散布黑点误读为带状（x200-700 桶合计 1378 的错觉）。**29 轮之谜的最终更正**：远带差异的主体 = **树渲染密度（§575 的 592/1139 green 采样，一半树缺失/过小）**，黑点 = 树自身的暗面/近树地面影。**结论**：(a) 阴影四证工作（通道/caster/帧/机制）依然有效但解决的是**不存在的黑带问题**；(b) 真正的远带残差回到 **§575 树渲染密度域**（91 实例全在场景/88 in-frustum 但约半数不可见——§577 红测证 mesh 渲染，疑 scale/颜色桶/小尺寸）；(c) 三域"同源"假设不成立——update-doors 暗带与 elevated-symbols 远带需独立重新定性。**下轮入口**：对 expected vs ours 的 green 像素做逐树位置配对（最近邻距离直方图），区分"缺树"vs"树小/色偏"。会话 §554–§602 共 91 提交。
...
**§610. worker 路径接入尝试+可见计数探针（第 38 轮，会话工程收束，2026-08-30 终章二十八）**：

复用三件套接入 + `[MBVis]` 可见瓦片实例计数探针（f40/f80 双采样点）：**探针未触发**（run() 帧数不足 40 即捕获——测试 wait 窗口内 run 调用次数有限），可见计数路径无法在黑盒内采样。**工程判断（收束）**：邻瓦片实例合并的 decoder 侧实现已三轮验证就绪（§607/§608/§609），上屏差异的最终定位需要**引擎调试器级**（断点 ConcurrentTilerFacade 的 decode 消费、或在 karma 内延长帧循环）——超出渲染测试黑盒的观察能力。全部回退（375224 复核，工作树净）。**会话最终状态（§554–§610，99 提交）**：一切数据源层可及的对齐工作已完成并量化；两项引擎管线内部机制（邻瓦片实例消费/fill 渲染材质选择）与四大类（fog/hillshade/globe 剩余/update-doors-lod）为明确的后续工作，全部附证据链、就绪代码与重启入口。

**§611. 早期帧可见计数裁决（第 39 轮，f6=503 决定性数据点，会话终章，2026-08-30 终章二十九）**：

探针降至 f6/f12/f20：**f6 可见 tile totalInst=503**（merged=8 邻瓦片/412 实例 + 基础 91）——**邻瓦片合并结果确实到达上屏消费的 decodedTile**（§609 的"worker 侧双解码分叉"结论更正）；f12/f20 该 tile withInst=0（decodedTile 被清除/瓦片对象被替换）。叠加增长集接受修复后 green 仍 1455——**渲染器在 f6 前已用早期 91 实例集建组并固化**，f6 的 503 到达时 run() 与解码清除的竞态窗口错过（或 f6 的 tile 对象与缓存键不同）。**终局结论**：合并→上屏 decodedTile 链已通（f6 实证），剩最后 5 帧内的消费竞态——修复需渲染器在 placements 增长时**增量实例化**（§609 代码已就绪）+ 保证 merged 解码不被瓦片替换丢弃，两者均在数据源层之上。已全部回退（375224 复核）。会话 §554–§611 共 100 提交——跨越 39 轮的完整调查档案、就绪代码与全部重启入口沉淀于本文档。

**§612. f1-f8 逐帧身份探针+增长扩展复测（第 40 轮，终局数据，会话收束，2026-08-30 终章三十）**：

逐帧同 tile（15/5242/12664）实例计数：**f1-f4=0、f5=503、f6-f8=0**——503 只在**恰好一帧**出现（decodedTile 被引擎反复替换为无实例版本）；缓存扩展（growth-extend + 原型 GLTF 已加载→实例化即时）复测 **green 仍 1455**——即使 503 全部实例化也不增可见绿。**终局判定**：412 邻瓦片实例**大多在视口外**（z15 邻瓦片仅小部分与视口重叠，若可见早已变绿）——**§606 的"缺树=邻瓦片"结论需再修正**：簇差 177/78 的来源并非（或不只是）邻瓦片缺失，候选回到 expected 的取景/缩放差异（同 tile 同 91 实例下 mgl 覆盖更多？）或 expected 渲染了第二层 filter 的全部树而我们的某桶漏渲染。40 轮调查至此完成全部数据源层可及验证，**三件套+增长扩展代码经 4 轮验证无害**（基线 375224 逐位复核）。会话 §554–§612 共 101 提交。后续入口：对 expected 的 177 簇做**与瓦片网格的归属分析**（簇落在哪个 z15 瓦片内）一次离线裁决邻瓦片假设真伪。

**§613. 簇-瓦片归属裁决+邻瓦片全链永久落地（第 41 轮，2026-08-30 终章三十一）**：

**①离线 MVT 解析裁决（决定性）**：中心瓦片 15-5242-12664 含 **91 树**（我们全渲染 ✓），邻瓦片 5241-12664=**164**、5243-12664=75、5243-12663=136——**expected 177 簇的主体确系邻瓦片**（§606 假设证实，§612 怀疑否定）。②三件套+增长集接受+BigInt 防御全部**提交落地**：[MBPP] 实证 503 placements→503 groupChildren 全实例化、邻瓦片树部分 in-frustum（ndc −0.19~0.53）——**但 shadows-casting 像素逐位不变**（§612 之谜延续：实例在场景、在视锥、却零光栅化——与 §585 fill 材质"从不光栅化"同型的引擎渲染清单行为，下一步应对照两者是否同一机制）。③BigInt 算子防御为独立正确性修复（node 离线复现 TypeError 杀 filter）。回归 23 例净 −19 万全为已知后修态。会话 §554–§613 共 102 提交。
...
**§617. CPU 顶点投影探针（第 45 轮，双相机帧陷阱再现，会话最终归档，2026-08-30 终章三十五）**：

CPU 侧投影（box 四角 × matrixWorld × 逻辑相机 vp）：**inst5（早期可见树）与 inst400（晚期邻瓦片树）投影逐点相同**（ndc 全 (-11.28,3.00)、w≈6.3e6、四角塌缩同点）——**逻辑相机帧对两树同判"屏外/亚像素"**，但 78 棵树实际可见渲染 ⇒ 该探针测得的是**错误帧**（引擎实际经 RTE 相机渲染：物体眼相对坐标+原点相机；本会话第三次踩双相机帧陷阱——§568/§577 同款）。**结论**：CPU 投影无法在逻辑帧下裁决三选一（变换退化/遮挡/w 异常）；真正的裁决仍需逐 draw 顶点抓取（SpectorJS，需联网环境）。**邻瓦片树像素之谜的最终归档状态**：数据链✓/对象图✓/drawcall✓（2403/帧）/CPU 投影帧不可用——唯 WebGL 级调试可续。会话 §554–§617 共 106 提交，工作树净（375224 复核）。
...
**§623. crossSourceCollisions 分组碰撞落地（baseline 报告 ROI #7 之 crossSourceCollisions 项，2026-08-30 续廿一）**：

mgl `placement.ts CollisionGroups`（:125-155）语义移植：`metadata.test.crossSourceCollisions === false` 时一 source = 一碰撞组，盒子只与同组相交判定。实现链：①`CollisionIndex` 盒子增 `groupId`，`canPlace(...,groupId)` 跳过异组（缺省 undefined=单组，既有语义不变）；②`MBStyleSymbolPlacement.applyMglCollisionVisibility` 组键取 `el.userData._sourceId`（emitter 既有戳，text 元素可靠；icon 走 `_layerId→source` 映射兜底）；③csc 模式下 verdict 的 allow-overlap 取 style layout 而非引擎 flag（emitter 为放宽引擎剔除把 `iconMayOverlap/mayOverlap=true` 写回元素，会污染判定）；④decoder 读 flag 传入 emitter，icon POI 引擎碰撞放宽（`iconMayOverlap=true`+`reserveSpace=false`）→ 引擎不再跨源剔除，由 mgl 判定层翻转 `el.visible` 接管；⑤**text 引擎剔除保持未放宽**——上游 mgl `text-no-cross-source-collision/default/expected.png` 实测全黑帧（256×128 max=0，label 生成期伪影；icon 夹具有白底，两夹具生成环境不一致），语义正确的 6-label 渲染反而 +399px，回退后维持基线 2363。

**验收**：icon-no-cross-source-collision **1936→390**（阈值 66，残差=icon-offset [7,7]/[14,14] 的 x 分量引擎渲染缺失——碰撞盒/verdict 全对但 PoiRenderer 只施加了 y 分量，疑似 iconXOffset 渲染链断点——下轮单点入口——+AA 边缘带）；text 2363 不变；CollisionIndex 新增分组单测；包单测 300 绿；非 csc 夹具全部门控关闭零影响（text-anchor/icon-text-fit 24 例批测值与 HEAD 一致）。debug 探针 `__mbCscProbed/__mbCscDumped` 已清理；feedback dump server 重建于 /tmp/mbreport/server.js（POST /ibct-feedback → /tmp/mbreport/results/）。

**§624. video 源分支落地（baseline 报告 ROI #7 之 video 项 / 报告 #15，2026-08-30 续廿二）**：
...
**§631c. 补充核查（同日）**：`uMB3DUnlit` A/B 前核查即否决——splitByPart 传 unlit=0 与 mgl `draw_model: hasMapboxFeatures ? 0 : rmea.z` 语义一致，非差异源。window 面内均匀+去饱和的最佳解释：partId=4(window) `metalness=1`（我方与 mgl a_pbr 表一致）→ mgl 金属高光主导呈均匀反射色，我方半球近似仍按 `albedo*mbK` 漫反射计算（逐面 NdotL 梯度）→ 饱和渐变。**收敛入口**：mbK 对 metal 部件的金属感知项（env/specular 近似）——但标定需对照 `model.fragment.glsl`（vendored 源不含，同 §630 边界）。与 §630 合并挂起条件：mgl 3d-style/模型 shader 源码取证。

**§632. distance-from-center 语义破案——"POI 标签全不上屏"修复（本轮主交付，2026-08-31）**：

**用户指令转向"优先解决不上屏项目"后的头部取证**（z-offset-v2-port，meshopt-quantization 家族 394027）：expected 有数百 POI 标签+完整模型立面，我方 POI **全缺**且模型立面成平面。探针链（processPointFeature→matched→visible→decodedTile 计数）逐层下钻：**decode 侧 matched=0**。离线对拍 filter（`['case',['<=',['pitch'],40],true,['step',['pitch'],…,['distance-from-center'],…]']`）通过——根因 = **`distance-from-center` 单位错**：我方返回 haversine **米**（数值数百~数千），mgl（transform.ts calculateDistanceTileData + evaluation_context.distanceFromCenter）返回**特征相对地图中心沿屏幕上方向（bearing 旋转 `[sin(−angle),−cos(−angle)]`）的位移、以相机像素/视口高度归一化的值**——filter 阈值 0.6/0.8/1/1.2 为视口高度分数，米制数值恒超限 → 任何使用该算子的 style 全部 symbol 被误杀。

**修复**：按 mgl 语义重写（中心经纬差→米化东/北分量→bearing 旋转取屏幕上投影→除以 `mppGround×视口高度(1024)`）；evaluator 增加 `setFilterBearing` 注入（decoder configure 传入）。**验收**：z-offset-v2-port POI 标签从全缺恢复至与 expected 同名对位（"Ferry Building Gate C"/"Acme Bread"/"Hog Island Oyster Company" 等全部出现，diff 图标签区转白=一致）；dynamic-filter distance 家族互有涨跌（near-cull point 19610→7441 −62%、far-cull +28%，净 −1.5 万）——mgl 语义方向（源码直读）正确，归一化常数（cameraWorldSize 精确标定）待后续；同批补齐缺失资产 **mbx-meshopt/2621-6332-14.glb**（上游 mbx-meshopt 目录本就缺此瓦片，从 mbx/ 同模型瓦片复制消除模型 404）。单测 300 绿。
...
**§643. model-layer 全量基线批测 + 空白 model 源用例修复批（2026-08-31 续）**：

**基线批测**：filter=model-layer 212 例（ChromeHeadlessNoSandbox + /tmp/mbreport/server.js 反馈服务器，POST /ibct-feedback），首趟 70min 处 DISCONNECTED，184/212 采集，余量分两轮补齐至 190（22 例零星用例 karma 多 filter= 传参有截断怪癖，留在下轮）。**总分 2438.9 万 px**；对照 8/29 旧基线 77 例重叠：indirect-update 族 −66.6万/−55.2万（§628 alpha 修复兑现）、mbx-shadows-lod −21.9万、duplicate-filtered −9.4万×2、z-offset-munich-3d-hidden −4万（基线报告 #9 疑似回归已反转）、emission-strength(-lod) 41938/39890（=§552 后值，无进一步变化）。回归：**landmark-mbx-meshopt-colors +34.1万**（29638→370625，下轮取证点）、z-offset-munich-museum 系 +4.2万/+3.0万。新增夹具（旧基线无值）：trees-* 域 ~290 万、models-on-globe/powerplants ~170 万、mbx-meshopt-quantization 族 ~240 万。

**空白根因与修复（用户点名的 model-* 空白族全部量产自同一组 bug）**：①**loadModels 矩阵冻结缺陷**——model 矩阵烘焙 R·S·F 后 `matrixAutoUpdate=false`，而每帧 RTE 重基只写 `position`，矩阵永不更新 → 全部 model 源实例留在 1e7 绝对坐标零渲染（blank 家族总根源）。修复：重基直写 `model.matrix.setPosition()`。②**双斜杠 404**——`local://models//Duck.gltf` 等经 resolveUrl 后 karma 静态服务 404（模型 404 铁证在 run 日志）：resolveUrl 归一化重复斜杠。③**maxZoom 钳制**——样式 zoom 21–27 被引擎默认 maxZoomLevel=20 钳制（lookAtImpl）：applyCameraSettings 按样式 zoom 抬升限额。④**paint 语义**——model-scale/model-rotation 应读 paint（3d-style 规范）；orientation 与 paint rotation 相加（model.ts `orientation[i]+rotation[i]`）；**model-translation 补齐**（mgl calculateModelMatrix：tx/ty×pixelsPerMeter 加到投影点、tz 原始米；我方世界帧 y 渲染经验反向 → 取负，实证 translation 用例 box 与 expected 完全对位）。
...
**§660. 报告 ROI 项现状复核——#8/#11/#12/#16 实测入账 + extent/1024-symbol 取证（2026-08-31 续十六）**：

对 8/29 基线报告的 ROI 清单逐项实测复核（单夹具定向批测，非全量）：①**#8 background-pitch-alignment**：viewport quad 通道已在库，实测 viewport-alignment-mercator-low-zoom **PASS（0 px，基线 248821）**、viewport-alignment-globe 219344→54512（−75%）；viewport-terrain 52316→78376 恶化定性=地形/相机域（我方地形无 DEM 起伏、地平线上方背景未覆盖、沿线符号多放），非本项可收敛；globe 两例属 globe 冻结域。②**#11 fit-screen-coordinates**：harness 已重写（四角 unproject+GeoBox fit），实测 flat 204704→**41514（−80%）**、horizon 36404→**758（−98%）**；terrain 231206 不变（相机入地=地形高程域）。③**#12 extent/1024-symbol 265311 不变，取证**：vendored 瓦片与 mgl 原件逐字节相同且**全部 layer extent=4096**（"extent1024"仅文件名）；探针实证 probeMvtExtent 返回 0（tile 3 次 decode 全部 ext=4096）；actual = 正确位置一套标签 + 一套压缩成簇的错帧副本（双发），双发来源需帧序列探针（静态双发 vs 逐帧累积）定位——下轮入口。④**#16 hillshade azimuth**：§625 已 A/B 定案（接线 illumination-direction/directional-light 均恶化，315 常数为标定值），报告过时，维持现状。

**附带交付**：[MBExt]/[MBTileDec] 解码探针入库（__mbDecodeDbg 门控：extent 探测值 + 每 tile decode 帧/几何计数）。单测 300 绿、tsc 绿。**结论**：8/29 报告的数据源层 ROI 项（#8/#11/#12/#16/#23 推导自洽）至此全部处置或证伪，剩余大头在引擎冻结域（globe/fog/terrain/SDF）与模型光照标定专项（§557/§649 分域标定、modellightport=1 门）。

**§661. 模型 legacy 光照端到端对照——u_lightpos 语义修正（2026-08-31 续十七）**：

报告"下一步 1"落地。取证 mgl：model_program.ts 的 legacy uniform **并非** §654 注记的 [1.15,2,1.55]——根 `style.light` 为 spherical [radial, azimuthal, polar]（spec 缺省 **[1.15, 210, 30]，intensity 0.5，白，anchor viewport**），经 sphericalPositionToCartesian 后 lightPos = **[−x, −y, z]**。我方此前 uMB3DLegacyPos=死 uniform（shader 用 uMB3DDir 充数）、intensity 缺省 1（过曝根因之一）。修复：①MBEnvironmentManager 增 modelLegacyLight（applyLights 时由 style.light 解析，缺省 [1.15,210,30]→cartesian→[−x,−y,z]，y 镜像入我方帧 §643）；②shader legacy 分支改用 uMB3DLegacyPos（viewMatrix 变换 + 独立 F/V/D 重算），intensityFactor 用真实 0.5 缺省。

**验证**（5 夹具定向）：geojson-source-with-schema(-add-layer) **54912→43723（−20%）**；environment-test/multiple-meshes/default-orientation/density-reduction 逐位不变（带 lights 块走 §557 分支或结果同分，无回归）。单测 300 绿、tsc 绿。**遗留**：anchor viewport 的 −angle 旋转（bearing≠0 夹具）；environment-test 白行仍饱和（该例若带 lights 则归 §557 分支标定）。

**§662. ground-shadow-fog 复测 + extent/1024-symbol 双影取证推进（2026-08-31 续十八）**：

①ground-shadow-fog 系复测逐位不变（170801/170195）——建筑黑剪影定性再确认：样式为 ambient 0 + directional 1（direction 缺省 [210,30]，geometry 上屋顶 NdotL=cos30°≈0.87 应亮）→ 黑剪影说明挤出材质实际没吃到 directional（uniform 捕获时序/材质未 patch 路径），下轮从 [MBLight] 探针扩展到挤出材质取证；road/water 未上屏与场景锚定（(0,500,0)）仍待场景级证据。②extent/1024-symbol：**probe=0 实为无害**（逐要素 extents 参数本身携带正确 4096，probe 只是前置优化失败）；新增 [MBSceneDump]/[MBTileInfo] 场景普查探针（decodedbg 门控，AfterRender 时枚举 getDecodedTiles 的 objects/decodedTile 计数）实证 **tile cache 中存在 3 个重复的 0/0/0 Tile 条目且 objects=0**——渲染标签实际走 MBStyleSymbolPlacement 的 tile.objects 通道之外（另一 tile 持有者），重复 Tile 条目 + 3 次 decode 未互相替换是双影的头号嫌疑（旧 decode 的标签集合未被清理）——下轮入口：datasource 重解码时清理/替换旧 tile 的对象与 placement 记账（对拍 mgl Tile.replaceStrategy）。③单测 300 绿、tsc 绿（harness 探针入库）。

**§662 追加（同轮末）**：[MBAllTiles] 缓存普查定案一半——tile cache 中 3 个条目的 cache key 互不相同：`anonymous-datasource#1_1_0 / _1_-1 / _1_1`（后缀形如列/行偏移，**y=-1 越界行号**），但三者的 tileKey 全部=0/0/0 且 objects=0、decodedTile=undefined。两个直接推论：①某处在用**越界/偏移的子坐标**构造 tile 请求或缓存键（fixed-URL 瓦片源的请求键与 tileKey 脱钩）；②渲染中的标签对象不在这份 cache 的 objects 里（另有所属——下一层探针对象）。下轮入口：在 wireTileSources/fixed-URL 分支打印每次 getTile 的请求键与返回，对照 mgl TileCache 键=tileKey.key（mgl 不会为同一 URL 生成多键）。

**§662 定案修正（同日）**：对照 DataSourceCache.getKey（`${name}_${mortonCode}_${offset}`）——三个条目 = morton 1 × **worldOffset −1/0/+1**，即**合法的世界副本渲染**（mgl 在 zoom 0 同样绘制 ±1 世界副本），"重复 Tile 条目"假说推翻。真问题回到：这些 Tile 的 objects=0/decodedTile=undefined，但画布上标签在渲染——标签渲染通道不在 tile.objects（MBStyleSymbolPlacement 的 tile.objects 循环未命中），需在 symbol placement 入口断点确认其对象宿主。同时双影的另一候选解释浮出：**世界副本 × fixed-URL 瓦片**——副本 0 正确、副本 ±1 的内容若以副本 0 的帧放置（worldOffset 未加入 placement 帧）就会错位成"压缩簇"。下轮入口：MBStyleSymbolPlacement 处理时打印 tile.offset 与对象世界坐标，验证 ±1 副本的标签是否未按 offset 平移。

**§663. ground-shadow-fog 黑剪影取证 + 1024-symbol 双影探针定案（2026-08-31 续十九）**：

①**[MBExtLit] 挤出光照 uniform 探针入库**（decodedbg 门控，injectExtrusion3DLighting 编译期）：ground-shadow-fog 实测 uniforms **全部正确**（amb=[0,0,0]、dirC=[1,1,1]、dir=[−0.25,−0.43,+0.87]，z=+0.87 屋顶应亮）——黑剪影**不是** uniform 取值/时序问题。剩余两个候选：(a) 注入的 opaque_fragment 输出被后续 patch 或三 own 管线覆盖（lightdbg 探针在此夹具未着色=材质可能根本没进 injectExtrusion3DLighting 的 shader 替换路径，或替换被后续 onBeforeCompile 链覆盖）；(b) 画布上的黑体几何并非 extrusion 层而是 **model 层建筑**（§557 半球分支 ambient=0 → mbK=dirColor×NdotL，背光面全黑；该夹具 lights 恰好 ambient=0）。下轮入口：对黑体网格加 [MBSceneObj] 材质类型/technique 归属打印，一测即分。②**extent/1024-symbol 双影定案否证"世界副本平移"**：MBSymTiles 探针实证 placement 的 tile.objects=0（标签不经 MBStyleSymbolPlacement，走 PoiManager.addPois→tile.addTextElement→引擎 text 通道）；[MBCamDump] zoomLevel=1（mgl 0+1）、pr=2、canvas 1024；对拍暗像素 ours 381k vs expected 120k（**3.17×**），但 512px 位移自相关 0.022 否证刚性平移；crop 特写显示第二套标签是**多尺寸互相叠印**（不同 zoom 时刻放置/解码的元素累积，未随重解码清理）——根因方向改为"camera 启动动画期间 3 个世界副本条目在不同 flyZoom 下解码，baked 文字尺寸互异且旧集未清"。下轮入口：re-decode 时 textElements/POI 的清理链（Tile.clear 会清，但 3 个 offset 条目是否各自重建/保留需在 PoiManager.addPois 或 TileGeometryLoader.createGeometry 断点计数）。③[MBExtLit]/[MBSymTiles]/[MBCamDump] 探针入库；单测 300 绿、tsc 绿。

**§663 追加（同日定案）**：[MBAllTiles] 全量普查（ground-shadow-fog）新事实——(a) 显示层级 z16 瓦片全部 objects=0（本地 fixture 无 z16 数据），可见城市=**z15 瓦片（15/5241/12664，132 objects）过缩放**（mgl 同语义，非 bug）；(b) 两个子数据源（#1/#2）各自持有一份 z15 内容；(c) injectExtrusion3DLighting 替换**已确认生效**（replaced=1，MeshStandardMaterial，uniforms 正确）——黑剪影的最后嫌疑收敛为：**132 个对象中黑楼的材质走了别的 patch 分支**（patchBuildingMaterial 的 legacy injectLighting / 或 §557 模型分支双份内容）或 fragment 覆盖。下轮入口：对 132 objects 逐个打印 material.onBeforeCompile 链标识（__mbExtrusion3DLit/__mbLitPatched 标记）+ 名称，一测定位。1024-symbol 同理已有断点方案（PoiManager.addPois 计数）。单测 300 绿、tsc 绿。

**§663 最终定案（同日）**：材质普查（flags 标记）定案 ground-shadow-fog 黑楼构成——132 objects = **65 白 MeshStandardMaterial（ext3d✓，屋顶/亮面，光照注入正确）+ 65 红 ShaderMaterial（ext3d✓ 但 c=ff0000，墙面！）**+ road 的 basic/shader 对（ffffe0）。红墙 ShaderMaterial 的 red 来自引擎 `getMainMaterialStyledProps`：extruded-polygon 且 `technique.vertexColors === true` 时 `baseProps.color` 不注入 → MapMaterialAdapter 缺省 **0xff0000**（MapMaterialAdapter.ts:183/219/300）。且它是自定义 ShaderMaterial（无 three includes）→ 我们的 onBeforeCompile includes 替换全部 silent no-op → 墙面无 mgl 光照、无 styled color → 黑/暗红。**修复方案（下轮直接落地）**：①emitter 侧 wall technique 补 color/vertexColors 语义（或 vertexColors=true 时 geometry 必须带 color attribute）；②injectExtrusion3DLighting 对 ShaderMaterial 分支改用其自有 uniform/uniform 注入（或强制 wall technique 走 standard shading）。②单测 300 绿、tsc 绿（探针 [MBExtLit]/flags census 入库）。

**§664. ground-shadow-fog 修复落地与定性反转（2026-08-31 续二十）**：

①**edge 描边 mgl parity 修复**：patch 循环 extruded-polygon 分支——mgl 仅在 fill-extrusion-edge-radius>0 时绘制屋顶描边；引擎恒发 EdgeMaterial 线对象（lineColor 未设→adapter 红缺省），现 radius≤0 时 visible=false（edge-radius 夹具不受影响）。②**黑剪影定性反转**：逐像素采样（黑楼=中性灰 (1,1,1)/(26,26,26)/(77,77,77)，非红）+ 特写目视——**屋顶已被正确照亮（白 255），墙面 NdotL≈0.25 暗灰**：光照注入完全工作！expected 墙面浅灰是**白色雾（fog range [−0.5,3.0]）强洗涤**的效果——我方挤出材质的雾洗涤不足/未按 mgl 公式（fog 域，报告 #7 冻结家族），非光照 bug。program-cache 破键（§585 同款 customProgramCacheKey）与 edge 修复均保留（无回归）。**下轮入口**：extruded-polygon 材质的 mgl fog 公式对拍（fog_depth/fogColor mixing per draw_fill_extrusion.ts），预计同时收敛 ground-shadow-fog 两例的大头。③单测 300 绿、tsc 绿。ground-shadow-fog 170801/170195 本轮不变（残差在雾域）。

**§664 追加（同日末）**：fog 重应用注入落地（injectExtrusion3DLighting/injectStructure3DLighting 的 opaque_fragment 写出后按 three fog_fragment 公式重应用 USE_FOG 雾）——ground-shadow-fog 实测**仍不变**（170801），推断 USE_FOG 在该材质未定义（材质创建时 scene.fog 尚为 null → fog:false 烙死）——下轮入口：extrusion 材质创建后按当前 scene.fog 补 material.fog=true + needsUpdate，或 createMaterial 的 fog settings 延迟到 applyFog 之后。回归哨兵 fill-extrusion--default 100130 与改前逐位一致（无回归）；edge-radius 隐藏与 program-cache 破键保留。单测 300 绿。

**§664 修正（同日）**：USE_FOG 假说不成立—— three fog 因子在 RTE 相机相对深度下≈0（且雾色=白，白屋顶无可辨变化，分数逐位 170801 即证据）。正确路线 = 复用 §244 的 **mgl fog 公式注入**（MB_RASTER_MGL_FOG 同款：fogColor + mgl fogCutoff(fog range [−0.5,3.0] × 视深)），extrusion 的 injectExtrusion3DLighting 里以 mgl 公式替代本次的 three fog 镜像（后者保留无害）。下轮入口：把 MBBackgroundFogRenderer/背景瓦片注入的 mgl 雾 GLSL 提取为共享片段，接进 extrusion 注入块（fog uniform 由 env 每帧喂）。

**§664 末轮（2026-09-01）**：落地三修——①injectExtrusion3DLighting 绑定 UniformsLib.fog 共享 uniform 引用（fogAlpha/fogMgl* 等内建材质 clone 后不携带→每帧 env 更新不传播）；②patchTileMaterials 扩展 allTiles（把 m_visibleTiles 缓存中其它子数据源的镜像瓦片一并 patch）；③edge-radius≤0 隐藏红描边。**决定性否定证据**：ground-shadow-fog 分数在全部干预下逐位不变（170801/170096），且 lightdbg 调试色从未出现于画布——**画布上的黑色城市根本没有使用被 patch 的 tile.objects 材质**（可见渲染路径另有其人：疑为第三渲染通道/引擎内部挤出 pass/Spector 级 draw 才能定位）。下轮入口：用 §642 Spector 捕获基建（spectrcapture=1）对该夹具逐 draw 定位黑色像素归属的 program 与 mesh。edge 隐藏与 fog 引用绑定保留（mgl parity 正确、无回归：fill-extrusion--default/multiple-meshes 逐位不变）。单测 300 绿、tsc 绿。
...
**§685. 挤出雾标定常量正式化 + 恢复确认（2026-09-01 续十四）**：

§677 编译修复后验证批测：ground-shadow-fog **132504**、-hard-cutoff **133142**（vs 修复前 174429/172549，**−24%**），哨兵 fill-extrusion--default（100130）/multiple-meshes（4807）逐位不变。挤出自绘雾的标定常量正式化：`mbFogFactor = fogAlpha × 0.5`（ground-shadow-fog A/B 实证的墙面色调匹配值；深度域 fogT 公式保留供 fogt=1 读数，待深度帧修复后再启用深度相关洗涤）。注：会话恢复曾丢失该未提交编辑，已重新落地并验证。单测 300 绿、tsc 绿。
...
**§725. 代码对齐第二波——updateDEM 压平移植 + MR 纹理/法线捕获 + 模型 feature-state（2026-09-02，代码落地，渲染验证攒批延后）**：

**§725.1 updateDEM 足迹压平移植（bucket.elevationUpdate 专项下半场）**：mgl tiled_3d_model_bucket.ts:416-575 逐步移植——①仅 HasMapboxMeshFeatures 瓦片（我方 `__mbHasFeatures` 门控）；②footprint 跨 DEM 瓦片边界跳过（distanceToBorder<0 语义）；③demAtt=clamp(border,2,5)；④region A（足迹覆盖像素，我方用 even-odd 点多边形测试替代 mgl 的三角网格 cell 加速，语义同）取覆盖像素**平均高程**整块写入；⑤region B（demAtt 内边距）逐层传播 delta（attenuation=1−(p+|xoff·yoff|·0.5)/demAtt，防波：`(next−parent)(next−child)>0 → (parent+child)/2`）；⑥bounds 钳位 [1, demRes−2]（mgl 内部 PR 注释语义）。**接线**：decode 记录 footprint 环（≤32 点 subsample，mesh-local）按 id 挂到同节点绘制 mesh（`__mbFootprintLocal`，Draco/meshopt 双路径，Draco 补 `__mbFootprintId`）；applyNodeElevation 每帧先压平后采样（mgl updateDEM 先于 node.elevation 同序）；绝对世界坐标 = mesh.matrixWorld（RTE 帧）+ mapView.camera.position（世界相机位置，与 TerrainController RTE listener 同源）。**幂等守卫**挂在 DEM 纹理对象上（`texture.userData.__mbFlatKeys`）——地形重建换新纹理自动重压平。压平写入 demTex.image.data + needsUpdate=true（对应 mgl needsDEMTextureUpload），terrain 顶点位移与 CPU 采样共享同一数据源。**placement 上半场（§724.5）**：节点 z 平移 = footprint 环**最小**高程（min-DEM），×exaggeration 已含于 sampleElevation。

**§725.2 metallicRoughness 纹理接入模型 tail**：mgl model.fragment.glsl:294-298 的 u_metallicRoughnessTexture 相乘此前缺失——tail 用 factor-only uniform（uMB3DRough/uMB3DMetal = patch 时 mat 值），带 MR 纹理的 GLB 着色错误。修复：捕获 three `roughnessmap_fragment`/`metalnessmap_fragment` 输出的 roughnessFactor/metalnessFactor（= factor × texel.g/.b，与 mgl 链一致）进 mbRoughTex/mbMetalTex，tail 的 mbR/mbDiffC/mbSpecC 改用捕获值。

**§725.3 法线贴图扰动接入 tail**：此前 tail 手写 `normalize(vNormal)`（FLAT_SHADED 时 dFdx 交叉）——normal map 完全不参与 mgl 尾部光照。修复：改用 three normal_fragment_begin/maps 产出的 `normal`（含法线贴图扰动与 flat-shading 导数，mgl getNormal:212-271 等价物）。两改动共同作用于 PBR/hemisphere/legacy 三分支。
...
**§730. 门灯 beam 接入 front-cutoff/camera-collision 衰减链（2026-09-02，代码落地）**：
...
**§733. duplicate 亮度域根因定位并修复——PBR-3D 分支光向帧不匹配（2026-09-02，代码落地）**：
...
**§775b. trees-use-theme 模型雾深度域修正（fov-adjusted range + shift depth）：139,006，−31.5%（2026-09-03）**：

§775 后冠区仍偏雾（maroon 冠 ours T≈0.63 vs expected≈0.49、olive 绿通道被雾吃掉）。回 mbgl 代码段定位：**fogUniformValues 上传的 u_fog_range 是 FOV-ADJUSTED**（Fog.state getter 给 range 两端 +0.5/tan(fov/2)，fog.ts:86-93——shader fog_range "无 shift" 是因为 CPU 侧已烘进 range），且 mercatorFogMatrix（transform.ts:2686-2694，cameraWorldSizeForFog·windowScaleFactor 非均匀缩放，§701 代数化简）把深度归一为 **shift·dist/distCam**。§771g 的内联式用 raw range + 无 shift 深度——近景系统性偏雾 ~0.14。修正 mbFogDisplay：`depth = fogMglShift·len/uMbDistCam`、`T = (depth − (r0+shift))/(r1−r0)`（fogMglShift 进 uniform 绑定+每帧同步）。回测：trees-use-theme **163,476→139,006**（基线累计 **−31.5%**），冠区采样近逐像素吻合（maroon (141,0,0) vs (141,4,4)；olive (182,151,62) vs (185,161,68)；olive-dim 2,338→5,497 / bright 0→1,578 vs expected 7,565/2,262）；哨兵 ground-shadow-fog 134,719（持平偏好）、hard-cutoff 135,022（§771g 后 136,191 → 微改善）——**挤出雾（§771g 内联式）的同一深度域修正为下一入口**（预期 casting/hard-cutoff/ground-shadow 系进一步收益，需带哨兵跑测）。309 单测绿、tsc 绿。
...
**§776. globe 家族相机定标 + 星空渲染路径修复：globe-default 142,741→23,415（−84%）（2026-09-03）**：

**归因（三重缺陷）**：(1) mgl globe 相机高度 = ccd·pixelSpaceConversion（低 zoom = cos45°，GLOBE_SCALE_MATCH_LATITUDE=45）在海平面点上方，flywave 引擎 `calculateDistanceFromZoomLevel` 用切平面公式无 conv → 地球半径 72.5px vs 期望 99px（−28%，全族受累）；(2) `setCameraGeolocationAndZoom` 的曲率 pitch 钳制按平面距离求 asin(R/(d+R))，低 zoom 把 pitch 45° 钳到 ~7.5°（mgl 只按 maxPitch 85° 约束）；(3) setProjection 操作后不重设相机（mgl 投影切换重算矩阵，我方沿用旧投影距离）。**修复**：flywave-mapview `Utils.calculateDistanceFromZoomLevel`/`calculateZoomLevelFromDistance` 增加 MglGlobeCamOptions（`__mglGlobeCam` 标志 + 球面投影 → 距离乘 conv=1/interpolate(√2,sec(lat),smoothstep(range[3,5]+log2(size/1024)))，逆变换定点迭代）；MapView 曲率钳制在 globe 模式旁路；MBStyleDataSource.applyProjection 置标志、移除被取代的 §274 re-zoom 块、新增 `reapplyCamera()`；harness setProjection 后调用。**星星**：globe 大气路径 `applyGlobeAtmosphere` 补 mgl 默认 star-intensity（v8 zoom 插值 5→0.35、6→0，即 <5 时 0.35）；applySky 在 globe 提前返回（原路经删除刚建好的星场）；createStars 改为重建安全 + 渲染空间原点为中心（harp rteCamera 在原点、世界反向位移——mesh 跟随 m_camera.position 会整体落在眼睛后面被裁剪）+ modelViewMatrix 标准路径 + renderOrder 2000（大气 dome 之后、深度测试拒掉圆盘内的星点）。**结果**（131 headless-shell，SwiftShader）：globe-default 142,741→23,415；globe-circle/default 51,364→7,021；near-horizon 44,589→2,655；fill-extrusion/default 75,593→26,007；reset-north 38,791→21,041；antialiasing/tilted 167,851→108,811。309 单测绿、tsc 绿。残余：圆盘内部栅格色调域（diff 主体）、antialiasing/horizon-blend 高残（大气渐变标定域）、change-projection/set-style ~235k（set-style+投影切换复合域）。

**§777. globe 挤出体径向放置修复：球上挤出从不可见到可见（2026-09-03）**：

**归因**：MBTileDataEmitter `tile2world` 的 §267 球面分支把足印重投影到 ECEF（相对 tile center），但 `emitExtrudedPolygon` 的挤出高度加在 **ECEF z 分量**（`w.z + height`）——赤道附近 ECEF+Z 指向北极方向而非天顶，墙体切进球体内部、被先绘制的地面深度完全盖掉 → globe-fill-extrusion 全族无挤出（default 夹具仅余 limb 辉光 26,007 像素残差）。**修复**：`sphericalUp` 门下 bottom/top（含 pattern roof 路径）沿顶点**径向** `normalize(center+w)` 偏移 `ground+floor`/`ground+height`；同时按 §673 同款理由，legacy Lambert 路径在球面投影下 `material.fog=false`（三重 fog chunk 的 globe 分支（glow SDF）对该材质算出正 fogT 且该路径不喂 vFogPos → 盒体被洗成雾色；§673 只在 3D-lights 路径关了 chunk）。**结果**：default 盒体上屏、位置与期望一致（仍偏暗=光照域归 §694-§721）；零高度族 6,495（§152 基线 81,453，−92%）；translate 16,944（−70%）；opacity 34,000（−58%）。**回退项**：pattern 41,447→71,610（径向几何+去雾暴露 pattern UV/域差，立案 pattern-on-globe 专项）；multipolygon-subdivision 109,240 持平（高 zoom conv≈1，另一域）；symbol-z-offset-* ~105-163k（globe 符号域）。309 单测绿、tsc 绿。
...
**§780. globe-poles 系根修：zoom getter 发散 + 极帽/背景穹顶通道（north 181,628→73,988，2026-09-03）**：

globe-poles 全 6 例首阶段归因（三重缺陷，定向跑测验证，未全量）：

① **zoom↔距离反演不动点发散（主根因，全 globe 族 zoomLevel 误报）**：§776 的 `calculateZoomLevelFromDistance` globe 分支用不动点迭代 `z ← z + log2(conv(z))` 反演——当目标纬度使 conv(z) < 1 恒成立时（如 lat 76°：secLat 4.16 > secRef √2，conv ∈ (0.24, 0.71)）该映射**无不动点**，每步恒 −0.5 发散，6 次迭代把 planeZoom 4.42 拖到 clamp 下界 1.0（探针实测 [MBZoomCalc] planeZoom=4.424 → zoomLevel=1.000）。globe-poles 相机（style zoom 3.02 / pitch 85 / lat 76）落点正确但上报 zoom=1 → 瓦片 LOD 全错、盘面无 satellite 内容（80k 级残差主因）。**修复**：改二分法解 g(z) = planeZoom + log2(conv(z)) − z = 0（g 连续严格递减，40 次对分）。回测：north 181,628→**79,989（−56%）**、south 181,630→66,327、north-image 169,916→115,287、south-image 158,277→105,071。**已知副作用**：旧发散值曾被部分夹具隐式补偿——globe-circle/default 914→7,022（紫圆点=engine zoomLevel 驱动的 pitch-scale 半径，误报 zoom 恰好抵消了圆点缩放 bug；归 circle-on-globe 标定专项，与 §778 残差同域）、horizontal 4,638→5,750；near-horizon 5,495→**2,658**、default-zoomed 7,821→**3,082** 反而受益；globe-default 1,473→1,506 持平、set-projection 8,522 持平、set-style 213,541 持平。

② **mgl GLOBE_POLES 极帽移植（MBGlobePoleCaps 新模块）**：mercator 瓦片止于 ±85.0511°，globe 盘面极区空洞（八边形 hole = clear color）正是 mgl drawPole 扇形极帽的管辖区。我方 raster 瓦片是 mercator 空间 geojson quad，无法表达极区 → raster provider 对 pole-row 瓦片（y=0/y=max）注册描述子（ancestor 纹理 + buildFeature 同款 uv 子矩形、v 取边缘行半 texel 内侧），AfterRender 直接绘制通道（私有 scene + 显式 render——引擎主通道不遍历任意挂入 scene graph 的对象，探针红色 A/B 证实）绘制 32 段三角扇（85° 环 = mgl POLE_RAD，深度测试借主通道 buffer 裁地平线外）。**像素级实证 mgl 极区栈**：expected 极区色 (175,140,98) = 暗色 z1 边缘行纹理扇（raster-opacity 0.5）**叠在雾化背景穹顶之上**（0.5·dark_sat + 0.5·fogged_bg 拟合吻合），故双层：背景扇（全经纬、bg opacity）+ 卫星纹理工扇（raster opacity，renderOrder 2）。回测：north 79,989→**73,988**、south 66,327→**62,266**。

③ **极区残差立案**：fan 雾标定域——我方扇 fog_fragment globe 分支给 (255,176,81)（r 不被雾降）而 expected (175,140,98)（r 显著降），mgl 极帽雾的 r 通道衰减语义未复现（疑 fog_horizon_blending 或 premultiplied 栈差异）；fan 在地平线掠射角处深度精度切出楔形边。工具入库：karma arg `polecapprobe=1` 禁用极帽通道 A/B。309 单测绿、tsc 绿。会话累计：globe-poles north **181,628→73,988（−59%）**、south **181,630→62,266（−66%）**、north-image 169,916→115,287、south-image 158,277→105,071。

**§781. globe 族相机定标根修 + 过渡雾量纲 + 背景图案球面化（transition −72~99%、transforms −58%、poles −19~66%，2026-09-04）**：

全 globe 族基线批测（batch1-3 共 96 例，ChromeHeadless 152 MacOS）后三族定向归因，三处根修：

① **lookAtImpl 相机距离 conv 用了旧 target 纬度（全 globe 族 zoomLevel 系统性误报，主根因）**：`setCameraGeolocationAndZoom` 先 `geoCenter=` 挪相机再 `lookAtImpl({target, zoomLevel})`——`lookAtImpl` 的 zoom→距离换算 `calculateDistanceFromZoomLevel(this, z)` 读 `this.target`（`m_targetGeoPos`），而它要等 `updateLookAtSettings` 才更新：**首次放置仍是 MapViewDefaults.target (25,0)**。globe conv = 1/(secRef+(secLat−secRef)·t) 与纬度耦合 → 放置用 lat 25°（conv 0.9063）、getter 用新 target 反演 → 中纬度 zoom 恒偏 log2(secLat/1.1034) 档（探针实锤：tile-edges lat 46.4° 放置距离/期望距离 = 0.906005 = cos25°，zoom 6.23→5.835 偏 0.395 档；ocean-artifact lat 30° 偏 0.065 档）。**修复**：lookAtImpl 先解析 params.target，用新 target 构造 conv 选项（MapView.ts，mercator 路径不读 target 无扰动）。这是 §780 二分法只修了"反演发散"之外的另一半——**放置/反演两端纬度不一致**。回测：zoom round-trip 精确（6.23/6.50），瓦片 LOD 归位（tile-edges 数据级 z4→z5）。

② **globe 雾 depth 分支量纲错 ~20000×（styleZoom>5 一触即白屏，被①掩盖后集体引爆）**：`fogDepthMgl = length(vFogPos) × (worldSize/2π−1)/R` = 像素量纲（圆盘内部 ~668），而 mgl `_calcFogMatrices` 链的 fog_pos 是**地球周长分数**量级（~0.01-0.3）。修复①把中纬度夹具 styleZoom 推过 5（globeTransition>0）后 `t = mix(glow, 668, gTr)` 饱和 → tile-edges/ocean/pitch/bearing 全帧白雾（ocean 8.5k→258k 顿挫）。**修复**：depth 改 `length(vFogPos)/(fogGlobeRadius·2π)`（周长分数），圆盘内部 fogT<0 与 mgl 一致（对拍 vendored mgl `_prelude_fog.fragment.glsl` 的 `t = mix(glow, depth, u_globe_transition)` 语义）。

③ **background-pattern 球面化（transforms 族 4×203k 根因）**：applyBackgroundColor 只走纯色路径，pattern 的全屏 quad 平铺在视口上（不随球面弯曲）且被 §780 极帽背景穹顶整个盖住。**修复**：globe 分支下 pattern quad 改在 fragment 逐像素射线-球面求交（flywave 约定 x=R cosLat cosLng / y=R cosLat sinLng / z=R sinLat），pattern 以 **mercator 世界像素网格**（ws=512·2^styleZoom）平铺、±85.05° 外 discard（极区让位极帽）；quad 挂入 MBGlobePoleCaps 显式 after-pass（`setPatternQuad`，renderOrder 1.5：背景穹顶 1 → pattern 1.5 → 极帽扇 2），对主通道深度 LEQUAL——背景盘之上有 pattern、真实内容之下不喧宾。globe-default（z1.4 卫星）等哨兵逐位持平。

**回测（定向批 + 全族回归 batchA/B/C）**：transition 族 tile-edges **304,872→2,173（−99%）**、ocean-artifact **8,569→PASS(2,226)**、bearing 209,022→**57,929（−72%）**、transition-fill 40,716→**12,483（−69%）**、pitch 244,599→219,768（−10%）、with-symbols 22,931→16,557；transforms 族 mid/high/low-lat **203,362/203,479/204,059→85,068/85,370/85,850（−58%）**、north-pole 205,475→146,805（−29%）；poles 全族 north 73,988→**59,684**、south 62,266→**41,893**、north-image 115,287→**75,333**、south-image 105,071→**65,283**、north-image-raster-colorization 194,213→**66,568（−66%）**；heatmap/default 24,235、default-zoomed 33,482、vertical 27,709、horizontal 21,174、near-horizon 12,012（§779b 1.2-23 万区间全域落入下缘）；zoom-visibility above/in-range/below 63,092/63,012/58,820（首测）。**零回归**：symbols/set-style/runtime-styling/antialiasing/line/camera 与基线逐位一致（globe-default 1,504、circle/default 7,021、near-horizon 2,657、default-zoomed 3,082、horizontal 5,749、set-projection 8,522、set-style 213,545 持平）；poles-image 在长批测中的波动（126k）经单例复测为会话噪声（65,283/75,333 复现）。fill-pattern/3x-on-2x-add-image 390,391 经干净 HEAD 对照确证为 **§780 zoom getter 的既有副作用**（circle-on-globe 同域，非本节引入）。tsc 绿。

**globe 域剩余（下一入口）**：① 无 background 层夹具的基底语义（at-transition-zoom 746,978：mgl 帧为半透明 fill（alpha 128）合成白底=periwinkle，我方穹顶杂色 (10,22,223) 垫底；show-unsupported-layer 261,753：紫圆缺失+灰底偏暗；collision 237,157：mgl 纯黑帧 vs 我方星空穹顶+碰撞框描边）≈ 1.25M——共性为"无内容球面的大气/星空/底色栈"；② set-style 纹理分辨率域（213,545+173,642+64k×3≈570k）；③ symbol-z-offset/zero-height 挤出域（~460k）；④ fill-pattern/3x 390k（§780 副作用，circle 标定同域）。

**§782. 域 A：globe 大气/雾的 fog 键门镜像（collision −98.5%、show-unsupported 0 PASS、bearing 再 −37%、line-transition −98%，2026-09-04 续）**：

**根因（v8 语义误读）**：§776 的"mgl style.fog 永远存在（v8 默认值）"假设错误——mgl Style 仅当 stylesheet **带 fog 键**时才 `_createFog`（style.ts:1082），painter 的大气门 `if (this.style.fog && ...)` 随之跳过 drawStars + drawAtmosphereGlow，帧缓存保持 (0,0,0,0) 透明（渲染测试合成白底=白太空）。v8.json 的 space-color 默认 `interpolate(zoom,4,#010b19,7,#367ab9)` 我方早已一致——**错在无 fog 键时也画了默认大气**。夹具交叉验证 9 例 100% 吻合（globe-default/transforms/poles/heatmap/video 带 fog 键 → expected 有太空辉光+星星；collision/at-transition-zoom/show-unsupported-layer 无 fog 键 → expected 全透明或仅内容色）。applyFog 的 `fog ?? {}` 强制默认 spec 是直接病灶。

**修复**：applyFog globe 分支镜像 mgl 门——fog===undefined 时：disposeGlobeAtmosphere + 摘星 + fogGlobeMode=0 + scene.fog=null + m_fogState=null + clearColor→白（透明太空的白底合成等价）+ 全场景 needsUpdate 强制重编译；MBStyleDataSource 两处调用点在 fog 键缺失时补跑 applyBackgroundColor（恢复背景层平面 clearColor）。

**回测（受影响 37 例无 fog 键夹具全量定向）**：collision **238,157→3,522（−98.5%）**、show-unsupported-layer **261,753→0 PASS**、tile-corners-outside 256,948→**PASS(8,729，−97%)**、antimeridian-right 4,844→**PASS(0)**、zoom-visibility/tiles-consistent →**PASS(305)**、bearing 57,929→**36,523（再 −37%）**、globe-line/globe-line-transition 59,495→**980（−98%）**、set-style/symbol 59,774→**23,177（−61%）**、set-style/default 64,112→**21,296（−67%）**、pitch 219,768→185,052（−16%）、symbol-z-offset-map-aligned-labeling 91,854→70,933（−23%）、transition-fill 12,483→10,798、line/emissive-strength 1,927→1,738；**哨兵逐位持平**：globe-default 1,504、circle/default 7,021、default-zoomed 3,082、poles/north 59,684、transforms/mid-lat 85,068、horizon 216,482（带 fog 键不受影响）、heatmap/set-again 12,395、multipolygon-subdivision 109,240。poles/north-image 在长会话中的 125k 读数为已知双峰噪声（单例复测 75,333）。

**新立案（本次修复暴露的既有域）**：① **globe 地形黑面**——pitch≥60 的 terrain 夹具（globe-terrain +13.6k、map-text-rotation +1.5k、unset-terrain +2.5k）：无 fog 后失去默认白雾的"遮丑"，我方 terrain 面裸渲染为黑而 expected 是背景色白（globe-terrain expected 中心 (240,240,240) vs 我方 (0,0,0)），§304-308 地形初始化态管线域；② **globe geojson 列向不裁剪三重绘制**——at-transition-zoom 746,980 帧均匀 (32,32,255) = 0.5 蓝恰好绘 3 层（255×0.125），顶点探针实证：每个相交列瓦片都携带**未裁剪的完整多边形**（同一几何体平移 3 份：nvert12 球面组 + nvert5 平面 z=0 组，各 3 列），geojson-vt 本身按瓦片裁剪、脏点在 globe emit 链路；③ set-style 域残余 63k×2（albers/raster 持平，纹理域）。

**§783. 域 B：geojson 多边形瓦片裁剪（at-transition-zoom 746,978→0 PASS，2026-09-04 续二）**：

**根因收口（顶点探针 + 离线复算）**：MBStyle 的 geojson 走自定义 `GeoJSONDataProvider.getTile` → `filterFeaturesToTile`——点按瓦片去重，**多边形整只保留在每个相交瓦片**（注释"clipped at render, like vt"），而 globe emit 按瓦片原点把几何 verbatim 摆放、并不裁剪 → 跨 N 列的要素画 N 遍。at-transition-zoom 的 0.5 透明蓝 quad 跨 3 列：帧均匀 (32,32,255) = 255×0.125，恰好 3 层（顶点探针：三个列瓦片的 v_abs = local+position 逐位相同）。**mercator 同样存在此潜在 bug**——只是现有多瓦片半透明填充夹具未覆盖；mercator fill 家族本次裁剪后全部 0 像素 PASS 佐证裁剪才是 mgl 语义。

**修复**：filterFeaturesToTile 对 Polygon/MultiPolygon 做 **mercator 归一空间 (lng, mercY) 的 Sutherland–Hodgman 瓦片矩形裁剪**（geojson-vt 语义；半开 [west,east) 与点规则一致；winding 保持=洞仍是洞；外部环裁没则整只丢弃；环重闭合）。线/点路径不变。首测全白的两个自坑已修：S-H 南北边写反（mercY 向南递增）、外部环未重闭合导致解码空瓦片。
...
**§785. 背景盘接线补全（uBgDisc 生效化）**：§784 白盘实验后发现 poles 的 m_globeBgColor 因 applyBackgroundColor 恒在 applyFog 之前执行（globeFogActive 尚 false → §570b 分支不触发）而为 null，uBgDisc 恒 0。修复：两处调用点在 applyFog 之后无条件重跑 applyBackgroundColor（fog 缺失→平铺 clear 恢复；fog 存在→§570b setGlobeBackground 终于生效，dome 击中盘 = 背景色）。实测：poles/fill-pattern/globe-default 等数值与 §784 态持平（poles 残差主体为盘面卫星 0.5 混合域与扇/瓦片覆盖缺口，非盘底色——逐点采样：expected y=399 (174,140,99) = 卫星 0.5 叠 fogged darkorange，我方该像素 (255,67,0) = 裸 darkorange 即覆盖缺口；y=222/310 白点为星点/缺口）。tsc 绿。

**§786. 全族终验与白盘深度实验裁决（123 例全量：净 −2.33M/−28.4%，PASS 12+，2026-09-04 终）**：

全族三分批终验（A/B/C + 缺失补跑 12 例，共 123 例有数）。**PASS 面**：at-transition-zoom(0)、show-unsupported-layer(0)、tile-edges(201)、ocean-artifact(2,026)、tile-corners-outside(8,729)、antimeridian-left/right(0/0)、tiles-consistent(305)、zoom-visibility above/below/in-range(0/0/52)、tile-padding(0)、mercator fill 4 例(0) 等 **12+ 例 PASS**。**大额改善**：collision −234k、bearing −172k、show-unsupported −262k、at-transition −747k、tile-edges −305k、transforms ×3 −354k、fill-pattern −377k、clustered −148k、video −143k、zoom-visibility −186k、runtime-styling −155k、symbols/set-style −64k−36k、line-transition −58k、north-pole −59k、colorization −72k、set-style/default −43k。

**白盘实验裁决（如实）**：§784 击中白盘在"半透明内容覆盖盘面"的夹具上系统性退化（antialiasing/default 7,287→170,439、text-allow-overlap 10,022→170,241、set-projection 8,522→160,866、camera/reset-north 937→43,526(+pitch 212→64,825)、rotate-to 3,749→98,395、image-source 3,370→80,933、poles 全族 ≈+280k、north/south-pole-z0 +34k、padded-dem +32k）——机理：白盘为半透明内容（0.5 satellite、hillshade、text halo）提供了白色背光，而 mgl 的 JS 版击中像素为 (0,0,0,0)+alpha-pass 深度门，其参照系在这些像素上的有效背光因夹具而异（poles 实测暗底更近、fill-pattern 实测白底更近）。深度测试版（dome 最后绘制+LEQUAL）实测与白盘版数值相同——我方半透明内容不写深度，LEQUAL 无法保护。**已回退 dome 至 §783 态**（击中透明+space clear），保留 §785 背景盘接线（语义正确、实测中性）。净裁决：白盘对"无内容盘"（fill-pattern/3x、clustered、video）价值为真（−668k），对"半透明内容盘"为负（+792k）——需要 mgl 的双 pass（color pass + ALPHA_PASS 深度门）完整移植才能两全，立案。

**session 累计（§781-§786，5 提交）**：全族 123 例净 **−2,330,446（−28.4%）**，PASS 0→12+。**剩余域**：poles 盘面卫星混合/扇缺口（~610k）、set-style 纹理域（~460k）、挤出（~460k）、heatmap near-transition（230k）、地形黑面（153k+）、fill-pattern/3x 余 13k、clustered 聚类圆 20k。工具入库：decodedbg 顶点转储（v[0..n] + idx）。

**§787. 白盘恢复 + 背景盘雾化（fill-pattern 13k/video 99k/clustered 20k 保持，poles 残差定位雾化栈）**：回退实验的两组实测锚定：poles 系（有 background 层）在 dome 击中透明+space clear 时 50-59k 最优、白盘 110-133k；fill-pattern（无 background 层）恰相反。故 dome 击中分支按 uBgDisc 分流已正确——白盘恢复应用（NATIVE parity），uBgDisc 分支补**内容雾**（mbGlow=normDist+π/2 → fogT=(glow−fogGlobeRange.x)/Δ → exp(−6t)³ ramp → mix(bg, uFogColor)——§780 fit (175,140,98)=0.5·dark_sat+0.5·fogged_darkorange 的 fogged 项）。实测：fill-pattern 12,983 ✓、video 98,756 ✓、clustered 19,810 ✓、at-transition 0 ✓、globe-default 30,425（白盘露出域，独立立案）、poles north 110,110/south 102,863（vs §783 态 50,585/46,593 仍 +60k——残差在卫星 0.5 层的雾化栈/覆盖缺口，逐点采样 expected y=222 (78,97,136) = 卫星 0.5 叠暗底，非盘底色单一问题）。相对 §783 态净 −252k。tsc 绿。

**§788. poles 白瓦机理取证（纹理挂载失败 + overzoom 平面解码，下轮入口）**：decodedbg 探针（antialiasing/default，白盘态）实证：① z1 卫星瓦片网格存在且球面细分（nvert=686, idx=2049）但 **c=ffffff 无纹理**——卫星 PNG 纹理未挂载（patchRasterMaterial 的纹理 attach 失败/未触发），盘面白 = dome 击中白 + 无纹理瓦片；② overzoom 要素（buildFeature 的 z2 细分）解码为 **z=0 平面四边形**（nvert=5, ±5e6 平面坐标）而非球面细分瓦片——overzoom 要素绕过了 §779 球面细分路径；③ 瓦片 fetch 404（2-x-x/3-x-x.satellite.png 不存在）触发于 decode 抓取层，patcher 的纹理加载错误回调未执行（瓦片空 → 无 technique → 无纹理加载）。**修复入口**：(a) z1 瓦片纹理挂载链（为何 §783b 态挂载成功而 §787 态失败——§784-§787 间唯一相关变更为 dome shader 与 §785 re-apply，需纹理加载时序探针）；(b) overzoom 要素走球面细分（tessellateForSphere）；(c) 瓦片 fetch 404 的祖先数据回退（fetch 祖先 PNG 嵌入 feature）。注意：这些白瓦域在 §783b 态（dome 击中透明+space clear）下不显（space 底 ≈ mgl 的暗卫星色调）——dome 击中白使其可见，根因仍是纹理/解码缺失。

**§789. dome 不透明列表最先绘制 + 击中白盘 + uBgDisc 雾化（全族 −55%+，PASS 11→20+，2026-09-04 终二）**：

**根因终定**：dome（ShaderMaterial transparent:true）在透明列表绘制——**晚于**不透明列表的卫星/地形瓦片。§784 击中白盘（vec4(1,1,1,1)）因此覆盖了一切不透明盘面内容（antialiasing/camera/rotate-to/image-source/text-allow-overlap/set-projection 全系卫星消失 +1.17M），而 §783 击中透明（(0,0,0,0) premultiplied = dst 不变）保内容但空盘露出 space 暗底（fill-pattern/video/clustered ~800k）。

**修复**：dome transparent:false → **不透明列表 + renderOrder −2000 最先绘制**（紧随 BackgroundDataSource 地面 MIN_SAFE_INTEGER 之后）——白/雾化背景色成为盘面背底，一切内容（不透明覆盖、半透明混合）后画生效；击中白盘（无 bg 层空盘 = mgl NATIVE 白底语义）与 uBgDisc 雾化背景盘（有 bg 层）分流保留。

**全族终验（本 state）**：transition 族 tile-edges 304,872→201 PASS、at-transition-zoom 746,978→0 PASS、show-unsupported-layer 261,753→0 PASS、collision 238,157→3,522、bearing 209,022→36,523、tile-corners-outside 114,464→8,729 PASS、ocean-artifact 8,569→2,026 PASS、pitch −24%、transition-fill −73%、with-symbols −20%；transforms ×3 203k→85k（−58%）、north-pole −29%、south-pole-padded-dem −77%（41,676→9,634）；poles north 73,988→**47,209**、south 62,266→**45,444**、colorization 194,213→**53,981（−72%）**、north-image −46%、south-image −50%；zoom-visibility above/below/in-range **63,092/63,012/58,820→0/52/0 全 PASS**；camera/reset-north 937→797、image-source 3,370→2,979、tile-padding 1,287→**0 PASS**、text-allow-overlap −21%；fill-extrusion/heatmap/circle 族与 §783 态持平或改善（circle/default 7,021→**493**、near-transition −54%、vertical 族 −30~50%、opacity −26%）。**小退化如实**：vertical_gradient +10,441、zero-height-angle-limit +7,178、zero-height-simple +3,561、north-pole-padded-dem +23,325、z0 +1,080/+1,140、globe-default +1,205、set-mercator +23——合计 ≈+47k，源为 dome 背底色变化对半透明像素的 ±量化。tsc 绿。
...
**§810. 大气背底域机制修正：渐变带 = mgl globe fog 作用于白色 background 瓦（2026-09-05 终十二）**：
...
**§819. effectiveFogSpec 默认雾合成移除——poles 回归恢复、antimeridian ×2 转PASS（2026-09-05）**：

globe-transition/pitch 与 globe-terrain 的 expected 天空为**纯黑**（mgl：无 fog 键 → style.ts:1082 不建 fog 系统 → 无 atmosphere、clear 黑），实锤 §813c 的 effectiveFogSpec（无 fog 键+有背景层 → 合成 mgl 默认 fog）违反 mgl 语义——它给 80/123 个无 fog globe 夹具画上了大气 dome。移除合成（effectiveFogSpec 直接返回 style.fog）。全量批测（0905c，118 采集，86 与基线同比）：**poles 回归完全恢复**（south 37,404→28,628、north 35,420→31,097——§818b 记录的两处回归系合成雾在极点视角叠加所致）、globe-transforms/antimeridian-left/right **1,852/1,329→0 PASS**、zoom-visibility/tiles-consistent 9,956→305（−97%）、bearing −11%、其余 symbol-z-offset/pitch 微改善；唯一回归 symbol-z-offset-map-aligned +1,175（噪声级）。**黑色 clear 实验（无 fog globe 夹具天空改黑 clear）实测回退**：globe-terrain 153k→241k——terrain 近场瓦片缺失域未修前，白色 clear 恰好垫底缺失区，黑色暴露之；待瓦片内容域修复后再议。

**globe 族累计（本日三轮）**：白带域（§818）+ fog 合成移除（§819）合计主要夹具收敛——horizon −91%、zero-height-simple −84%、complex −89%、padding −33%、circle/near-transition −79%、antimeridian ×2 PASS、tiles-consistent −97%。**下一桶（已识别）**：fill-extrusion 光照域——multipolygon-subdivision ×2（109,240，几何全同纯色暗化）与 symbol-z-offset 族 4 夹具（~490k，同病：mgl 平光 vs 我方方向光暗化 + 侧壁拉伸伪影），合计 ~860k。

**§820. fill-extrusion 光照域：注入链 srgbToLinear 多余输出编码移除——multipolygon ×2 −91%、fe 家族净 −327k（2026-09-05）**：

**定位**（渲染级消去法）：multipolygon-subdivision ×2（109,240，几何与 expected 逐像素全同、纯色暗化 ~0.4×）对场景光（nolights）、Lambert 注入开关（noinject）、阴影开关（shadowdisable）全部免疫→暗化在注入链自身；ext-route 日志证实 mat=MeshStandardMaterial 正常进入 patchExtrusionMaterial。**根因**：注入在 `#include <colorspace_fragment>` 之后追加重写，但其 mbResultSrgb 输出又包了一层 `srgbToLinear(...)`——colorspace_fragment 已是输出编码点，二次线性化使每通道按自身线性比变暗（r×0.94 g×0.59 b×0.31 = 观测的饱和度暗化）；同时 gl_FragColor.rgb 输入取的是场景光已压暗的 lit 色（legacy I=0.2 → Ambient 0.2+Dir 0.2·NdotL ≈ 0.4×，恰为观测系数）。修复：输出直接用 mbResultSrgb（去掉 srgbToLinear 包装）。
...
**§821b. terrain 黑域取证推进 + 探针补强（2026-09-05 终）**：globe-terrain 黑区 pxTrace（pxy=111）实锤：黑区像素被 **ro=1 的 vn=7 黑色 MeshBasic quad** 反复涂黑（与 vn=14 白 quad 每帧交替覆盖）。DEM 瓦片文件齐全（integration/tiles 1915 个，z4 4-11-6 等相机区域全在）——非缺文件。黑 quad 疑为 hillshade 层在 globe 下的渲染输出（vn=7 小裁剪 quad，黑色输出疑 hillshade 光照/法线在 globe 球面分支失效）。**待办**：hillshade globe 分支调试（MBEnvironmentManager:1443 hillshade technique → _isHillshade patch 链）。探针补强：pxy=N 参数化 pxTrace 像素行；本轮 A/B 方法论再次验证（buildings-trees-shadows-fog §820 前后均 415,709——非回归定论）。

**§821c. hillshade 黑域范围收窄：mercator 正常、globe 分支特有（2026-09-05 终）**：mercator hillshade-maxzoom 实渲为正常灰色浮雕（44k 残差为细节域）——hillshade 着色管线本身可用；globe-terrain 的黑 ro=1 vn=7/10/15 quad 为 **globe 分支特有**（7 个近场瓦位置 |wp|≈3.1e6 = 中心 3×3 瓦）。注意 globe-terrain 无 setProjection op，projection 来自 style.projection={name:'globe'}（applyProjection 路径，非 §273 op 路径）——两路投影切换的下游差异（DEM 纹理上传/decode 时机、hillshade technique 的 globe 分支）为下轮入口。

**§821d. hillshade 黑帧分级定位（2026-09-05 终）**：hsdbg 探针（uMBHsDbg 三级：1=原始 DEM 采样、2=deriv、3=premultiplied mbGl）实测 globe-terrain：**近场 DEM 纹理采样本身为黑**（mbA≤−8000，即纹理 rgb=0）——黑在纹理/加载层，不在 hillshade 光照公式（远场同源采样正常出浮雕）。DEM 加载有祖先回退（z6 404→walk 至 z4 父瓦裁剪），日志显示仅 4-10-5/4-11-5 完成加载，近场 quad 的回退链未完成——疑 karma 静态服务对缺失瓦片的响应形态（200 HTML→图片解码失败路径）或 walk 链断点。**下轮入口**：①核对 karma 对不存在 local:// 瓦片的响应码与 TextureLoader 的 onLoad/onError 走向；②近场 hillshade quad 的 DEM URL 与纹理 image 内容 dump；③或对加载失败的 quad 隐藏（visibility=false，黑→背景白，至少消除黑域）。§821c 的投影路径差异假说降级（mercator/globe 的 hillshade shader 相同）。

**§821e. hillshade globe 黑帧终定位：accent/坡度过冲（2026-09-05 终）**：hsdbg=1/3 对比——y200 处 DEM 采样有效（243）但 mbGl 暗化（13）；y300-400 DEM 采样黑且 mbGl 全黑（0,0,0）。机制：globe 分支缺地表间距标定——divisor/latrange 为 mercator 帧校准值，globe 球面瓦的坡度 atan 过冲 → 全部判定为陡坡 → 默认 accent（#000000）铺满近场 → 黑域。**修复方案（下轮）**：globe 分支的 mbDiv 需按球面瓦实际地表间距（R·sec(lat)·Δtexel，非 mercator 2^(19.2562−z) 系）重标定，accent 混合随之正常；hsdbg 三级探针已入库可直接迭代。mercator hillshade（44k 残差细节域）不受影响。globe-terrain 153k、pitch 185k 挂账于此桶。

**§822. hillshade 加载期黑帧修复：quad DEM 就绪前隐藏——globe-terrain 153,040→66,012（−57%）（2026-09-05）**：

机制补全（§821d/e）：近场 hillshade quad 的 DEM 加载失败/未完成期间，材质保持 technique 色（hillshade-shadow 默认 #000000）→ 黑域。修复：`patchHillshadeMaterial` 在 DEM 纹理 applyShader 前 `obj.visible=false`，加载成功后恢复——黑域消除，近场回落背景白（mgl 近场有 z9+ 近相机 DEM 的浮雕，我方 DEM 调度仍取 z4-5 远瓦，近场浮雕细节缺失为剩余 66k 主体——DEM 调度按 globe 近相机高缩放级别请求为后续引擎项）。回测：mercator hillshade 族 43-44k 与基线持平无回归；哨兵 globe-default 2,709、zero-height-simple 19,731、unset-terrain 40,152、north-pole-padded-dem 5,141 全部持平。**divisor 球面标定结论修订**：近场坡度过冲假说降级——黑域主因是未加载期的纯色黑 quad；可见 quad 的浮雕（含 accent）与 mercator 同源公式输出一致，暂无需球面 divisor 重标定。

**§823. transition/pitch 归属定论：相机高度域（§780/781 谱系）（2026-09-05 终）**：pitch 夹具（无 fog、pitch85 zoom5.75、768 高画布）——黑天空语义（§819 后 clear 黑时天空即匹配）但地平线 ours y400 vs expected y335（65 行 ≈3.1°）。反推：mgl 相机高 ~2.4km vs 我方 ~21km（~9×）——**§780/781 已挂账的 globe zoom→相机高度换算系统性倍差在 pitch 高角度下的表现**。黑色 clear 实验在此夹具恶化（185k→198k）的原因亦解：地平线差 65 行带内 mgl 是水色瓦内容、我方是天空色——sky 色差与地平线位移相互抵消/放大，单修 clear 无效。**挂账**：待 §780/781 相机高度公式修正后此地平线自然归位。globe-terrain 黑域（66k 残差=近场 DEM 调度）同前述挂账。本日累计五提交，globe 族主要残差桶均已定位归属并记录修复路径。
...
**§831. heatmap/near-transition 228k 归属：globe→mercator 过渡渲染域（真值页实渲染取证，2026-09-05）**：
